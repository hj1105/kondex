...
internal static class KondexCliLauncher
{
    private static int Main(string[] args)
    {
        try
        {
            string launcherDirectory = Path.GetDirectoryName(typeof(KondexCliLauncher).Assembly.Location);
            ...
            string electronPath = Path.Combine(appDirectory, "Kondex.exe");
            ...
            {
                Console.Error.WriteLine("Unable to locate Kondex.exe next to \"{0}\"", resourcesDirectory);
                return 1;
            }

            if (!File.Exists(cliPath))
            {
                Console.Error.WriteLine("Unable to locate the Kondex CLI entrypoint at \"{0}\"", cliPath);
                return 1;
            }
            ...
            string requestedCliCommand = Environment.GetEnvironmentVariable("KONDEX_CLI_COMMAND")
                ?? Environment.GetEnvironmentVariable("ORCA_CLI_COMMAND");
            string cliCommand = requestedCliCommand == "orca" ? "orca" : "kondex";
            Environment.SetEnvironmentVariable("KONDEX_CLI_COMMAND", cliCommand);
        ...
        {
            Console.Error.WriteLine("Unable to start the Kondex CLI: {0}", error.Message);
            return 1;
        }
    }
