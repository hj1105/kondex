    ...
    private func buildSnapshot(
        ...
        guard accessibilityTrustedSettled() else {
            ...
            throw ProviderError.coded(
                "permission_denied",
                "Accessibility permission is required for Kondex Computer Use. Run `kondex computer permissions` or open Settings > Computer Use, grant Accessibility to Kondex Computer Use, then retry."
            )
        }
        ...
        let screenshotStatus: ScreenshotStatus = if screenshot != nil {
            ...
            .failed("Screen Recording permission is required for Kondex Computer Use; grant permission or pass --no-screenshot to inspect accessibility state only.")
...
private func focusedWindow(appElement: AXUIElement, app: AppDescriptor, visibleWindowCount: Int, allowRecovery: Bool) throws -> AXUIElement {
    ...
    if visibleWindowCount > 0 {
        ...
        throw ProviderError.coded("permission_denied", "app '\(app.name)' has visible windows but no accessibility window (AX reads stayed blocked for \(outcome.waitedMs)ms after retries). macOS Accessibility may need Kondex Computer Use toggled off and on again in System Settings.")
    }
...
private final class PermissionWindowController: NSWindowController {
    ...
    convenience init(initialPermission: PermissionKind? = nil, terminateWhenDragAssistantCloses: Bool = false) {
        ...
        window.title = "Enable Kondex Computer Use"
...
private enum PermissionKind: CaseIterable {
    ...
    var dragInstruction: String {
        switch self {
        case .accessibility:
            "Drag Kondex Computer Use into the list above to allow Accessibility."
        case .screenshots:
            "Drag Kondex Computer Use into the list above to allow Screenshots."
        }
    }
...
private final class PermissionView: NSView {
    ...
    private func build() {
        ...
        let titleText = checking
            ...
            : (ready ? "Computer Use is Ready" : "Enable Kondex Computer Use")
...
private final class PermissionDragAssistantController: NSWindowController {
    ...
    convenience init(
        ...
        window.title = "Drag Kondex Computer Use"
...
private final class DraggableAppTile: NSView, NSDraggingSource {
    ...
    private func build() {
        ...
        let title = NSTextField(labelWithString: "Kondex Computer Use")
...
private func runStdio() {
    fputs("Kondex Computer Use provider must be launched by Kondex in app-agent mode.\n", stderr)
    exit(13)
}
