...
function setRichStatus(
  tab: TerminalTab,
  args: { agentType?: 'claude' | 'codex'; prompt: string; at: number }
): void {
  tab.agentStatus = {
    state: 'working',
    prompt: args.prompt,
...
describe('session-tabs rich-status boundaries', () => {
  ...
  it('does not renew a previous task across a done-to-working title boundary', () => {
    ...
    emitTitle(harness, '⠋ Codex', firstWorkingAt)
    ...
    emitTitle(harness, '⠙ Codex', nextWorkingAt)
  ...
  it('preserves the same task across a neutral title', () => {
    ...
    emitTitle(harness, '⠋ Codex', workingAt)
    ...
    emitTitle(harness, '⠙ Codex', resumedWorkingAt)
    expect(latestStatus(harness)).toMatchObject({
      state: 'working',
      prompt: 'current task',
  ...
  it('does not carry a stale question into the next title interval', () => {
    ...
    emitTitle(harness, 'Codex - action required', firstWorkingAt)
    vi.advanceTimersByTime(1)
    emitTitle(harness, 'bash')
    harness.publications.length = 0
  ...
  it('keeps a retained OSC question visible under a shell title until the next agent interval', () => {
    ...
    harness.runtime.onPtyData(
      PTY_ID,
      '\x1b]9999;{"state":"waiting","prompt":"current question","interactivePrompt":"question-payload","agentType":"codex"}\x07',
      Date.now()
    )
  ...
  it('does not renew rich status from a replaced provider generation', () => {
    ...
    emitTitle(harness, '⠙ Codex', replacementWorkingAt)
    expect(latestStatus(harness)).toMatchObject({
      state: 'working',
      prompt: '',
  ...
  it('keeps invalidation across an identity-only owner change', () => {
    ...
    emitTitle(harness, 'Codex', identityChangeAt)
    ...
    expect(latestStatus(harness)).toMatchObject({
      state: 'done',
      prompt: '',
      updatedAt: identityChangeAt,
      stateStartedAt: identityChangeAt
    })
    harness.unsubscribe()
  })
})
