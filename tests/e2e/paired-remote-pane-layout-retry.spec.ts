...
async function readHostLayout(
  ...
  const tab = snapshot.tabs.find((tab) => tab.type === 'terminal' && tab.parentTabId === hostTabId)
  return tab?.type === 'terminal' ? (tab.parentLayout ?? null) : null
}
