...
test.skip(!isPairedBrowserRun, 'Run with ORCA_E2E_WEB_CLIENT=1 and an E2E web build')
...
async function createPairingOffer(hostPage: Page): Promise<RuntimePairingOffer> {
  return hostPage.evaluate(async () => {
    const offer = await window.api.runtimeEnvironments.createLocalAccessLink({
      address: '127.0.0.1',
      rotate: true,
      reach: 'this-computer'
    })
...
async function openPairedClient(
  ...
  // Paired clients own their locale instead of inheriting the host test profile.
