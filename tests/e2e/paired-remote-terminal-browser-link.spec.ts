...
async function readTabInventory(
  ...
  return page.evaluate(
    async ({ environmentId, worktreeId }) => {
      ...
      function readTabs(result: unknown): { type?: unknown }[] {
        if (
          !result ||
          ...
          throw new Error('host tab inventory returned an invalid result')
        }
        return result.tabs.map((tab: unknown) => {
          if (!tab || typeof tab !== 'object') {
            throw new Error('host tab inventory returned an invalid tab')
          }
          return { type: 'type' in tab ? tab.type : undefined }
        })
      }
      const sessionTabs = readTabs(sessionResponse.result)
      const browserTabs = readTabs(browserResponse.result)
      return {
        ...
        hostAuthoritativeBrowserTabs: sessionTabs.filter((tab) => tab.type === 'browser').length,
        hostRegisteredBrowserPages: browserTabs.length,
        hostTerminalTabs: sessionTabs.filter((tab) => tab.type === 'terminal').length
      }
    },
