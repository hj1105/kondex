...
type OrcaTestFixtures = {
  ...
  sharedPage: Page
  orcaPage: Page
  // Historical name: true seeds a stable migrated profile, while fresh-profile
  // goldens opt out to exercise the zero-project landing path.
  dismissOnboarding: boolean
  // Why: most E2E specs need a ready project before assertions start. Golden
  // first-run specs opt out so they can prove the zero-project landing path.
...
export const test = base.extend<OrcaTestFixtures, OrcaWorkerFixtures>({
  ...
  electronApp: async (
    ...
    if (dismissOnboarding) {
      // Seed the stable migrated profile used by the bulk of the suite.
      writeFileSync(
  ...
  // Default: use the stable migrated profile; first-run goldens opt out.
