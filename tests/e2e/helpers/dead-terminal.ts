...
export async function removeWorktreeViaStore(page: TestPage, worktreeId: string): Promise<void> {
  await page.evaluate(async (id) => {
    try {
      const state = window.__store?.getState()
      const matches = state?.allWorktrees().filter((worktree) => worktree.id === id) ?? []
      // A bare locator must not choose between worktrees on different hosts.
      if (!state || matches.length !== 1) {
        return
      }
      const worktree = matches[0]!
      await state.removeWorktree(
        { id: worktree.id, executionHostId: worktree.hostId ?? null },
        true
      )
    } catch {
      /* best-effort */
