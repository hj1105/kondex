...
async function importPairThenDeleteAlias(
  ...
  await expect(picker.getByRole('button', { name: 'Add all 2 to Kondex' })).toBeEnabled()
  await picker.getByRole('button', { name: 'Add all 2 to Kondex' }).click()
  await expect(page.getByText('Added 2 hosts to Kondex.')).toBeVisible({ timeout: 15_000 })
...
test.describe('SSH config host import (bulk + settings re-adopt)', () => {
  ...
  // ── P5 ─────────────────────────────────────────────────────────────
  test('P5: already-in-Kondex badge, disabled row, and Add all counts only new hosts', async ({
    electronApp,
    orcaPage
  }) => {
    ...
    await expect(alphaRow.getByText('In Kondex', { exact: true })).toBeVisible()

    await expect(bravoRow).toBeVisible()
    await expect(bravoRow).toBeEnabled()
    await expect(bravoRow.getByText('In Kondex', { exact: true })).toHaveCount(0)

    await expect(picker.getByRole('button', { name: 'Add all 1 to Kondex' })).toBeEnabled()
    await expect(picker.getByRole('button', { name: 'Add all 2 to Kondex' })).toHaveCount(0)
  })

  // ── P6 ─────────────────────────────────────────────────────────────
  test('P6: Add all N to Kondex imports new hosts; re-open shows all in Kondex', async ({
    electronApp,
    orcaPage
  }) => {
    const hosts = await seedPairConfig(electronApp, HOST_PREFIX)
    ...
    await picker.getByRole('button', { name: 'Add all 2 to Kondex' }).click()
    await expect(orcaPage.getByText('Added 2 hosts to Kondex.')).toBeVisible({ timeout: 15_000 })
    ...
    await expect(
      configHostRow(reopened, hosts.alpha).getByText('In Kondex', { exact: true })
    ).toBeVisible()
    await expect(configHostRow(reopened, hosts.bravo)).toBeDisabled()
    await expect(
      configHostRow(reopened, hosts.bravo).getByText('In Kondex', { exact: true })
    ).toBeVisible()
  ...
  test('P7: Add all does not re-adopt deleted config hosts (suppress tombstones)', async ({
    ...
    await expect(alphaRow.getByText('Removed from Kondex', { exact: true })).toBeVisible()
    await expect(alphaRow.getByText('In Kondex', { exact: true })).toHaveCount(0)
    await expect(configHostRow(picker, hosts.bravo)).toBeVisible()
    await expect(
      configHostRow(picker, hosts.bravo).getByText('In Kondex', { exact: true })
    ).toBeVisible()
    await expect(picker.getByRole('button', { name: 'No new hosts to add' })).toBeDisabled()
    await expect(picker.getByRole('button', { name: /Add all \d+ to Kondex/ })).toHaveCount(0)
