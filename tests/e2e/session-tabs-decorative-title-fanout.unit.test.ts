...
type RuntimeInternals = {
  ...
  ptysById: Map<string, { launchAgent: 'claude' | 'codex' | null; lastOscTitle: string | null }>
...
describe('real PTY decorative session-tabs fanout', () => {
  ...
  it('bounds host publication and renderer application across remote worktrees', () => {
    ...
    for (const ptyId of ptyIds) {
      runtime.onPtyData(ptyId, '\x1b]0;⠋ Codex\x07', Date.now())
    }
    ...
    for (let cycle = 0; cycle < 4; cycle += 1) {
      for (const frame of DECORATIVE_FRAMES) {
        for (const ptyId of ptyIds) {
          runtime.onPtyData(ptyId, `\x1b]0;${frame} Codex\x07`, Date.now())
        }
    ...
    for (const ptyId of ptyIds) {
      expect(evidence.rawChunksByPty.get(ptyId)).toEqual(
        Array.from({ length: 4 }, () =>
          DECORATIVE_FRAMES.map((frame) => `\x1b]0;${frame} Codex\x07`)
        ).flat()
      )
    }
    ...
    resetCounters(counters)
    resetEvidence(evidence)
    // Bare Codex is idle, so alternating it with working frames is a real state change.
    for (let cycle = 0; cycle < 4; cycle += 1) {
      for (const frame of DECORATIVE_FRAMES) {
        for (const ptyId of ptyIds) {
          runtime.onPtyData(ptyId, '\x1b]0;Codex\x07', Date.now())
    ...
    expect(counters).toMatchObject({
      hostPublications: WORKTREE_COUNT * DECORATIVE_FRAMES.length * 8,
      rendererApplyCalls: WORKTREE_COUNT * DECORATIVE_FRAMES.length * 8,
      rendererStoreMutations: WORKTREE_COUNT * DECORATIVE_FRAMES.length * 8,
    ...
    for (const ptyId of ptyIds) {
      expect(evidence.rawChunksByPty.get(ptyId)).toEqual(
        Array.from({ length: 4 }, () =>
          DECORATIVE_FRAMES.flatMap((frame) => ['\x1b]0;Codex\x07', `\x1b]0;${frame} Codex\x07`])
        ).flat()
      )
    }
    expect(evidence.publishedByWorktree.size).toBe(WORKTREE_COUNT)

    resetCounters(counters)
    resetEvidence(evidence)
    for (const ptyId of ptyIds) {
      runtime.onPtyData(ptyId, '\x1b]0;Codex ready\x07', Date.now())
    }
    vi.advanceTimersByTime(50)
    expect(counters.hostPublications).toBe(WORKTREE_COUNT)
    ...
    for (let index = 0; index < WORKTREE_COUNT; index += 1) {
      ...
      expect(viewerState.tabsByWorktree[worktreeId]).toEqual([
        expect.objectContaining({ title: 'Codex ready', worktreeId })
      ])
      expect(
        detectAgentStatusFromTitle(viewerState.tabsByWorktree[worktreeId]?.[0]?.title ?? '')
      ).toBe('idle')
      expect(evidence.rawChunksByPty.get(`pty-${index}`)).toEqual(['\x1b]0;Codex ready\x07'])
    }
  ...
  it.each([
    {
      ...
      firstTitle: '⠋ Codex - project',
      heartbeatTitles: ['⠋ Codex - project', '⠙ Codex - project']
    },
    {
      agent: 'claude' as const,
      firstTitle: '⠋ Claude Code - project',
      heartbeatTitles: ['⠋ Claude Code - project', '⠙ Claude Code - project']
    }
  ])('renews exact and decorative $agent frames beyond the viewer stale boundary', (testCase) => {
    const runtime = new OrcaRuntimeService()
    const firstWorkingTitleAt = Date.now()
    ...
    ).toEqual(testCase.heartbeatTitles)
  ...
  it('does not publish a stored working row over a later permission title', () => {
    ...
    runtime.onPtyData(ptyId, '\x1b]0;Codex - action required\x07', permissionAt)
    vi.advanceTimersByTime(50)
  ...
  it('does not publish a prior done row over a newer working title', () => {
    ...
    runtime.onPtyData(ptyId, '\x1b]0;⠙ Codex\x07', nextWorkingAt)
    vi.advanceTimersByTime(50)
  ...
  it('publishes explicit Codex titles without waiting for a legacy owner-rebranding probe', async () => {
    ...
    await vi.advanceTimersByTimeAsync(50)
    const settledPublicationCount = publications.length
    expect(settledPublicationCount).toBeLessThanOrEqual(2)
    expect(publications.at(-1)?.tabs[0]).toMatchObject({
      type: 'terminal',
      title: '⠙ Codex',
    ...
    runtime.onPtyData(ptyId, '\x1b]0;⠹ Codex\x07', Date.now())
    await vi.advanceTimersByTimeAsync(50)
    expect(publications).toHaveLength(settledPublicationCount)

    await vi.advanceTimersByTimeAsync(SESSION_TABS_AGENT_STATUS_HEARTBEAT_INTERVAL_MS)
    runtime.onPtyData(ptyId, '\x1b]0;⠸ Codex\x07', Date.now())
    await vi.advanceTimersByTimeAsync(50)
    expect(publications).toHaveLength(settledPublicationCount + 1)
    unsubscribe()
  })

  it('clears tracked title evidence when the provider resets during a foreground read', async () => {
    let resolveForegroundProcess: (agent: string | null) => void = () => undefined
    const foregroundProcess = new Promise<string | null>((resolve) => {
      resolveForegroundProcess = resolve
    })
    ...
    expect(internals.ptysById.get(ptyId)?.lastOscTitle).toBe('⠋ Codex')
