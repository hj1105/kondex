...
import type { RasterTerminalInternals } from './terminal-raster-artifact-analysis'
...
async function terminalWheelTarget(
  ...
  return page.evaluate(() => {
    ...
    return {
      ...
      cellHeight:
        (pane.terminal as unknown as RasterTerminalInternals)._core?._renderService?.dimensions?.css
          ?.cell?.height ?? rect.height / pane.terminal.rows
    }
  })
}
