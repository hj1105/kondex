...
export type MaterializeReleaseCheckoutTestHooks = {
  /** Unit fixtures use their own Git history; wire tests retain REPO_ROOT. */
...
function git(args: string[], repoRoot = REPO_ROOT): string {
...
function resolveCommit(ref: string, repoRoot = REPO_ROOT): string {
  try {
    return git(['rev-parse', `${ref}^{commit}`], repoRoot)
  } catch (error) {
    throw new Error(
...
export async function materializeReleaseCheckout(
  ...
  const sourceRepoRoot = options.testHooks?.sourceRepoRoot ?? REPO_ROOT
  const commit = resolveCommit(ref, sourceRepoRoot)
  ...
  try {
    ...
    await (hooks?.populateStaging
      ...
      : extractReleaseCheckoutTree(sourceRepoRoot, staging, commit))
