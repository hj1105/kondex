...
import {
  ...
  type MaterializeReleaseCheckoutOptions,
...
function git(args: string[]): string {
  return execFileSync('git', args, { cwd: sourceRepoRoot, encoding: 'utf8' }).trim()
}

function createSourceRepository(): void {
  sourceRepoRoot = temporaryCacheRoot()
  git(['init', '--quiet'])
  git(['config', 'user.name', 'Checkout Fixture'])
  git(['config', 'user.email', 'checkout-fixture@example.invalid'])
  git(['config', 'commit.gpgsign', 'false'])
  git(['config', 'core.hooksPath', join(sourceRepoRoot, '.git', 'fixture-hooks')])
  for (const directory of ['main', 'shared', 'preload', 'renderer', 'types']) {
    const target = join(sourceRepoRoot, 'src', directory)
    mkdirSync(target, { recursive: true })
    writeFileSync(join(target, 'fixture.ts'), 'export const fixture = true\n')
  }
  const shared = join(sourceRepoRoot, 'src', 'shared')
  writeFileSync(join(shared, 'terminal-stream-protocol.ts'), 'export const fixture = true\n')
  writeFileSync(join(shared, 'protocol-fixture.ts'), 'export const version = 1\n')
  writeFileSync(
    join(shared, 'protocol-version.ts'),
    "export { version as RUNTIME_PROTOCOL_VERSION } from './protocol-fixture'\n"
  )
  git(['add', '-A'])
  git(['commit', '--quiet', '-m', 'first fixture release'])
  git(['update-ref', 'refs/heads/release/test', 'HEAD'])
  writeFileSync(join(shared, 'protocol-fixture.ts'), 'export const version = 2\n')
  git(['commit', '--quiet', '-am', 'second fixture release'])
  git(['update-ref', 'refs/heads/release_test', 'HEAD'])
}

function materializeFixtureCheckout(
  ref: string,
  options: MaterializeReleaseCheckoutOptions = {}
): Promise<ReleaseCheckout> {
  return materializeReleaseCheckout(ref, {
    ...options,
    testHooks: { ...options.testHooks, sourceRepoRoot }
  })
}
...
function startMaterializerChild(
  ...
  writeFileSync(
    ...
    [
      ...
      `const config = ${JSON.stringify({ ...config, sourceRepoRoot })}`,
      ...
      `  }`,
      `}`,
      `const hooks = { lockOptions: config.lockOptions, sourceRepoRoot: config.sourceRepoRoot }`,
...
describe('release checkout materialization', () => {
  ...
  it('loads a baseline module whose source imports another checkout-root file', async () => {
    const cacheRoot = temporaryCacheRoot()
    const checkout = await materializeFixtureCheckout('release/test', { cacheRoot })
    const protocol = await importReleaseCheckoutModule(checkout, '/src/shared/protocol-version.ts')

    expect(protocol.RUNTIME_PROTOCOL_VERSION).toBeTypeOf('number')
  ...
  it('keeps an import live while another colliding release label materializes', async () => {
    const firstRef = 'release/test'
    const secondRef = 'release_test'
    expect(git(['rev-parse', `${firstRef}^{commit}`])).not.toBe(
      git(['rev-parse', `${secondRef}^{commit}`])
    )

    const cacheRoot = temporaryCacheRoot()
    const first = await materializeFixtureCheckout(firstRef, { cacheRoot })
    ...
    const second = await materializeFixtureCheckout(secondRef, { cacheRoot })
  ...
  it('fails before acquiring a lock when the requested fixture ref is absent', async () => {
    let acquired = false
    await expect(
      materializeFixtureCheckout('missing-release', {
    ...
    ).rejects.toThrow('could not resolve ref "missing-release"')
  ...
  it('causally single-flights a rival process before publishing an in-use checkout', async () => {
    ...
    const published = await materializeFixtureCheckout('release/test', { cacheRoot })
  ...
  it('keeps the real lock live while publication work exceeds its stale interval', async () => {
    ...
    const publisher = materializeFixtureCheckout('release/test', {
      cacheRoot,
      testHooks: {
    ...
    const active = await Promise.race([
      stagingReady,
      publisher.then(() => {
        throw new Error('Publisher completed without acknowledging staging')
      })
    ])
  ...
  it('recovers a crashed lock owner and scavenges only its orphaned staging trees', async () => {
    ...
    try {
      ...
      mkdirSync(unrelated)
      const recovered = await materializeFixtureCheckout('release/test', {
        cacheRoot,
        testHooks: {
          lockOptions: COMPRESSED_LOCK_OPTIONS,
          populateStaging: populateMinimalStaging
  ...
  it('never stamps an incomplete tree produced through the injectable test seam', async () => {
    const cacheRoot = temporaryCacheRoot()
    await expect(
      materializeFixtureCheckout('release/test', {
        cacheRoot,
