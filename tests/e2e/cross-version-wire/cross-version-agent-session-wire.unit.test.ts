...
describe('cross-version structured agent sessions', () => {
  ...
  describe('retired post-auth mobile capability negotiation', () => {
    it('rejects the retired mutation without changing current client capabilities', async () => {
      ...
      expect(current.methodNames).not.toContain(CLIENT_CAPABILITY_UPDATE_METHOD)
      expect(current.protocolVersion).toBe(baseline.protocolVersion)
