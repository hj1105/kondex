...
for (const closeMode of ['terminal-close-cli', 'worker-release'] as const) {
  test(`completed background worker ${closeMode} retires resume authority before first activation`, async ({
    ...
    expect(
      await orcaPage.evaluate(
        (worktreeId) => window.__store?.getState().tabsByWorktree[worktreeId] ?? [],
        targetWorktreeId
      )
    ).toEqual([])
    expect(readCompletedWorkerLedger().filter((event) => event.event === 'spawn')).toHaveLength(1)
    await expect(orcaPage.locator('.session-restored-banner')).toHaveCount(0)
    // Empty workspaces stay empty; opening a fresh shell must not resurrect the retired worker.
