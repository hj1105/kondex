...
test('adopts runtime-owned agent and Setup PTYs on first mount', async ({
  ...
  await expect
    .poll(() =>
      orcaPage.evaluate(
        async ({ repoId, agentCommand, terminalWindowsShell }) => {
          const state = window.__store?.getState()
        ...
        { repoId, agentCommand: fakeCodexCommand, terminalWindowsShell: FAKE_AGENT_WINDOWS_SHELL }
      )
    )
    .toBe(true)
  ...
  if (!agent?.ptyId || !setup?.ptyId || !canary?.ptyId) {
    throw new Error('Agent, setup and canary terminals must each publish a live PTY id')
  }
