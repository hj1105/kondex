...
In the historical upstream update cluster the uninstaller was unsigned while the
main executable and terminal daemon were signed.
...
prevalence is keyed on **file hash**. A widely installed unsigned binary can
clear on install count alone, while every new build starts with new hashes.
Kondex does not inherit the upstream signing identity or update channels, so a
future distribution design must establish its own reputation and signing policy.
