# Agent skill installer upstream boundary
...
Kondex needs bounded archive ingestion, host-owned destination resolution, crash-safe local
transactions, provenance, and mixed-version remote installation. `vercel-labs/skills` exposes a
CLI but does not provide these application-runtime contracts.
...
Kondex implements its package validation, installation, provider-placement, and recovery behavior
...
documentation and verify them with real installations. Kondex does not depend on the upstream CLI,
...
- Kondex owns stability, security, compatibility, and maintenance of this narrower installer.
