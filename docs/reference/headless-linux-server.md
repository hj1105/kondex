...
Build the AppImage from this checkout, then install the local artifact:

```bash
pnpm build:linux
sudo mkdir -p /opt/orca
sudo install -m 755 dist/orca-linux.AppImage /opt/orca/orca-linux.AppImage
...
never silently omitted. Stop a foreground server with `Ctrl+C`. Stable reasons
are `disabled_by_operator`, `websocket_unavailable`,
...
# Name the local build and point to the AppImage copied to this server
ORCA_VERSION=local-2026-09-04
KONDEX_APPIMAGE_SOURCE=/absolute/path/to/orca-linux.AppImage

# Select the expected binary architecture on the server where Kondex runs
case "$(uname -m)" in
  x86_64)
...
# 1. Stage and verify the new local build while the server stays online
sudo install -m 755 "$KONDEX_APPIMAGE_SOURCE" /opt/orca/orca-linux.AppImage.new
