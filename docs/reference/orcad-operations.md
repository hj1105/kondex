...
network access-link generation, which normally rebinds to all interfaces, is refused while the bind is
...
- **Logs.** orcad writes human-readable diagnostics to **stderr** and its readiness contract
  ...
  `KONDEX_DIAGNOSTICS_DISABLED=1`). Rotation of that file is not implemented — see
