...
while ((SECONDS < deadline)); do
  if grep -Eq '^\[startup\] services-initialized t=[0-9]+$' "$stderr_log"; then
    marker_seen=true
    break
  fi
  if ! identity_alive "$launcher_pid" "$launcher_start_ticks"; then
    report_launcher_exit 'the services-initialized marker'
  fi
  sleep 0.2
done
if [[ "$marker_seen" != true ]]; then
  ...
  echo "FAIL: desktop AppImage did not emit services-initialized within ${startup_timeout_seconds}s" >&2
  exit 1
fi
if ! identity_alive "$launcher_pid" "$launcher_start_ticks"; then
