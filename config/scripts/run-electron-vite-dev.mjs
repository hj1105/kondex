...
import { configureDevKontextSidecarEnvironment } from './kontext-sidecar-source.mjs'
...
configureDevKontextSidecarEnvironment({ repoRoot, environment: process.env })
...
function createDockTitle(branch, label) {
  return `Kondex: ${branch || label || 'dev'}`
}
...
function seedDevInstanceIdentityEnv() {
  ...
  process.env.KONDEX_DEV_REPO_ROOT ||= repoRoot
  process.env.ORCA_DEV_REPO_ROOT ||= process.env.KONDEX_DEV_REPO_ROOT
...
function prepareMacDevElectronApp() {
  ...
  const title = process.env.ORCA_DEV_DOCK_TITLE || 'Kondex: dev'
  ...
  // piling up "Kondex: <branch>" rows forever and breaking the notification
  ...
  if (
    ...
    console.warn(
      `[kondex-dev] Another dev instance is running from this bundle; reusing it instead of rebuilding. Quit the other instance (or delete ${distDir}) to force a rebuild.`
    )
    process.env.ELECTRON_EXEC_PATH = executablePath
    return
  }
  ...
  try {
    ...
    console.warn(
      `[kondex-dev] notification-status helper build failed (permission card falls back to probes): ${error?.message ?? error}`
    )
  }
  ...
  try {
    ...
    console.warn(
      `[kondex-dev] keyboard-layout helper build failed (shifted Option composition stays conservative): ${error?.message ?? error}`
    )
  }
  ...
  try {
    ...
    signed = false
    console.warn(
      `[kondex-dev] ad-hoc codesign failed (dev notifications will not deliver): ${error?.message ?? error}`
    )
  }
...
function getDevUserDataPath() {
  ...
  if (process.platform === 'darwin') {
    return path.join(process.env.HOME ?? '', 'Library', 'Application Support', 'kondex-dev')
  }
...
function prepareDevCliWrapper() {
  ...
  console.log(`[kondex-dev] Prepared wrapper in ${binDir}`)
}
...
function prepareDevWebClient() {
  ...
  if (!existsSync(getDevWebClientIndexPath()) && process.env.ORCA_DEV_WEB_PREPARE !== '1') {
    console.error(
      '[kondex-dev] Web client bundle missing; skipping pairing web build. Run `pnpm run build:web` or set ORCA_DEV_WEB_PREPARE=1 when you need browser pairing.'
    )
    return
  }
  ...
  console.error('[kondex-dev] Building web client for pairing...')
  execFileSync(
    process.execPath,
...
if (!isHelpOrVersion && process.env.ORCA_DEV_INSTANCE_LABEL) {
  console.error(`[kondex-dev] Instance: ${process.env.ORCA_DEV_INSTANCE_LABEL}`)
}
...
if (!isHelpOrVersion && process.env.ORCA_BACKGROUND_LAUNCH === '1') {
  console.error('[kondex-dev] Background launch: window shows without stealing focus')
}
let forwardedExtras = []
if (!userPassedPort && !isHelpOrVersion) {
  ...
  if (envPortRaw) {
    ...
    if (port === null) {
      console.error(
        `[kondex-dev] Ignoring invalid REMOTE_DEBUGGING_PORT=${JSON.stringify(envPortRaw)}; falling back to probe.`
      )
    }
  }
  if (port === null) {
  ...
  if (port !== null) {
    ...
    console.error(`[kondex-dev] Remote debugging on http://127.0.0.1:${port}`)
  } else {
    console.error(
      '[kondex-dev] No free debug port found in sweep; starting without --remote-debugging-port.'
    )
  }
}
prepareDevWebClient()
