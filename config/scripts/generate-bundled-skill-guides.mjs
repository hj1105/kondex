...
const GUIDE_ALIASES = {
  'kondex-cli': ['orca-cli'],
  'kondex-computer-use': ['computer-use'],
  'kondex-emulator': ['orca-emulator'],
  'kondex-emulator-android': ['orca-emulator-android'],
  'kondex-orchestration': ['orchestration']
}

// Why: a stubbed topic ships a hybrid discovery stub as its installable projection while
// `kondex skills get <topic>` still serves the full version-matched guide from the binary.
  ...
  const guideEntries = guides
    .map((guide) => {
      ...
      return [
        ...
        `    installMarkdown: ${JSON.stringify(guide.installMarkdown)},`,
