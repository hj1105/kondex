...
describe('electron-builder markdown file associations', () => {
  ...
  // next installation, with APP_UNASSOCIATE never restoring it. `rank: 'Alternate'` cannot
  ...
  it('points the single NSIS include at the installer hooks file on disk', () => {
    ...
    expect(basename(includePath)).toBe('kondex-installer-hooks.nsh')
  })
  ...
  it('recognizes an APP_ASSOCIATE-style default-handler write', () => {
    for (const takeover of [
      '  WriteRegStr SHELL_CONTEXT "Software\\Classes\\.md" "" "Kondex.Markdown"',
      'WriteRegStr  SHELL_CONTEXT  "Software\\Classes\\.markdown"  ""  "$0"'
    ]) {
      expect(takeover).toMatch(DEFAULT_HANDLER_WRITE)
    }
    expect(
      'WriteRegNone SHELL_CONTEXT "Software\\Classes\\.md\\OpenWithProgids" "Kondex.Markdown"'
  ...
  it('registers Windows markdown Open With additively, never as the default', async () => {
    ...
    // The additive hint that puts Kondex in Explorer's "Open with" list.
    expect(hooks).toMatch(
      /WriteRegNone\s+SHELL_CONTEXT\s+"Software\\Classes\\\$\{EXT\}\\OpenWithProgids"/
    )
    expect(hooks).toMatch(/!macro\s+KONDEX_REGISTER_MARKDOWN_OPEN_WITH\s+EXT/)
    for (const ext of MARKDOWN_EXTENSIONS) {
      expect(hooks).toContain(`KONDEX_REGISTER_MARKDOWN_OPEN_WITH ".${ext}"`)
      expect(hooks).toContain(`KONDEX_UNREGISTER_MARKDOWN_OPEN_WITH ".${ext}"`)
    }
