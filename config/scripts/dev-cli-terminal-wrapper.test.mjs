...
describe('dev CLI terminal wrappers', () => {
  it('writes profile-scoped Windows wrappers for worker terminals', () => {
    const root = mkdtempSync(path.join(tmpdir(), 'kondex-dev-terminal-wrapper-'))
    ...
    const wrapper = readFileSync(path.join(userDataPath, 'cli', 'bin', 'kondex-dev.cmd'), 'utf8')
    ...
    expect(readFileSync(path.join(userDataPath, 'cli', 'bin', 'kondex.cmd'), 'utf8')).toBe(wrapper)
    expect(readFileSync(path.join(root, 'out', 'bin', 'kondex-dev.cmd'), 'utf8')).toBe(wrapper)
    expect(readFileSync(path.join(root, 'out', 'bin', 'kondex.cmd'), 'utf8')).toBe(wrapper)
  })

  it('escapes literal percent signs in every Windows batch path', () => {
    const root = path.join(
      mkdtempSync(path.join(tmpdir(), 'kondex-dev-terminal-wrapper-')),
      '%repo%'
    )
    const userDataPath = path.join(root, '%profile%')
  ...
  it('writes executable-style POSIX wrappers with the same profile identity', () => {
    ...
    const wrapper = readFileSync(path.join(userDataPath, 'cli', 'bin', 'kondex-dev'), 'utf8')
    ...
    expect(readFileSync(path.join(userDataPath, 'cli', 'bin', 'kondex'), 'utf8')).toBe(wrapper)
    expect(readFileSync(path.join(root, 'out', 'bin', 'kondex-dev'), 'utf8')).toBe(wrapper)
    expect(readFileSync(path.join(root, 'out', 'bin', 'kondex'), 'utf8')).toBe(wrapper)
  })
})
