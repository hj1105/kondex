...
describe('guarded entry names', () => {
  ...
  it('rejects a guarded name that is no longer a rollup input', () => {
    ...
    const input = Object.fromEntries(
      GUARDED_ENTRY_NAMES.filter((name) => name !== 'port-scan-command-worker-entry').map(
        (name) => [name, `${name}.ts`]
      )
    )

    expect(() => runBuildStart(plugin, input)).toThrow('"port-scan-command-worker-entry"')
  })
...
describe('worker thread entry guard', () => {
  ...
  it('follows shared chunks out of a worker entry', () => {
    const plugin = createPlainNodeEntryGuardPlugin()
    const bundle = {
      'session-scanner-worker-entry.js': workerChunk(
  ...
  it.each(['skills/skill-provider-runtime-roots', 'skills/bundled-agent-skill-install'])(
    'rejects Electron in CLI skill entry %s',
    (name) => {
      const plugin = createPlainNodeEntryGuardPlugin()
      const bundle = { [`${name}.js`]: workerChunk(name, 'require("electron")') }
