const { existsSync, readFileSync, realpathSync, mkdtempSync, rmSync } = require('node:fs')
...
function runCli(outDir, args, timeoutMs = CLI_COMMAND_TIMEOUT_MS, options = {}) {
  const entry = resolve(outDir, 'cli', 'index.js')
  const env = { ...process.env, ...options.env, NODE_PATH: '' }
  delete env.ORCA_CLI_CWD
  const result = spawnSync(process.execPath, [entry, ...args], {
    cwd: options.cwd ?? dirname(outDir),
    encoding: 'utf8',
    env,
    killSignal: 'SIGKILL',
...
function verifyBundledInstall(outDir) {
  const root = realpathSync(mkdtempSync(join(tmpdir(), 'kondex-built-skill-install-')))
  try {
    const options = { cwd: root, env: { ORCA_USER_DATA_PATH: join(root, 'state') } }
    const installed = parseJson(
      'bundled install',
      runCli(
        outDir,
        ['skills', 'install', '--skill', 'kondex-cli', '--local', '--agent', 'universal', '--json'],
        CLI_COMMAND_TIMEOUT_MS,
        options
      )
    )
    const canonicalPath = join(root, '.agents', 'skills', 'kondex-cli')
    if (
      installed.status !== 'complete' ||
      installed.skills?.[0]?.status !== 'installed' ||
      installed.skills[0].canonicalPath !== canonicalPath
    ) {
      throw new Error(
        '[verify-skills-cli-runtime] bundled install did not complete inside its temporary workspace'
      )
    }
    const markdown = readFileSync(join(canonicalPath, 'SKILL.md'), 'utf8')
    if (!markdown.includes('name: kondex-cli') || !markdown.includes('discovery stub')) {
      throw new Error(
        '[verify-skills-cli-runtime] installed payload is not the Kondex discovery stub'
      )
    }
    const updated = parseJson(
      ...
      runCli(
        outDir,
        ['skills', 'update', '--skill', 'kondex-cli', '--local', '--json'],
        CLI_COMMAND_TIMEOUT_MS,
        options
      )
    )
    if (updated.status !== 'complete' || updated.skills?.[0]?.status !== 'unchanged') {
      throw new Error('[verify-skills-cli-runtime] bundled update is not idempotent')
    }
  } finally {
...
function verifySkillsCliRuntime(outDir, artifactRoot = dirname(outDir), options = {}) {
  ...
  for (const topic of ['kondex-cli', 'kondex-computer-use']) {
  ...
  verifyBundledInstall(absoluteOutDir)
  return { closureFiles: closure.length, commands: 7 }
}
...
module.exports = { collectRuntimeClosure, runCli, runtimeImportSpecifiers, verifySkillsCliRuntime }
