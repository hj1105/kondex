...
const wrapperPath = path.join(projectDir, 'config', 'scripts', 'kondex-dev.mjs')

describe('kondex-dev package bin', () => {
  it('uses a Node entrypoint for cross-platform package installs', () => {
    expect(packageJson.bin['kondex-dev']).toBe('./config/scripts/kondex-dev.mjs')
    expect(readFileSync(wrapperPath, 'utf8')).toMatch(/^#!\/usr\/bin\/env node\n/)
  })

  it('runs the dev CLI through Node without requiring Bash', () => {
    const root = mkdtempSync(path.join(tmpdir(), 'kondex-dev-bin-'))
    ...
    execFileSync(process.execPath, [wrapperPath, '--help'], {
      env: {
        ...process.env,
        KONDEX_DEV_CLI_ENTRY_PATH: cliEntry,
        KONDEX_DEV_USER_DATA_PATH: path.join(root, 'user-data'),
