...
describe('dev-electron-bundle-identity', () => {
  it('leaves process.env untouched, including its object identity', () => {
    ...
    patchesUnder('Kondex: some-branch', 'some-branch')
  ...
  it('patches a stable bundle id for the app and its helper', () => {
    expect(DEV_BUNDLE_ID).toBe('app.kondex.desktop.dev')
  ...
  it('gives the dev app a legible display name so notifications are not just "Electron"', () => {
    ...
    expect(DEV_BUNDLE_DISPLAY_NAME).toBe('Kondex Dev')
  ...
  it('keeps the bundle display name in step with the name safeStorage keys off', () => {
    // Two independently hardcoded 'Kondex Dev' strings: this one names the bundle (notifications,
  ...
  it('produces byte-identical patches on two different branches', () => {
    ...
    expect(patchesUnder('Kondex: fix-login-crash', 'fix-login-crash')).toEqual(
      patchesUnder('Kondex: perf-2', 'perf-2')
    )
    expect(patchesUnder('Kondex: dev', 'main')).toEqual(
      patchesUnder('Kondex: some-worktree @ feature/x', 'feature/x')
    )
  })

  it('leaks no branch, worktree, or title text into any patched value', () => {
    const branch = 'fix-login-crash'
    const worktree = 'Kondex-safe-storage-lock'
    for (const patch of patchesUnder(`Kondex: ${branch}`, branch)) {
