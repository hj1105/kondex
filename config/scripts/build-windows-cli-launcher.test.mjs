...
describe('Windows CLI launcher', () => {
  it('reuses a compiled launcher that is at least as new as the C# source', () => {
    const root = mkdtempSync(join(tmpdir(), 'kondex-cli-launcher-reuse-'))
    try {
      const sourcePath = join(root, 'KondexCliLauncher.cs')
      const outputPath = join(root, '.build', 'kondex.exe')
  ...
  itCrossHost('fails closed when the Windows launcher cannot be compiled on this host', () => {
    const outputRoot = mkdtempSync(join(tmpdir(), 'kondex cross-host launcher '))
    try {
  ...
  itCrossHost('never materializes the child environment block from ProcessStartInfo', () => {
    ...
    const source = readFileSync(
      join(projectRoot, 'native', 'windows-cli-launcher', 'KondexCliLauncher.cs'),
      'utf8'
    )
  ...
  itWindows('preserves a multiline argument from PowerShell through the native launcher', () => {
    const appRoot = mkdtempSync(join(tmpdir(), 'kondex cli launcher '))
    try {
      const resourcesPath = join(appRoot, 'resources')
      const launcherPath = join(resourcesPath, 'bin', 'kondex.exe')
      const cliPath = join(resourcesPath, 'app.asar.unpacked', 'out', 'cli', 'index.js')
      mkdirSync(join(resourcesPath, 'bin'), { recursive: true })
      mkdirSync(dirname(cliPath), { recursive: true })
      copyFileSync(process.execPath, join(appRoot, 'Kondex.exe'))
      writeFileSync(
        cliPath,
  ...
  itWindows('survives an inherited environment block containing PATH and Path', () => {
    const appRoot = mkdtempSync(join(tmpdir(), 'kondex duplicate path launcher '))
    try {
      const resourcesPath = join(appRoot, 'resources')
      const launcherPath = join(resourcesPath, 'bin', 'kondex.exe')
      const cliPath = join(resourcesPath, 'app.asar.unpacked', 'out', 'cli', 'index.js')
