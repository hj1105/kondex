...
describe('static AppImage package contract', () => {
  it.each([
    ['kondex-linux-x64.AppImage', 0x3e, 1],
    ['kondex-linux-arm64.AppImage', 0xb7, 'arm64']
  ...
  it.each([
    ['x64 filename for an arm64 runtime and target', 'kondex-linux-x64.AppImage', 0xb7, 3],
    ['arm64 filename for an x64 runtime and target', 'kondex-linux-arm64.AppImage', 0x3e, 1],
    ['x64 runtime for an arm64 target', 'kondex-linux-x64.AppImage', 0x3e, 3],
    ['arm64 runtime for an x64 target', 'kondex-linux-x64.AppImage', 0xb7, 1],
    ['arm64 artifact filename for an x64 target', 'kondex-linux-arm64.AppImage', 0xb7, 1],
    ['x64 runtime under an arm64 artifact filename', 'kondex-linux-arm64.AppImage', 0x3e, 3]
  ...
  it.each([undefined, 0, 'ia32'])(
    ...
    async (targetArch) => {
      await withFixture('kondex-linux-x64.AppImage', createRuntime(), (path) => {
  ...
  it('accepts PT_DYNAMIC relocation metadata without dependencies', async () => {
    ...
    await withFixture('kondex-linux-x64.AppImage', runtime, (path) => {
  ...
  it('rejects artifact names outside the release contract before reading them', () => {
    expect(() => verifyStaticAppImagePackage('/missing/kondex-preview.AppImage')).toThrow(
