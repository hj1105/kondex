...
/** Builds a temporary Kondex project fixture with a compiled CLI entrypoint. */
function makeProjectWithCli(
  content,
  { mode = 0o755, rootPackageType, writeOutPackageJson = true } = {}
) {
  const projectDir = mkdtempSync(path.join(tmpdir(), 'kondex-cli-bin-'))
  ...
  writeFileSync(
    ...
    JSON.stringify({ bin: { kondex: './out/cli/index.js' }, type: rootPackageType }),
    'utf8'
  )
...
describe('verifyPackageCliBin', () => {
  ...
  it('rejects an empty package bin target', () => {
    ...
    expect(() => verifyPackageCliBin({ projectDir })).toThrow('bin.kondex target is empty')
  })
  ...
  it('rejects an upstream-only manifest instead of checking the wrong command', () => {
    ...
    expect(() => verifyPackageCliBin({ projectDir })).toThrow('must declare bin.kondex')
  })

  it('checks Kondex when a manifest also contains an upstream command', () => {
    const { projectDir, cliPath } = makeProjectWithCli('#!/usr/bin/env node\n')
    writeFileSync(
      path.join(projectDir, 'package.json'),
      JSON.stringify({ bin: { kondex: './out/cli/index.js', orca: './missing-upstream.js' } })
    )

    expect(verifyPackageCliBin({ projectDir }).binPath).toBe(cliPath)
  })
})
