...
// Why: product names and agent labels stay Latin — MT reads them as common words (Codex→copy).
...
function applyCjkLatinTermSpacing(localeValue, locale) {
  ...
  if (locale === 'ko') {
    ...
    // boundary — re-glue "Orca 에"/"PR 을"/"에서는" but keep product terms naturally spaced.
