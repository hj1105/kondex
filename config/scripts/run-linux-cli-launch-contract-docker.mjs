...
try {
  if (!appImage) {
    fail(
      'Usage: run-linux-cli-launch-contract-docker.mjs --appimage /path/to/kondex-linux-x64.AppImage [--platform linux/amd64|linux/arm64]'
    )
  }
...
function stageArtifacts() {
  ...
  docker(
    [
      ...
      `${artifactVolume}:/artifacts`,
      '-v',
      `${appImage}:/input/kondex-linux.AppImage:ro`,
      '--entrypoint',
      'bash',
      tag,
      '-lc',
      [
        'set -euo pipefail',
        'cp /input/kondex-linux.AppImage /artifacts/kondex-linux.AppImage',
        'chmod +x /artifacts/kondex-linux.AppImage',
        'chown -R orca:orca /artifacts',
        // Use the AppImage runtime's no-FUSE extraction path.
        'cd /artifacts && runuser --user orca -- ./kondex-linux.AppImage --appimage-extract >/dev/null',
