...
const guidePath = join(projectDir, 'skill-guides', 'kondex-computer-use.md')
const stubPath = join(projectDir, 'skills', 'kondex-computer-use', 'SKILL.md')
const bundledGuide = BUNDLED_SKILL_GUIDES.find(
  (guide) => guide.name === 'kondex-computer-use'
)?.markdown
...
describe('computer-use skill guidance', () => {
  it('keeps discovery scoped to desktop control and out of the embedded browser', () => {
    ...
    expect(description).toContain("Do not use for Kondex's embedded browser")
    expect(description).toContain('page-only browser automation')
    expect(description).toContain("`kondex-cli` for Kondex's embedded pages")
    ...
    const kondexCli = readFileSync(
      join(projectDir, 'skill-guides', 'kondex-cli.md'),
      'utf8'
    ).replace(/\s+/gu, ' ')
    expect(kondexCli).toContain('browser embedded inside the Kondex app')
  })

  it('keeps web-app targeting on the computer-use surface', () => {
    const skill = readFileSync(guidePath, 'utf8')

    expect(skill).toContain('Use this skill for desktop UI through `kondex computer`')
    expect(skill).toContain('external desktop browser window that needs desktop-level control')
    expect(skill).not.toContain('kondex goto')
    expect(skill).not.toContain('kondex snapshot')
    expect(skill).not.toContain('kondex click')
    expect(skill).not.toContain('kondex fill')
...
describe('computer-use install stub', () => {
  it('points at the version-matched guide and preserves the safe resolver', () => {
    ...
    expect(stub).toContain('KONDEX skills get kondex-computer-use')
    // The safe CLI-resolution contract must survive in the stub, never a bare `orca`.
    expect(stub).toContain('ORCA_CLI_COMMAND')
    expect(stub).toContain('KONDEX_CLI_COMMAND')
    expect(stub.indexOf('KONDEX_CLI_COMMAND')).toBeLessThan(stub.indexOf('ORCA_CLI_COMMAND'))
    expect(stub).toContain('belongs to Kondex')
