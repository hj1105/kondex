...
describe('Electron Vite output contract', () => {
  ...
  it('gives the dev terminal daemon helper the TCC identity watched by Kondex', () => {
  ...
  it('keeps visible dev-runner identity and state separate from Orca', () => {
    expect(devRunner).toContain("return `Kondex: ${branch || label || 'dev'}`")
    expect(devRunner).toContain('[kondex-dev]')
    expect(devRunner).toContain("'Application Support', 'kondex-dev'")
    expect(devRunner).not.toContain('[orca-dev]')
    expect(devRunner).not.toContain("'Application Support', 'orca-dev'")
  })
})
