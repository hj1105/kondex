...
const appPath = path.join(packagePath, '.build', 'release', 'Kondex Computer Use.app')
...
const bundleId = process.env.ORCA_COMPUTER_MACOS_BUNDLE_ID ?? 'app.kondex.desktop.computer-use'
const displayName = 'Kondex Computer Use'
...
function codesignArgs(identity, targetPath) {
  ...
  if (process.env.KONDEX_MAC_SIGN === '1' && identity !== '-') {
...
function resolveSigningIdentity() {
  ...
  if (process.env.KONDEX_MAC_SIGN !== '1' && developmentMatch) {
...
<dict>
  ...
  <string>Kondex Computer Use needs Accessibility permission to read and interact with app interfaces when you ask Kondex to use apps.</string>
  <key>NSScreenCaptureUsageDescription</key>
  <string>Kondex Computer Use needs Screen Recording permission to capture app windows when you ask Kondex to inspect your screen.</string>
</dict>
</plist>
`
}
