  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      try {
        ...
        const launcherPath = join(resourcesDir, 'bin', 'kondex')
        const upstreamLauncherPath = join(resourcesDir, 'bin', 'orca-ide')
        ...
        await writeFile(
          ...
          [
            ...
            "if (args[1] === 'list') console.log(JSON.stringify({ topics: [{ name: 'kondex-cli' }, { name: 'kondex-computer-use' }] }))",
        ...
        await writeFile(upstreamLauncherPath, '#!/usr/bin/env bash\n', {
          encoding: 'utf8',
          mode: 0o644
        })
        ...
        expect((await stat(upstreamLauncherPath)).mode & 0o111).toBe(0)
        await expect(
