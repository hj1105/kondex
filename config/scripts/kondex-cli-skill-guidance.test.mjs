...
// Why: kondex-cli now ships a hybrid discovery stub, so its version-sensitive command
...
const guidePath = join(projectDir, 'skill-guides', 'kondex-cli.md')
const stubPath = join(projectDir, 'skills', 'kondex-cli', 'SKILL.md')
// Why: orchestration and kondex-emulator also ship hybrid stubs now, so their version-sensitive
// command guidance lives in the guide sources — read the cross-guide worktree-id contract there.
const orchestrationSkillPath = join(projectDir, 'skill-guides', 'kondex-orchestration.md')
const emulatorSkillPath = join(projectDir, 'skill-guides', 'kondex-emulator.md')

function readSkill(path = guidePath) {
  return readFileSync(path, 'utf8')
}

describe('kondex CLI skill guidance', () => {
  it('keeps external browser routing at the OS/page boundary', () => {
    ...
    expect(skill).toContain(
      'For external Chrome/Safari/webviews or Kondex app chrome/settings, use the Computer Use skill/tool only when the task requires OS/window-level control'
    )
    expect(skill).toContain(
      "Use `kondex-cli` for Kondex's embedded pages and a page-automation tool such as Playwright or CDP for external pages"
    )
  })
  ...
  it('keeps independent worktree lineage separate from Git base selection', () => {
    ...
    expect(skill).toContain('`--no-parent` only controls Kondex lineage')
    expect(skill).toContain('omit `--base-branch` so Kondex uses the repo default base')
  ...
  it('keeps browser injection guidance narrow and avoids literal secret examples', () => {
    ...
    expect(skill).toContain('`kondex eval` expressions, or `kondex exec` commands')
...
describe('kondex CLI install stub', () => {
  ...
  it('does not mistake resolution or execution failures for an older binary', () => {
    ...
    // Falling through can silently pair a version-matched guide with the wrong Kondex build.
  ...
  it('drops the changing command reference from the installable file', () => {
    ...
    expect(stub).not.toContain('KONDEX automations create')
