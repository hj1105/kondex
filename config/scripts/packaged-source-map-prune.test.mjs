...
const FIXTURE_RUNTIME_PACKAGE_JSON =
  '{"name":"@fixture/runtime-sdk","type":"module","main":"./dist/index.cjs","exports":{".":{"require":"./dist/index.cjs","import":"./dist/index.mjs"}}}'

async function createPackagedNodeModulesFixture(resourcesDir) {
  const packageDir = join(resourcesDir, 'node_modules', '@fixture', 'runtime-sdk')
  ...
  await writeFile(join(packageDir, 'package.json'), FIXTURE_RUNTIME_PACKAGE_JSON, 'utf8')
  ...
  await writeFile(
    ...
    'exports.RuntimeClient = class RuntimeClient {}',
    'utf8'
  )
  ...
  await writeFile(join(nodePtyDir, 'index.d.ts'), 'export {}', 'utf8')
  await writeFile(join(nodePtyDir, 'index.d.ts.map'), '{}', 'utf8')
  return { packageDir, distDir, webhooksDir, jsYamlDir, nodePtyDir }
}

describe('packaged runtime type-declaration and source-map pruning', () => {
  it('removes dependency source maps while preserving runtime files and non-JS maps', async () => {
    ...
    try {
      ...
      const sdk = createRequire(join(resourcesDir, 'consumer.cjs'))('@fixture/runtime-sdk')
      expect(typeof sdk.RuntimeClient).toBe('function')
    } finally {
      await rm(resourcesDir, { recursive: true, force: true })
    }
  })

  it('removes source maps from every packaged dependency', async () => {
  ...
  it('removes type declarations and declaration maps in the same walk', async () => {
    ...
    try {
      const { nodePtyDir } = await createPackagedNodeModulesFixture(resourcesDir)
  ...
  it('runs the artifact prune through aggregate runtime cleanup', async () => {
    ...
    try {
      const { distDir, nodePtyDir, packageDir } =
        await createPackagedNodeModulesFixture(resourcesDir)
