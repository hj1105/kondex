...
function defaultOutputPath(projectRoot) {
  return join(projectRoot, 'native', 'windows-cli-launcher', '.build', 'kondex.exe')
}
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  if (process.platform !== 'win32') {
    ...
    // continue toward a Windows package whose declared kondex.exe does not exist.
    throw new Error(
  ...
  const sourcePath = join(repoRoot, 'native', 'windows-cli-launcher', 'KondexCliLauncher.cs')
  const outputPath = readArg('--output') ?? defaultOutputPath(repoRoot)
  const compilerPath = findFrameworkCompiler(process.env)

  if (!compilerPath) {
    throw new Error('Unable to find the .NET Framework C# compiler required for kondex.exe.')
  }
