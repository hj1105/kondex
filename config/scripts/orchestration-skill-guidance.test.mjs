...
const guidePath = join(projectDir, 'skill-guides', 'kondex-orchestration.md')
const stubPath = join(projectDir, 'skills', 'kondex-orchestration', 'SKILL.md')
...
describe('orchestration skill guidance', () => {
  it('keeps external browser routing at the OS/page boundary', () => {
    ...
    expect(description).toContain(
      "Use Computer Use for external browser windows, webviews, Kondex app UI, or desktop UI outside Kondex's embedded browser only when the task requires OS/window-level control such as focus, menus, dialogs, coordinates, or screenshots."
    )
    expect(description).toContain(
      "`kondex-cli` for Kondex's embedded pages and a page-automation tool such as Playwright or CDP for external pages."
    )
  })

  it('requires Kondex runtime state before claiming a worker was orchestrated', () => {
    const skill = readSkill()
    const toolBoundary = getSection(skill, 'Tool Boundary')

    expect(toolBoundary).toContain('must create or bind a Run')
    expect(toolBoundary).toContain('create the Task with `kondex orchestration task-create`')
    expect(toolBoundary).toContain('preferred `kondex orchestration worker-start` composition')
    expect(toolBoundary).toContain('low-level `kondex orchestration dispatch --inject` path')
    expect(toolBoundary).not.toContain('or `kondex orchestration run`')
    expect(skill).toContain(
      '`coordinator-start`, `coordinator-stop`, `run`, and `run-stop` are retired scheduler commands'
    )
    expect(toolBoundary).toContain(
      'Do not substitute non-Kondex subagent tools, generic agent-spawn APIs, or chat-only parallel worker features'
    )
    expect(toolBoundary).toContain('do not create Kondex task/dispatch provenance')
    ...
    expect(toolBoundary).toContain('kondex orchestration task-list --json')
    expect(toolBoundary).toContain('kondex orchestration dispatch-show --task <task_id> --json')
    expect(toolBoundary).toContain(
  ...
  it('keeps full handoffs out of dispatch lifecycle and off the active branch base', () => {
    ...
    expect(skill).toContain(
      '`--no-parent` only controls Kondex lineage; it does not choose the Git base.'
    )
    expect(skill).toContain(
      'never base it on the current feature branch unless the user explicitly asks'
    )
    expect(skill).toContain(
      'kondex worktree create --name <task-name> --no-parent --agent codex --prompt'
    )
    expect(fullHandoffs).toContain(
      'Before creating a new worktree from an active feature branch, decide and state whether the desired Kondex lineage is child or top-level'
    )
    expect(fullHandoffs).toContain(
  ...
  it('keeps review-only completions and named next-owner fixes in their lanes', () => {
    ...
    expect(skill).toContain(
      "If the user's plan names a next owner agent " +
        '(for example, "then use Codex to create a PR")'
    )
  ...
  it('makes settled worker terminal release an explicit coordinator step', () => {
    ...
    expect(workerLoop).toContain(
      'kondex orchestration worker-start --task <next_task_id> --terminal <handle> --json` so Kondex ' +
    ...
    expect(workerLoop).toContain(
      'kondex orchestration worker-retain --dispatch <dispatch_id> --json'
    )
    expect(workerLoop).toContain(
  ...
  it('documents @codex in the Messaging group address list', () => {
    const skill = readSkill()
    const messaging = getSection(skill, 'Messaging')

    expect(messaging).toContain('`@codex`')
  })

  it('documents @claude in the Messaging group address list', () => {
    const skill = readSkill()
    const messaging = getSection(skill, 'Messaging')

    expect(messaging).toContain('`@claude`')
  })
  ...
  it('keeps agent-first launch, handle recovery, and inbox injection distinct', () => {
    ...
    expect(agentFirstExample).not.toContain('kondex terminal list')
...
describe('orchestration install stub', () => {
  it('points at the version-matched guide and preserves the safe resolver', () => {
    ...
    expect(stub).toContain('KONDEX skills get kondex-orchestration')
    // The safe CLI-resolution contract must survive in the stub, never a bare `kondex`.
    expect(stub).toContain('ORCA_CLI_COMMAND')
    expect(stub).toContain('kondex-dev')
    expect(stub).toContain('KONDEX_CLI_COMMAND')
    expect(stub).toContain('legacy `orca` bridge on an SSH host')
    expect(stub).not.toContain('GNOME')
  ...
  it('does not tell agents to mutate orchestration state before loading the guide', () => {
    ...
    expect(preGuide).not.toContain('kondex orchestration task-create')
    expect(preGuide).not.toContain('kondex orchestration dispatch')
  })
