...
const cliEntry =
  process.env.KONDEX_DEV_CLI_ENTRY_PATH ??
  process.env.ORCA_DEV_CLI_ENTRY_PATH ??
  path.join(repoRoot, 'out', 'cli', 'index.js')

if (!existsSync(cliEntry)) {
  console.error("kondex-dev: CLI not built yet. Run 'pnpm run build:cli' first.")
  process.exit(1)
}

process.env.ORCA_USER_DATA_PATH =
  process.env.KONDEX_DEV_USER_DATA_PATH ??
  process.env.ORCA_DEV_USER_DATA_PATH ??
  getDefaultDevUserDataPath()
// Why: custom dev profiles do not necessarily contain "kondex-dev" in their path; carry explicit provenance into the CLI.
process.env.ORCA_DEV_CLI_INVOCATION = '1'
process.env.KONDEX_CLI_COMMAND = 'kondex-dev'
process.env.ORCA_CLI_COMMAND = 'kondex-dev'
...
// Why: headless `kondex-dev serve` skips the Electron dev runner that normally installs terminal CLI shims.
