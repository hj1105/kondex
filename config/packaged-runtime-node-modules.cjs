...
const { runtimeImportSpecifiers } = require('./scripts/verify-skills-cli-runtime.cjs')
...
// Why source maps: they embed dependency source trees and can add megabytes to the bundle, while
// nothing in the packaged app turns on Node's source-map support, so they are never read.
// The app's own main-process maps live outside node_modules and are handled separately.
