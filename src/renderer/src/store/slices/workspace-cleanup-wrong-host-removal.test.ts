...
describe('STA-4343 wrong-host cleanup removal (git worktree identity)', () => {
  it('removes only the confirmed SSH host path when two legacy runtime hosts share an id', async () => {
    ...
    installRemovalTransports({ [hostA]: hosts.hostARoot, [hostB]: hosts.hostBRoot }, routedHostIds)
    ...
    expect(routedHostIds).toEqual([hostB])
    expect(fs.existsSync(hosts.hostAMarkerPath)).toBe(true)
