...
function seedBatchStore() {
  ...
  const tab = makeTab({ id: 'tab-1', worktreeId: 'wt-1', title: 'codex' })
...
function makeUpdates(): AgentStatusBatchUpdate[] {
  return [
    ...
    {
      paneKey: FIRST_PANE,
      payload: { state: 'working', prompt: 'second turn', agentType: 'codex' },
      timing: { updatedAt: BASE_TIME + 3, stateStartedAt: BASE_TIME + 3 }
    },
    {
      paneKey: FIRST_PANE,
      payload: { state: 'done', prompt: 'late first turn', agentType: 'codex' },
      timing: { updatedAt: BASE_TIME + 2, stateStartedAt: BASE_TIME + 2 }
    },
    {
    ...
    {
      paneKey: SECOND_PANE,
      payload: { state: 'working', prompt: 'provider turn', agentType: 'codex' },
...
describe('setAgentStatuses', () => {
  ...
  it('coalesces batch freshness requests while preserving standalone scheduling', async () => {
    ...
    store
      ...
      .setAgentStatus(
        FIRST_PANE,
        { state: 'working', prompt: 'standalone', agentType: 'codex' },
        undefined,
    ...
    store
      ...
      .setAgentStatus(
        FIRST_PANE,
        { state: 'working', prompt: 'standalone stale', agentType: 'codex' },
        undefined,
  ...
  it('shares freshness across nested folds but isolates synchronous reentrant transactions', async () => {
    ...
    const update = (paneKey: string, updatedAt: number): AgentStatusBatchUpdate => ({
      paneKey,
      payload: { state: 'working', prompt: paneKey, agentType: 'codex' },
      timing: { updatedAt, stateStartedAt: updatedAt },
      routing: { tabId: 'tab-1', worktreeId: 'wt-1' }
    })
    let publications = 0
  ...
  it('keeps one completed-turn message per entry instead of one per history row', () => {
    ...
    for (let turn = 0; turn < 5; turn += 1) {
      turns.push({
        paneKey: FIRST_PANE,
        payload: { state: 'working', prompt: `turn ${turn}`, agentType: 'codex' },
  ...
  it('clears the completed-turn message when a done carried no assistant output', () => {
    ...
    store.getState().setAgentStatuses([
      ...
      {
        paneKey: FIRST_PANE,
        payload: { state: 'done', prompt: 'second turn', agentType: 'codex' },
        timing: { updatedAt: BASE_TIME + 3, stateStartedAt: BASE_TIME + 3 }
      },
      {
        paneKey: FIRST_PANE,
        payload: { state: 'working', prompt: 'third turn', agentType: 'codex' },
