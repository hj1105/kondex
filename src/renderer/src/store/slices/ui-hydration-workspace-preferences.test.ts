...
describe('createUISlice hydratePersistedUI', () => {
  ...
  it('ignores retired task-page resume state during hydration', () => {
    const store = createUIStore()
    const persisted = Object.assign(makePersistedUI({}), {
      taskResumeState: { githubMode: 'project', githubItemsPreset: 'invalid', githubItemsQuery: 42 }
    })
    store.getState().hydratePersistedUI(persisted)
    expect(store.getState()).not.toHaveProperty('taskResumeState')
  })
