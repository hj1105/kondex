...
describe('createWebRuntimeSessionTerminal', () => {
  ...
  it('uses the exact legacy Codex resume when an older host lacks resume authority', async () => {
      ...
      if (request.method === 'terminal.ensureAgentSession') {
        return {
          ...
          error: {
            ...
            message: 'structured dispatch must not run'
          }
        }
      }
      return {
    ...
    await expect(
      createWebRuntimeSessionTerminal({
        ...
        launchConfig: {
          agentCommand: 'codex',
          agentArgs: '',
          agentEnv: { CODEX_HOME: '/custom/codex' }
        },
        providerSession: { key: 'session_id', id: 'session-1' }
      })
    ).resolves.toEqual({ status: 'created' })
