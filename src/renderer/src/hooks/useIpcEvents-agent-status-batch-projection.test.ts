...
describe('useIpcEvents agent status snapshot integration', () => {
  ...
  it('projects ordered tab titles across panes in an inactive split snapshot', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(store.getState().tabsByWorktree[worktreeId]?.[0]?.title).toBe('Codex ready')
    })
  })
  ...
  it('projects synthetic pane titles across ordered same-pane batch updates', async () => {
    ...
    try {
      ...
      emit(1_700_000_000_000, 'working', 'codex')
      emit(1_700_000_000_001, 'waiting', 'codex')
      emit(1_700_000_000_002, 'waiting', 'codex')
      emit(1_700_000_000_003, 'done', 'codex')
      emit(1_700_000_000_002, 'waiting', 'codex')
      vi.advanceTimersByTime(40)
      ...
      expect(setAgentStatuses.mock.calls[0][0]).toEqual([
        expect.objectContaining({
          payload: expect.objectContaining({ agentType: 'codex', state: 'waiting' }),
          terminalTitle: 'Codex - action required'
        }),
        expect.objectContaining({
          payload: expect.objectContaining({ agentType: 'codex', state: 'waiting' }),
          terminalTitle: 'Codex - action required'
        ...
        expect.objectContaining({
          payload: expect.objectContaining({ agentType: 'codex', state: 'done' }),
