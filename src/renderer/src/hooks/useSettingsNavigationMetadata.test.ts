...
describe('settings navigation metadata', () => {
  ...
  it('keeps local automations first under Workflows', () => {
    const sections = buildSettingsNavigationMetadata({
      isMac: false,
      isWindows: false,
      isWebClient: false,
    ...
    expect(workflowIds).not.toContain('artifacts')
  })

  it('omits upstream cloud product settings', () => {
    expect(ids()).not.toEqual(expect.arrayContaining(['orca-account', 'mobile']))
    expect(ids()).not.toContain('setup-guide')
  })
