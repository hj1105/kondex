...
describe('useComposerState host-context boundaries', () => {
  ...
  it('keeps submit gates wired without blocking on retired Jira URL intent', () => {
    expect(COMPOSER_SOURCE.workspaceIdentity).toContain('const sourceIntentBlocksCreate = false')
    expect(COMPOSER_SOURCE.workspaceIdentity).not.toContain('isBlockingJiraUrlIntent')
  ...
  it('does not discover or launch retired VM recipes during workspace creation', () => {
    for (const source of Object.values(COMPOSER_SOURCE)) {
      expect(source).not.toContain('useEphemeralVmRecipeOptions')
      expect(source).not.toContain('window.api.ephemeralVm')
      expect(source).not.toContain('selectedEphemeralVmRecipeId')
    }
  })
})
