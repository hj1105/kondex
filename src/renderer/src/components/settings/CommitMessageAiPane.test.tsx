...
describe('CommitMessageAiPane', () => {
  ...
  it('only offers non-interactive generation agents for text generation actions', () => {
    ...
    expect(getAgentCatalogForAction('fixChecks', null).map((agent) => agent.id)).toEqual([
      'claude',
      'codex'
    ])
  })
  ...
  it('normalizes a retired saved text-recipe agent and shows supported alternatives', () => {
    const markup = renderPane(
      buildSettings({
        sourceControlAi: {
          enabled: true,
          ...
          actions: {
            commitMessage: {
              // @ts-expect-error Legacy settings may name a retired provider.
              agentId: 'aider'
            }
          },
          prCreationDefaults: {},
    ...
    expect(markup).not.toContain('Aider')
    expect(markup).toContain('Supported agents for this recipe:')
    expect(markup).toContain('Claude, Codex')
  })

  it('keeps action agent selector widths constrained', () => {
    const markup = renderPane(
      buildSettings({
        commitMessageAi: {
          enabled: true,
  ...
  it('allows default-agent recipes even when the old default generator is unsupported', () => {
    const markup = renderPane(
      buildSettings({
        // @ts-expect-error Legacy defaults must not hide supported recipe controls.
  ...
  it('keeps SSH discovered models out of the legacy local cache', () => {
    ...
    expect(merged.discoveredModelsByAgent?.claude).toEqual([{ id: 'auto', label: 'Auto' }])
    expect(merged.selectedModelByAgentByHost?.['ssh:conn-1']?.claude).toBe('remote-only')
    expect(merged.discoveredModelsByAgentByHost?.['ssh:conn-1']?.claude).toEqual([
      { id: 'remote-only', label: 'Remote Only' }
    ])
  })
