...
export function getHostDetailsDescription(details: RuntimeHostDetails | undefined): string | null {
  ...
  if (details.runtimeStatus === null && details.compatibility === null) {
    const unavailableDescription = translate(
      ...
      'SSH transport is connected, but the Kondex runtime did not answer. The host may still be running.'
    )
...
export function getActiveServerModeDescription(allowLocalRuntime: boolean): string {
  return allowLocalRuntime
    ? translate(
        ...
        'Use this computer by default. Choose a saved server only when you want supported projects, files, terminals, provider checks, and browser handoff to run through that server.'
      )
    : translate(
        'auto.components.settings.RuntimeEnvironmentsPane.2c85efb3e8',
        'Selecting a saved server makes this browser use that paired Kondex runtime as its default Host.'
      )
}
