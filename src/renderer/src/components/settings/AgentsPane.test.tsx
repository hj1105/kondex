...
describe('AgentsPane', () => {
  ...
  it('renders the keep-awake modes from settings', () => {
    ...
    expect(markup).toContain(
      'Choose On, Agent, or Off. Agent mode stays awake while agents are working. Kondex also asks this device to stay awake when the lid is closed, subject to its power policy.'
    )
  ...
  it('keeps catalog agent ids, labels, and commands discoverable in settings search', () => {
    ...
    for (const query of [
      ...
      expect(matchesSettingsSearch(query, getAgentsPaneSearchEntries()), query).toBe(false)
    }
  })
