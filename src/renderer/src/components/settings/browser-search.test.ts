...
describe('browser settings search copy', () => {
  ...
  it('keeps the pre-feature wording while inverting is off', () => {
    expect(getBrowserLinkRoutingDescription({ isMac: true })).toBe(
      "Open http(s) links in Kondex's built-in browser — from the terminal, markdown, and the editor. ⇧⌘-click always uses your system browser."
    )
...
describe('browser link routing modifier copy', () => {
  ...
  it('names the destination the modifier actually reaches', () => {
    expect(getLinkRoutingModifierTitle(false)).toBe('Hold Shift to open in Kondex')
  ...
  it('points the description at Kondex only when links currently open externally', () => {
    expect(getLinkRoutingModifierDescription({ openLinksInApp: false, isMac: true })).toContain(
      "Kondex's built-in browser"
    )
  ...
  it('phrases the Kondex branch as enabled-state copy', () => {
    expect(getLinkRoutingModifierDescription({ openLinksInApp: false, isMac: true })).toContain(
      'When enabled'
    )
  })
...
describe('Link Routing description localization', () => {
  ...
  it('renders the Korean copy with the shortcut interpolated', async () => {
    ...
    expect(description).not.toContain("Kondex's built-in browser")
    ...
    await i18n.changeLanguage('en')
    expect(getBrowserLinkRoutingDescription({ isMac: true })).toContain("Kondex's built-in browser")
  })
