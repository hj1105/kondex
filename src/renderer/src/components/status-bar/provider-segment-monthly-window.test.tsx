...
// Synthetic provider snapshots pin optional-window rendering independently of discovery.
function monthlyLimits(status: ProviderRateLimits['status']): ProviderRateLimits {
  return {
    provider: 'codex',
    session: null,
    weekly: null,
...
describe('ProviderSegment monthly window', () => {
  it('renders a monthly-only snapshot in the chip instead of a bare icon', async () => {
    ...
    const markup = renderToStaticMarkup(
      <ProviderSegment p={monthlyLimits('ok')} compact={false} display="used" mode="compact" />
    )
  ...
  it('selects a named bucket as the tightest provider window', async () => {
    ...
    const limits: ProviderRateLimits = {
      ...
      weekly: null,
      buckets: [
        { ...windowOf(25, 300), name: 'Fast' },
        { ...windowOf(80, 300), name: 'Reasoning' }
      ],
      updatedAt: Date.now(),
      error: null,
      status: 'ok'
    ...
    expect(markup).toContain('80% used Reasoning')
