  ...
  return {
    ...
    descriptionSuffix: args.isBatchDelete
      ? allFolderWorkspaceDeletes
        ? 'from Kondex. Project folders on disk will not be deleted.'
        : mixedFolderWorkspaceDeletes
          ? 'from Kondex. Git worktrees will also be removed from git and disk; folder workspaces will only remove the Kondex workspace entry.'
          : 'from git and delete their workspace folders.'
      : args.isFolderWorkspaceDelete
        ? 'from Kondex. The project folder on disk will not be deleted.'
