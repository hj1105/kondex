...
describe('buildTitleDerivedAgentRows', () => {
  ...
  it('still resolves Claude from a title that presents Claude, owner or not', () => {
    ...
    // Pane reuse: the user exited Codex and ran claude in the same pane.
    expect(rowsFor('✳ Claude Code', 'codex').map((row) => row.agentType)).toEqual(['claude'])
    ...
    expect(rowsFor('zsh', 'codex')).toHaveLength(0)
  })
