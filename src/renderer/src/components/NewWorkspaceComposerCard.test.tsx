...
describe('NewWorkspaceComposerCard folder task source mode', () => {
  ...
  it('passes folder child repos into the create-from field without a source trigger', () => {
    ...
    expect(current.container.textContent).not.toContain('Task Source')
  ...
  it('keeps the project picker separate from the run target picker', () => {
    current = renderCard({ projectHostSetupOptions: [localReadyHostOption] })

    const projectPicker = current.container.querySelector('[data-project-combobox-root="true"]')
    expect(projectPicker).toBeTruthy()
    expect(projectPicker?.querySelector('[data-run-target-combobox-root="true"]')).toBeNull()
    expect(projectPicker?.textContent).not.toContain('Run on')
  ...
  it('opens the add-host submenu on hover without a click', () => {
    ...
    expect(findRunTargetItem('Add Remote Kondex Server')).toBeTruthy()
  })

  it('opens the remote Kondex server add dialog over the composer without leaving for Settings', () => {
    current = renderCard({
      projectHostSetupOptions: [localReadyHostOption, devboxNeedsSetupHostOption],
      selectedProjectHostSetupId: 'setup-local'
    })
    ...
    openRunTargetPicker(current.container)
    act(() => findRunTargetItem('Add host')?.click())
    act(() => findRunTargetItem('Add Remote Kondex Server')?.click())
