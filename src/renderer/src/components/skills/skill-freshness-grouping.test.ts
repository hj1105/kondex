...
describe('groupSkillFreshness', () => {
  ...
  it('preserves a newer independent copy when a blocked sibling exposes its row', () => {
    const [group] = groupSkillFreshness(
      [
        placement('kondex-cli'),
        placement('kondex-cli', {
          topology: 'independent-copy',
          status: 'newer-known',
          unresolvedPath: '/home/.claude/skills/kondex-cli'
        })
      ],
      []
    )
    expect(group.status).toBe('cannot-update')
    expect(group.locations.find((location) => location.path.includes('.claude'))?.chip).toBe(
      'newer'
    )
    const reason = skippedReason(group.locations, group.name)
    expect(reason).toContain('later version')
    expect(reason).not.toMatch(/remove|reinstall|skills install/i)
  })

  it('keeps a completed independent copy current in the pinned update result', () => {
    const [group] = groupSkillFreshness(
      [placement('kondex-cli', { topology: 'independent-copy', status: 'current' })],
      [],
      ['kondex-cli']
    )
    expect(group.locations[0]?.chip).toBe('current')
  })
