...
describe('SkillFreshnessUpdateDialog', () => {
  ...
  it('shows the captured log verbatim without parsing it', async () => {
    ...
    await emitRun({
      ...
      names: ['orca-cli'],
      finishedAt: 2,
      output: 'kondex-cli: updated'
    })

    expect(container?.querySelector('pre')?.textContent).toContain('kondex-cli: updated')
  })
  ...
  it('shows the up-to-date state once every installation is current', async () => {
    ...
    expect(container?.textContent).toContain('All installed Kondex skills are up to date.')
  ...
  it('names the skill in the stale-record remedy so the command is runnable as-is', async () => {
    ...
    expect(container?.textContent).toContain('kondex skills install --skill orchestration')
  })
  ...
  it('keeps the stale-record remedy when a project copy is listed beside it', async () => {
    ...
    expect(row?.textContent).toContain('kondex skills install --skill orchestration')
  ...
  it('stays coherent while an idle re-scan is in flight', async () => {
    ...
    expect(container?.textContent).toContain('Checking installed Kondex skills…')
  ...
  it('shows incomplete plugin coverage without presenting a fabricated skill copy', async () => {
    ...
    expect(container?.textContent).toContain(
      'Kondex could not finish checking plugin-managed skills.'
    )
    ...
    expect(container?.textContent).not.toContain('All installed Kondex skills are up to date.')
