...
describe('SkillsPage', () => {
  ...
  it('preserves hidden selections when selecting all filtered results', async () => {
    ...
    await renderPage()
    await flushMicrotasks()
    await act(async () => fireEvent.click(buttonNamed('Delete skills…')))
  ...
  it('shows why bundled skills are disabled while allowing duplicate local names', async () => {
    const discover = vi.fn().mockResolvedValue({
      skills: [
    ...
    await flushMicrotasks()
    await act(async () => fireEvent.click(buttonNamed('Delete skills…')))
    expect(container?.textContent).toContain('Bundled with Kondex — it would be restored')
