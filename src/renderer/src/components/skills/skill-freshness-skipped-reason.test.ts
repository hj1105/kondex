...
describe('skippedReason', () => {
  it('requires verified ownership instead of advising deletion of a separate copy', () => {
    const reason = skippedReason([row('current'), row('duplicate')])
    expect(reason).toContain('separate copy')
    expect(reason).toContain('installation record')
    expect(reason).toContain('unchanged files')
    expect(reason).not.toMatch(/only refreshes the main copy|remove|reinstall/i)
  })
  ...
  it('hands over the reinstall command when no location is at fault', () => {
    ...
    expect(reason).toContain('installation record could not be verified')
    expect(reason).toContain('kondex skills install --skill orchestration')
  })
