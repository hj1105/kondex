...
vi.mock('../../../../shared/tui-agent-config', async (importOriginal) => {
  ...
  return {
    ...
    TUI_AGENT_CONFIG: {
      ...
      codex: { ...actual.TUI_AGENT_CONFIG.codex, windowsShiftEnterEncoding: 'csi-u' }
    }
  }
})
...
describe('resolveDashboardCardTerminalInput', () => {
  ...
  it('withholds kitty advertisement on ConPTY for a supported launch identity', () => {
  ...
  it('relays trusted Ctrl+Enter authority without coupling it to Shift+Enter', () => {
    const claude = resolveDashboardCardTerminalInput(
      stateWith({
    ...
    expect(claude.ctrlEnterCsiU).toBe(true)

    const codex = resolveDashboardCardTerminalInput(
      stateWith({
        paneForegroundAgentByPaneKey: {
          [WINDOWS_ARGS.paneKey]: {
            agent: 'codex',
    ...
    expect(codex.windowsShiftEnterEncoding).toBe('csi-u')
    expect(codex.ctrlEnterCsiU).toBe(false)
  })
