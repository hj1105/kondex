...
describe('buildDashboardWorktreeLaunchOptions', () => {
  ...
  it('filters retired and disabled providers while honoring the default', () => {
    const options = buildDashboardWorktreeLaunchOptions(
      state({
  ...
  it('uses each git workspace execution host instead of local detection', () => {
    const options = buildDashboardWorktreeLaunchOptions(
      state({
        ...
        runtimeDetectedAgentIds: { 'hub-1': ['claude'] }
      }),
      [
        card({ repoId: 'repo-ssh', worktreeId: 'ssh-worktree', agentType: 'codex' }),
        card({ repoId: 'repo-runtime', worktreeId: 'runtime-worktree', agentType: 'claude' })
      ]
    )

    expect(options).toEqual({ 'ssh-worktree': ['codex'], 'runtime-worktree': ['claude'] })
  })
  ...
  it('resolves folder workspace detection through its project host', () => {
    ...
    const options = buildDashboardWorktreeLaunchOptions(
      state({
        ...
        remoteDetectedAgentIds: { 'ssh-folder': ['claude'] }
      }),
      [card({ repoId: 'folder-workspace:group-1', worktreeId })]
    )

    expect(options).toEqual({ [worktreeId]: ['claude', 'codex'] })
  })
})
