...
describe('registerTerminalSideEffectFactConsumer', () => {
  ...
  it('hands off facts emitted between the parked watcher unregistering and the pane registering', () => {
    ...
    // and replay is title-only, so an unbuffered bell/agent-idle is lost.
    ...
    const events: unknown[][] = []
    _dispatchTerminalSideEffectBatchForTest(
      batch([{ kind: 'bell' }, { kind: 'agent-idle', title: 'Claude ready' }], {
        seq: 7
      })
    )
    ...
    expect(events).toEqual([['bell'], ['idle', 'Claude ready']])
    expect(watcher.events).toEqual([])
    ...
    registerTerminalSideEffectFactConsumer({
      ...
      callbacks: {
        ...
        onAgentBecameIdle: (title) => later.push(['idle', title])
      }
    })
