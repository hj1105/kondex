...
describe('terminal link action routing', () => {
  ...
  it('offers system browser first and Kondex second when system browser is the default', () => {
    const request = vi.fn()
    ...
    expect(request.mock.calls[0][0].alternate.label).toBe('Kondex Browser')
  ...
  it('offers Kondex first and system browser second when Kondex is the default', () => {
    const request = vi.fn()
    ...
    expect(request.mock.calls[0][0].primary.label).toBe('Kondex Browser')
  ...
  it('routes an explicit Kondex Browser action to the owning runtime', () => {
    const request = vi.fn()
  ...
  it('routes an explicit Kondex Browser action through the owning SSH workspace', () => {
    const request = vi.fn()
