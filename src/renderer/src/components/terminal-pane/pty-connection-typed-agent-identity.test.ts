...
import type * as CommandInferredPaneAgent from './pty-connection/command-inferred-pane-agent'
...
const inferredSessions = vi.hoisted(() => [] as ConnectPanePtySession[])
vi.mock('./pty-connection/command-inferred-pane-agent', async (importOriginal) => {
  const actual = await importOriginal<typeof CommandInferredPaneAgent>()
  return {
    ...actual,
    installCommandInferredPaneAgent: (session: ConnectPanePtySession) => {
      actual.installCommandInferredPaneAgent(session)
      inferredSessions.push(session)
    }
  }
})
...
describe('connectPanePty', () => {
  ...
  it.each([
    ...
    'keeps observed Codex status authoritative after typing %s',
    async (command, expectedHint) => {
      const { connectPanePty } = await import('./pty-connection')
      enableActiveRuntimeEnvironment()
      const paneKey = makePaneKey('tab-1', LEAF_1)
      const pane = createPane(1)
      const transport = createMockTransport('remote:web-env-1@@pty-claude')
      transportFactoryQueue.push(transport)
      const manager = createManager(1, 1)
      const deps = createDeps()

      connectPanePty(pane as never, manager as never, deps as never)
      sendTerminalInputThroughPane(pane, `${command}\r`)
      await flushAsyncTicks()
      expect(inferredSessions).toHaveLength(1)
      expect(inferredSessions[0]?.commandInferredPaneAgent).toBe(expectedHint)
      ...
      const onAgentStatus = createdTransportOptions[0]?.onAgentStatus as
        | ((payload: { state: 'done'; prompt: string; agentType: 'codex' }) => void)
        | undefined
      if (!onTitleChange || !onAgentStatus) {
        throw new Error('missing remote PTY callbacks')
      }
      onTitleChange('Codex ready', 'Codex ready')
      onAgentStatus({
        state: 'done',
        prompt: '',
        agentType: 'codex'
      ...
      expect(transport.sendInput).toHaveBeenCalledWith(`${command}\r`)
      expect(deps.setRuntimePaneTitle).toHaveBeenCalledWith('tab-1', 1, 'Codex ready')
      expect(deps.updateTabTitle).toHaveBeenCalledWith('tab-1', 'Codex ready')
  ...
  it('drives runtime title, tab title, and renderer policy from one title decision', async () => {
    ...
    // A retired provider title does not force a renderer fallback.
    expect(manager.setPaneGpuRendering).toHaveBeenCalledWith(1, true)
  })

  it('keeps GPU enabled when a pane-scoped Claude owner emits a Gemini-looking title', async () => {
    const { connectPanePty } = await import('./pty-connection')
    const paneKey = makePaneKey('tab-1', LEAF_1)
    mockStoreState = {
      ...mockStoreState,
    ...
    const pane = createPane(1)
    const transport = createMockTransport('pty-claude-gemini-cwd')
    transportFactoryQueue.push(transport)
    const manager = createManager(1, 1)
    const deps = createDeps()
    ...
    // Active pane ownership does not revive a retired provider-specific fallback.
    expect(manager.setPaneGpuRendering).toHaveBeenCalledWith(1, true)
  ...
  it('keeps the default renderer for a retired Gemini split pane even when the tab launched a non-Gemini agent', async () => {
    const { connectPanePty } = await import('./pty-connection')
    // The shared tab.launchAgent must not veto the renderer for a sibling pane.
    mockStoreState = {
      ...mockStoreState,
      tabsByWorktree: {
        'wt-1': [{ id: 'tab-1', ptyId: 'tab-pty', launchAgent: 'claude' }]
      }
    } as StoreState
    const pane1 = createPane(1)
    const pane2 = createPane(2)
    ...
    expect(manager.setPaneGpuRendering).toHaveBeenCalledWith(2, true)
  })

  it('keeps the default renderer for a retired Gemini title in a pane whose launch agent was non-Gemini', async () => {
    const { connectPanePty } = await import('./pty-connection')
    const pane = createPane(1)
  ...
  it('keeps the default renderer for a retired Gemini title when the only pane row is a done non-Gemini agent', async () => {
    const { connectPanePty } = await import('./pty-connection')
    const paneKey = makePaneKey('tab-1', LEAF_1)
    mockStoreState = {
      ...mockStoreState,
  ...
  it('keeps the default renderer for a retired Gemini title when the only pane row is a stale non-Gemini agent', async () => {
    const { connectPanePty } = await import('./pty-connection')
    const paneKey = makePaneKey('tab-1', LEAF_1)
    ...
    // A stale working row must not revive a retired provider-specific fallback.
    expect(manager.setPaneGpuRendering).toHaveBeenCalledWith(1, true)
  })

  it('infers Claude after shell word deletion without rewriting observed status', async () => {
    const { connectPanePty } = await import('./pty-connection')
    enableActiveRuntimeEnvironment()
    const paneKey = makePaneKey('tab-1', LEAF_1)
    const pane = createPane(1)
    const transport = createMockTransport('remote:web-env-1@@pty-claude-edited')
    transportFactoryQueue.push(transport)
    const manager = createManager(1, 1)
    const deps = createDeps()

    connectPanePty(pane as never, manager as never, deps as never)
    sendTerminalInputThroughPane(pane, 'codex \x17claude\r')
    await flushAsyncTicks()
    expect(inferredSessions).toHaveLength(1)
    expect(inferredSessions[0]?.commandInferredPaneAgent).toBe('claude')
    const onTitleChange = createdTransportOptions[0]?.onTitleChange as
      | ((title: string, rawTitle: string) => void)
      | undefined
    const onAgentStatus = createdTransportOptions[0]?.onAgentStatus as
    ...
    expect(transport.sendInput).toHaveBeenCalledWith('codex \x17claude\r')
    expect(deps.setRuntimePaneTitle).toHaveBeenCalledWith('tab-1', 1, 'Codex ready')
    expect(mockStoreState.agentStatusByPaneKey[paneKey]).toMatchObject({
      state: 'done',
      agentType: 'codex',
  ...
  it('keeps Codex remote runtime status as Codex after typed codex command', async () => {
    const { connectPanePty } = await import('./pty-connection')
    enableActiveRuntimeEnvironment()
    const paneKey = makePaneKey('tab-1', LEAF_1)
    const pane = createPane(1)
    const transport = createMockTransport('remote:web-env-1@@pty-codex')
    transportFactoryQueue.push(transport)
    const manager = createManager(1, 1)
    const deps = createDeps()
    ...
    await flushAsyncTicks()
    expect(inferredSessions).toHaveLength(1)
    expect(inferredSessions[0]?.commandInferredPaneAgent).toBe('codex')
    const onTitleChange = createdTransportOptions[0]?.onTitleChange as
      | ((title: string, rawTitle: string) => void)
      | undefined
    const onAgentStatus = createdTransportOptions[0]?.onAgentStatus as
  ...
  it('does not infer shell ownership from prompts typed inside an existing Codex session', async () => {
    const { connectPanePty } = await import('./pty-connection')
    enableActiveRuntimeEnvironment()
    const paneKey = makePaneKey('tab-1', LEAF_1)
    const now = Date.now()
    ...
    mockStoreState.runtimePaneTitlesByTabId = { 'tab-1': { 1: 'Codex ready' } }
    const pane = createPane(1)
    const transport = createMockTransport('remote:web-env-1@@pty-codex-prompt')
    transportFactoryQueue.push(transport)
    const manager = createManager(1, 1)
    const deps = createDeps()
    ...
    await flushAsyncTicks()
    expect(inferredSessions).toHaveLength(1)
    expect(inferredSessions[0]?.commandInferredPaneAgent).toBe(null)
    const onTitleChange = createdTransportOptions[0]?.onTitleChange as
      | ((title: string, rawTitle: string) => void)
      | undefined
    const onAgentStatus = createdTransportOptions[0]?.onAgentStatus as
    ...
    expect(transport.sendInput).toHaveBeenCalledWith('claude\r')
    expect(deps.setRuntimePaneTitle).toHaveBeenCalledWith('tab-1', 1, 'Codex ready')
    expect(mockStoreState.agentStatusByPaneKey[paneKey]).toMatchObject({
      state: 'done',
      agentType: 'codex',
  ...
  it('does not infer shell ownership from prompts typed in a title-only Codex session', async () => {
    const { connectPanePty } = await import('./pty-connection')
    enableActiveRuntimeEnvironment()
    const paneKey = makePaneKey('tab-1', LEAF_1)
    mockStoreState.runtimePaneTitlesByTabId = { 'tab-1': { 1: 'Codex ready' } }
    const pane = createPane(1)
    const transport = createMockTransport('remote:web-env-1@@pty-codex-title-only')
    transportFactoryQueue.push(transport)
    const manager = createManager(1, 1)
    const deps = createDeps()
  ...
  it('accepts a new Claude hint despite stale retained done status', async () => {
    const { connectPanePty } = await import('./pty-connection')
    enableActiveRuntimeEnvironment()
    const paneKey = makePaneKey('tab-1', LEAF_1)
    const now = Date.now()
  ...
  it('tracks cursor edits when inferring a typed claude command', async () => {
    const { connectPanePty } = await import('./pty-connection')
    enableActiveRuntimeEnvironment()
    const paneKey = makePaneKey('tab-1', LEAF_1)
    const pane = createPane(1)
    ...
    connectPanePty(pane as never, manager as never, deps as never)
    sendTerminalInputThroughPane(pane, 'clade\x1b[D\x1b[Du\r')
    await flushAsyncTicks()
    expect(inferredSessions).toHaveLength(1)
    expect(inferredSessions[0]?.commandInferredPaneAgent).toBe('claude')
    const onTitleChange = createdTransportOptions[0]?.onTitleChange as
    ...
    expect(transport.sendInput).toHaveBeenCalledWith('clade\x1b[D\x1b[Du\r')
    expect(mockStoreState.agentStatusByPaneKey[paneKey]).toMatchObject({
      state: 'done',
      agentType: 'codex',
      terminalTitle: 'Codex ready'
  ...
  it('tracks delete-key cursor edits when inferring a typed claude command', async () => {
    const { connectPanePty } = await import('./pty-connection')
    enableActiveRuntimeEnvironment()
    const paneKey = makePaneKey('tab-1', LEAF_1)
    const pane = createPane(1)
    ...
    connectPanePty(pane as never, manager as never, deps as never)
    sendTerminalInputThroughPane(pane, 'claudde\x1b[D\x1b[D\x1b[3~\r')
    await flushAsyncTicks()
    expect(inferredSessions).toHaveLength(1)
    expect(inferredSessions[0]?.commandInferredPaneAgent).toBe('claude')
    const onTitleChange = createdTransportOptions[0]?.onTitleChange as
  ...
  it('skips manual agent inference for large paste chunks', async () => {
    ...
    connectPanePty(pane as never, manager as never, deps as never)
    sendTerminalInputThroughPane(pane, `${'x'.repeat(4097)}claude\r`)
    await flushAsyncTicks()
    expect(inferredSessions).toHaveLength(1)
    expect(inferredSessions[0]?.commandInferredPaneAgent).toBe(null)
    const onTitleChange = createdTransportOptions[0]?.onTitleChange as
