...
vi.mock('./terminal-pane-split-completion', () => ({
  wasTerminalPaneCreated: mocks.wasTerminalPaneCreated
}))
...
describe('splitTerminalPaneWithInheritedCwd', () => {
  beforeEach(() => {
    mocks.wasTerminalPaneCreated.mockReset()
  ...
  it('keeps the existing local split-and-focus path unchanged', () => {
    ...
    expect(mocks.wasTerminalPaneCreated).toHaveBeenCalledWith(createdPane)
  })
  ...
  it('does not resolve cwd or split a stale manager when the live manager is gone', () => {
    ...
    expect(mocks.wasTerminalPaneCreated).not.toHaveBeenCalled()
  })
})
