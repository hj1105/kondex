...
describe('createRemoteRuntimePtyTransport', () => {
  ...
  it('degrades a Codex resume to a legacy launch when the host lacks resume authority', async () => {
    const { createRemoteRuntimePtyTransport } = await import('./remote-runtime-pty-transport')
    runtimeCall.mockResolvedValueOnce({
      id: 'rpc-status',
      ok: true,
      result: { runtimeProtocolVersion: 3, minCompatibleRuntimeClientVersion: 2, capabilities: [] },
      _meta: { runtimeId: 'runtime-remote' }
    })
    const transport = createRemoteRuntimePtyTransport('env-1', {
      worktreeId: 'repo1::/remote/wt',
      command: "codex 'resume' 'session_431324d7'",
      launchAgent: 'codex',
    ...
    // The read-only capability probe must select legacy before structured dispatch.
    expect(runtimeCall).not.toHaveBeenCalledWith(
    ...
    expect(runtimeCall).toHaveBeenCalledWith(
      expect.objectContaining({
        method: 'terminal.create',
        params: expect.objectContaining({ command: "codex 'resume' 'session_431324d7'" })
      })
    )
  })
