...
describe('createIpcPtyTransport', () => {
  ...
  it('bounds repeated retired-provider titles without restoring their special compaction', async () => {
    vi.useFakeTimers()
    try {
      ...
      const callbacks = { onData: vi.fn() }
      const repeatedTitles = Array.from({ length: 4_096 }, () => '\x1b]0;Cursor Agent\x07').join('')

      processor.processData(repeatedTitles, callbacks)

      expect(callbacks.onData).toHaveBeenCalledWith(repeatedTitles)
      expect(onTitleChange).not.toHaveBeenCalled()
      expect(vi.getTimerCount()).toBe(1)
      await vi.runOnlyPendingTimersAsync()

      expect(onTitleChange.mock.calls.length).toBeGreaterThan(0)
      expect(onTitleChange.mock.calls.length).toBeLessThan(MAX_PENDING_PTY_SIDE_EFFECTS)
      expect(vi.getTimerCount()).toBe(1)
      await vi.runAllTimersAsync()

      expect(vi.getTimerCount()).toBe(0)
      expect(onTitleChange).toHaveBeenCalledTimes(MAX_PENDING_PTY_SIDE_EFFECTS)
      expect(onTitleChange.mock.calls).toEqual(
        Array.from({ length: MAX_PENDING_PTY_SIDE_EFFECTS }, () => ['Cursor Agent', 'Cursor Agent'])
      )
    } finally {
      vi.useRealTimers()
    }
  ...
  it('lets a Claude idle title clear a pending stale-title fallback', async () => {
    vi.useFakeTimers()
    try {
      const { createPtyOutputProcessor } = await import('./pty-transport')
      ...
      processor.processData('\x1b]0;Claude\x07', callbacks)
      await vi.advanceTimersByTimeAsync(0)
      expect(vi.getTimerCount()).toBe(0)
      expect(onAgentBecameIdle).toHaveBeenCalledExactlyOnceWith('Claude')
  ...
  it('re-arms stale-title fallback after a later title-free output scan', async () => {
    ...
    try {
      ...
      processor.processData('later plain output\r\n', callbacks)
      await vi.advanceTimersByTimeAsync(0)
      expect(vi.getTimerCount()).toBe(1)
      await vi.advanceTimersByTimeAsync(3_000)

      expect(onTitleChange).toHaveBeenCalledExactlyOnceWith('Claude', 'Claude')
      expect(vi.getTimerCount()).toBe(0)
    } finally {
      vi.useRealTimers()
    }
