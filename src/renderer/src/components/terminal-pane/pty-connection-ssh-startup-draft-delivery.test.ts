...
describe('connectPanePty', () => {
  ...
  it('delivers a Claude draft through provider-owned argv without a duplicate timed paste', async () => {
    vi.useFakeTimers()
    const { connectPanePty } = await import('./pty-connection')
    const capturedDataCallback: { current: ((data: string) => void) | null } = { current: null }
    const transport = createMockTransport('pty-claude')
    transport.connect.mockImplementation(async ({ callbacks }: { callbacks: ConnectCallbacks }) => {
      capturedDataCallback.current = callbacks.onData ?? null
      return 'pty-ssh-claude'
    })
    transportFactoryQueue.push(transport)
    mockStoreState = {
      ...mockStoreState,
    ...
    expect(createdTransportOptions[0]?.command).toBe(plan.launchCommand)
    expect(plan.launchCommand).toContain('--prefill')
    expect(plan.launchCommand).toContain(prompt)
    expect(transport.sendInput).not.toHaveBeenCalledWith(`${plan.launchCommand}\r`)
