...
export function getTerminalOrcaFileOpenHint(showActions = true): string {
  ...
  return isMacPlatform()
    ? `${prefix}⌘+click to open in Kondex`
    : `${prefix}Ctrl+click to open in Kondex`
}
...
export function getTerminalUrlOpenHint(options: TerminalUrlOpenHintOptions = {}): string {
  ...
  if (invertsToOrca) {
    return isMacPlatform()
      ? `${prefix}⌘+click to open, or ⇧⌘+click to open in Kondex`
      : `${prefix}Ctrl+click to open, or Shift+Ctrl+click to open in Kondex`
  }
  return isMacPlatform()
...
export function getTerminalUrlOrcaBrowserHint(): string {
  return isMacPlatform() ? '⇧⌘+click to open in Kondex' : 'Shift+Ctrl+click to open in Kondex'
}
