...
describe('connectPanePty', () => {
  ...
  it('does not infer a live agent from retired Cursor screen replay', async () => {
    const { connectPanePty } = await import('./pty-connection')
    enableActiveRuntimeEnvironment()
    const transport = createMockTransport('remote:env-1@@terminal-1')
    const capturedReplayCallback: {
  ...
  // Styling must not turn retired screen text into live process evidence.
  it('does not infer a live agent from a CSI-styled retired Cursor screen', async () => {
    const { connectPanePty } = await import('./pty-connection')
    enableActiveRuntimeEnvironment()
    const transport = createMockTransport('remote:env-1@@terminal-1')
    const capturedReplayCallback: {
  ...
  // Old provider text in a large replay must not establish current ownership.
  ...
  it('resets shell modes when retired Cursor scrollback precedes the visible shell', async () => {
    const { connectPanePty } = await import('./pty-connection')
    enableActiveRuntimeEnvironment()
    const transport = createMockTransport('remote:env-1@@terminal-1')
    const capturedReplayCallback: {
    ...
    const disposable = await withMockedDocumentActiveElement(textarea, async () => {
      ...
      // The shell reset must restore the cursor and disarm focus reporting.
