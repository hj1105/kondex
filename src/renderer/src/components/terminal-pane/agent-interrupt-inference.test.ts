...
describe('agent interrupt inference', () => {
  ...
  it('uses generic settling for an old Droid row without its retired special case', () => {
    vi.useFakeTimers()
  ...
  it.each(['opencode', 'copilot'] as const)(
    'does not restore retired double-Escape handling for %s',
    (agentType) => {
      vi.useFakeTimers()
      let entry: AgentStatusEntry | undefined = makeEntry({ agentType })
      const inferInterrupt = vi.fn()
  ...
  it('replaces a pending Claude interrupt with the new turn baseline', () => {
    vi.useFakeTimers()
    ...
    tracker.observeInputIntent('plain-escape')
    entry = makeEntry({ agentType: 'claude', prompt: 'second task', stateStartedAt: 1_050 })
    tracker.observeInputIntent('plain-escape')
    ...
    expect(inferInterrupt).toHaveBeenCalledExactlyOnceWith({
      paneKey: PANE_KEY,
      baselineUpdatedAt: 1_000,
      baselineStateStartedAt: 1_050,
      baselinePrompt: 'second task',
      baselineAgentType: 'claude',
      intent: 'plain-escape'
    })
    tracker.dispose()
  ...
  it.each(['claude', 'custom-agent'] as const)(
    'does not emit a second %s interrupt when its timer follows an explicit flush',
    (agentType) => {
      vi.useFakeTimers()
      let entry: AgentStatusEntry | undefined = makeEntry({ agentType })
      const inferInterrupt = vi.fn()
      ...
      tracker.observeInputIntent('plain-escape')
      expect(tracker.flushPending()).toBe(true)
      vi.runOnlyPendingTimers()
  ...
  it('does not emit another Codex Escape after the row is already done', () => {
    vi.useFakeTimers()
    let entry: AgentStatusEntry | undefined = makeEntry({ agentType: 'codex' })
  ...
  it('still infers Ctrl+C for an unknown agent status row', () => {
    vi.useFakeTimers()
