...
describe('connectPanePty', () => {
  ...
  it('preserves observed Codex identity despite a different tab launch identity', async () => {
    const { connectPanePty } = await import('./pty-connection')
    ...
    mockStoreState.runtimePaneTitlesByTabId = { 'tab-1': { 1: 'Codex working' } }
    ...
    expect(deps.updateTabTitle).toHaveBeenCalledWith('tab-1', 'Codex working')
    titleHandler('π: tmp', 'π: tmp')
    expect(deps.setRuntimePaneTitle).toHaveBeenLastCalledWith('tab-1', 1, 'π: tmp')
    expect(deps.updateTabTitle).toHaveBeenLastCalledWith('tab-1', 'π: tmp')

    const statusHandler = createdTransportOptions[0]?.onAgentStatus as
      | ((payload: { state: 'working'; prompt: string; agentType: 'codex' }) => void)
      | undefined
    if (!statusHandler) {
      throw new Error('Expected onAgentStatus to be registered')
    }
  ...
  it('keeps GPU rendering enabled when raw cwd text names a retired provider', async () => {
    const { connectPanePty } = await import('./pty-connection')
    ...
    titleHandler('Codex working', '\u280b π: gemini')
