...
describe('#12112 opencode startup OSC 10/11 replies on the local path', () => {
  ...
  it.each(['codex', 'claude'])(
    '%s agent pane consumes the historical query burst and renders nothing',
    async (agent) => {
      const tty = createStartupTty()
      stubPtyApi(tty)
      const pane = await createRendererPane()

      // Keep the historical query bytes while exercising retained agent admission.
      expect(isTuiAgent(agent)).toBe(true)
