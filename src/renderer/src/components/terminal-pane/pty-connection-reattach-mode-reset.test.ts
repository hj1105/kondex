...
describe('connectPanePty', () => {
  ...
  it('preserves live modes and injects focus-in after focused agent reattach', async () => {
    ...
    await withMockedDocumentActiveElement(textarea, async () => {
      ...
      // Snapshot ends with ?25l (the saved TUI hides the cursor); the reset must preserve it, not force ?25h, or a stray block paints.
  ...
  it('does not treat ordinary shell scrollback mentioning Codex as a live agent reattach', async () => {
    const { connectPanePty } = await import('./pty-connection')
    const transport = createMockTransport('tab-pty')
    transport.connect.mockImplementation(async ({ sessionId }: { sessionId?: string }) => {
      if (sessionId) {
        return {
          id: sessionId,
          snapshot: '\x1b[?1004h\x1b[?25l$ grep -R "Codex" docs\r\nCodex IME notes\r\n'
        }
      }
      return null
    })
