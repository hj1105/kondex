...
describe('agent completion coordinator', () => {
  ...
  it.each([
    ...
    'pi',
    'omp'
  ])('only recognizes retained providers when receiving a %s hook', (agentType) => {
    const dispatchCompletion = vi.fn()
    const coordinator = createAgentCompletionCoordinator({
      paneKey: 'tab-1:leaf-1',
      getPtyId: () => 'pty-1',
  ...
  it.each(['codex', 'claude'])(
    'defers a %s done after working through the quiet window',
    (agentType) => {
      const dispatchCompletion = vi.fn()
      const coordinator = createAgentCompletionCoordinator({
        paneKey: 'tab-1:leaf-1',
      ...
      coordinator.observeHookStatus({ state: 'working', prompt: 'run the mission', agentType })
      coordinator.observeHookStatus({
        state: 'done',
  ...
  it('suppresses a Claude done when work resumes before the quiet window', () => {
    const dispatchCompletion = vi.fn()
    const coordinator = createAgentCompletionCoordinator({
      paneKey: 'tab-1:leaf-1',
      getPtyId: () => 'pty-1',
    ...
    coordinator.observeHookStatus({ state: 'done', prompt: 'run the mission', agentType: 'claude' })
    expect(coordinator.hasPendingHookDoneCompletion()).toBe(true)

    // Resumed work must cancel the premature completion notification.
    coordinator.observeHookStatus({
      state: 'working',
      prompt: 'run the mission',
      agentType: 'claude'
  ...
  it('still fires a pending Claude done when process inspection sees the agent exit first', async () => {
