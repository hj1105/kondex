...
describe('connectPanePty', () => {
  ...
  it('seeds a working status for Codex startup prompts after spawn', async () => {
    const { connectPanePty } = await import('./pty-connection')
    const sshPtyId = toAppSshPtyId('ssh-a', 'pty-codex')
    ...
    expect(mockStoreState.setAgentStatus).toHaveBeenCalledWith(
      makePaneKey('tab-1', LEAF_1),
      {
        ...
        agentType: 'codex',
        // Why: Kondex launched this agent, so the seed predates any provider signal (STA-4293).
  ...
  it('does not infer a working status from retired Command Code output', async () => {
    const { connectPanePty } = await import('./pty-connection')
    const capturedDataCallback: { current: ((data: string) => void) | null } = { current: null }
  ...
  it('ignores a delayed Codex spawn after the leaf rebinds to another SSH host', async () => {
    const { connectPanePty } = await import('./pty-connection')
    const oldPtyId = toAppSshPtyId('ssh-a', 'pty-codex')
    const newPtyId = toAppSshPtyId('ssh-b', 'pty-codex')
    ...
    connectPanePty(
      ...
      createDeps({
        startup: {
          command: 'codex',
          initialAgentStatus: { agent: 'codex', prompt: 'Do not seed the replacement host' }
        }
      }) as never
    )
    await flushAsyncTicks()
    ...
    expect(mockStoreState.setAgentStatus).not.toHaveBeenCalled()
    expect(mockStoreState.terminalLayoutsByTabId!['tab-1'].ptyIdsByLeafId?.[LEAF_1]).toBe(newPtyId)
  })

  it('does not invent an idle completion from retired Command Code output', async () => {
    vi.useFakeTimers()
    const { connectPanePty } = await import('./pty-connection')
    const capturedDataCallback: { current: ((data: string) => void) | null } = { current: null }
  ...
  it('does not invent working state from retired CLI repaints', async () => {
    vi.useFakeTimers()
    const { connectPanePty } = await import('./pty-connection')
    const capturedDataCallback: { current: ((data: string) => void) | null } = { current: null }
    const transport = createMockTransport('pty-command-code')
  ...
  it('preserves completed Codex state when retired TUI output replays', async () => {
    const { connectPanePty } = await import('./pty-connection')
    const capturedDataCallback: { current: ((data: string) => void) | null } = { current: null }
    const transport = createMockTransport('pty-command-code')
    transport.connect.mockImplementation(async ({ callbacks }: { callbacks: ConnectCallbacks }) => {
  ...
  it('does not start a new Codex turn from an unrelated retired TUI prompt', async () => {
    const { connectPanePty } = await import('./pty-connection')
    const capturedDataCallback: { current: ((data: string) => void) | null } = { current: null }
    const transport = createMockTransport('pty-command-code')
    transport.connect.mockImplementation(async ({ callbacks }: { callbacks: ConnectCallbacks }) => {
