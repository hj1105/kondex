...
describe('resolveProtectedMultilinePasteOptionsForPane', () => {
  ...
  it('keeps bracketed paste for Claude without Windows input-record capability', () => {
    expect(
      decide({
        hostPlatform: 'win32',
        agentStatusByPaneKey: {
          [AGENT_PANE_KEY]: agentEntry({ agentType: 'claude' })
        }
      })
    ).toEqual({ forceBracketedPasteForMultiline: true })
  })

  it('does not infer agent paste capability from retired Pi metadata', () => {
    const evidence = {
      hostPlatform: 'win32' as const,
      agentStatusByPaneKey: { [AGENT_PANE_KEY]: agentEntry({ agentType: 'pi' }) }
    }
    expect(decide(evidence)).toBeUndefined()
    expect(decide({ ...evidence, isWindowsClient: true })).toEqual({
      forceBracketedPasteForMultiline: true
    })
  })
})
