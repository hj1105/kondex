...
describe('Cmd+J palette middle-band ranking', () => {
  ...
  it('drops candidates that cover a minority of the words typed', () => {
    // A provider token alone must not satisfy a two-word query.
    const integrationSections: SettingsNavSection[] = [
      {
        id: 'gitlab',
        title: 'GitLab',
        description: 'How GitLab works in Kondex.',
        icon: Settings,
        searchEntries: [],
        group: 'capabilities'
      },
    ...
    expect(rank('gitlab triage')).toEqual([])
    expect(rank('gitlab')).toEqual(['settings:gitlab', 'settings:integrations'])
    // Why: a majority still counts, so one stray word cannot blank an otherwise good match.
    expect(rank('gitlab integrations triage')).toEqual(['settings:integrations'])
  })
