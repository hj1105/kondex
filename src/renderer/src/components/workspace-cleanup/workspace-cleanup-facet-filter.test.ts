...
describe('agent, git, review, ticket, context, and location filters', () => {
  ...
  it('filters by ticket source', () => {
    const github = makeFacets({ worktree: { linkedIssue: 9 } })
    expect(github.ticketSources).toEqual(['issue'])
    const state = filters((s) => {
      s.safety.dismissed = 'any'
      s.ticket.presence = 'some'
      s.ticket.sources = ['issue']
    })
    expect(matchesWorkspaceCleanupFilters(github, state, FACET_NOW)).toBe(true)
    expect(
      matchesWorkspaceCleanupFilters(
        makeFacets({
          worktree: {
            linkedWorkItem: {
              ...
              number: 9,
              title: 'GitLab issue',
              url: 'https://gitlab.com/acme/project/-/issues/9'
            }
          }
        }),
        state,
