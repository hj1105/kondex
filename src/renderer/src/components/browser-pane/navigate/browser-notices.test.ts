...
describe('browser notice formatting', () => {
  it('formats denied permissions with safe copy', () => {
    ...
    ).toBe('https://example.com asked for camera or microphone access, and Kondex denied it.')
    expect(
      formatPermissionNotice({
        browserPageId: 'browser-1',
    ...
    ).toBe('this page asked for your location, and Kondex denied it.')
  })
  ...
  it('names the storage permission in words rather than its raw token', () => {
    ...
    expect(notice).toBe(
      'https://example.com asked for cookie access on behalf of an embedded site, and Kondex denied it.'
    )
  })
  ...
  it.each([
    ...
    ['openExternal', 'permission to open a link outside Kondex'],
    ...
    ).toBe(`https://example.com asked for ${description}, and Kondex denied it.`)
  })
  ...
  it('falls back to the raw permission name for anything unmapped', () => {
    ...
    ).toBe('https://example.com asked for some-future-permission, and Kondex denied it.')
  })

  it('keeps opaque origin and permission text unchanged when naming the app', () => {
    expect(
      formatPermissionNotice({
        browserPageId: 'browser-1',
        permission: 'Orca extension permission',
        origin: 'https://Orca.example.com'
      })
    ).toBe('https://Orca.example.com asked for Orca extension permission, and Kondex denied it.')
  })
  ...
  it('formats popup outcomes', () => {
    ...
    ).toBe('https://example.com opened a new page in Kondex.')
    ...
    ).toBe('A site tried to open a popup Kondex does not support here.')
  })
  ...
  it('formats certificate failures without local-server recovery advice', () => {
    ...
    expect(formatLoadFailureDescription(loadError(-202), meta)).toBe(
      "Kondex doesn't trust the authority that issued the certificate for localhost:3443."
    )
    expect(formatLoadFailureDescription(loadError(-208), meta)).toBe(
      "Kondex couldn't verify the certificate for localhost:3443."
    )
