...
describe('automation-list-view', () => {
  ...
  it('filters rows by multiple supported agents', () => {
    const items = applyAutomationListView({
      automations: [
  ...
  it('sorts local automations by name', () => {
    const items = applyAutomationListView({
      automations: [
        makeAutomation({ id: 'zebra', name: 'Zebra job' }),
        makeAutomation({ id: 'alpha', name: 'Alpha digest' })
      ],
      runs: [],
  ...
  it('sorts by last run newest first and keeps never-run rows last', () => {
    const items = applyAutomationListView({
      automations: [
        ...
        makeAutomation({ id: 'recent', name: 'Recent' }),
        makeAutomation({ id: 'never', name: 'Never' })
      ],
      runs: [
        makeRun({ automationId: 'old', dispatchedAt: Date.parse('2026-08-11T09:00:00Z') }),
        makeRun({ automationId: 'recent', dispatchedAt: Date.parse('2026-08-12T09:00:00Z') })
      ],
      filter: { status: 'all', lastRun: 'all', agentIds: [] },
      sort: { field: 'lastRun', direction: 'desc' }
    })
    expect(items.map((item) => item.id)).toEqual(['recent', 'old', 'never'])
  })
})
