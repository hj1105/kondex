...
describe('TabGroupSplitLayout', () => {
  ...
  it('does not wire the retired feature tracker to resize-start', () => {
    const element = TabGroupSplitLayout({
      layout: {
        type: 'split',
        direction: 'horizontal',
    ...
    expect(resizeHandle.props.onResizeStart).toBeUndefined()
    expect(recordFeatureInteractionMock).not.toHaveBeenCalled()
  })
})
