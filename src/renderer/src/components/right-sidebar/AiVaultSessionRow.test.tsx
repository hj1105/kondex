...
const session = {
  id: 'local:codex:sess-1:/home/a/.codex/sessions/s.jsonl',
  ...
  branch: null,
  model: null,
  filePath: '/home/a/.codex/sessions/s.jsonl',
  codexHome: null,
  createdAt: null,
  updatedAt: null,
  modifiedAt: 0,
...
function renderRow(
  ...
  const rowSession = overrides.session ?? session
  return render(
    <TooltipProvider>
      <VaultSessionRow
        session={rowSession}
        liveState={null}
        resumeStartup={{ command: rowSession.resumeCommand! }}
        realHomeResumeStartup={{ command: rowSession.resumeCommand! }}
...
function expectAgentIdentity(): void {
  ...
  expect(within(metadata).getByText('Codex')).toBeTruthy()
...
describe('VaultSessionRow details toggle', () => {
  it('does not expand the row when a menu action is chosen', async () => {
    ...
    renderRow({
      ...
      session: {
        ...session,
        id: 'local:claude:sess-1:/home/a/.claude/projects/repo/sess-1.jsonl',
        agent: 'claude',
        filePath: '/home/a/.claude/projects/repo/sess-1.jsonl',
  ...
  it('keeps Codex deletion disabled without expanding the row', async () => {
    const onToggleDetails = vi.fn()
    const onRequestDelete = vi.fn()
