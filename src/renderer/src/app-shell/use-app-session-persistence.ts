...
export function useAppSessionPersistence(): void {
  ...
  // Why: beforeunload never fires on a hard kill (crash or TerminateProcess), so periodically capture agent session ids (not scrollback) so live agents keep a resume record.
  useEffect(() => {
