...
describe('resolveOpenTabOccupantAgent', () => {
  it('uses launchAgent when no hook or sleeping record exists', () => {
    expect(resolve({ launchAgent: 'codex' })).toBe('codex')
  })
    ...
    expect(
      resolve({
        ...
        agentStatusByPaneKey: {
          ...
          [makePaneKey(TAB_ID, LEAF_B)]: status(LEAF_B, 'codex')
        }
      })
    ).toBe('claude')
  })
  ...
  it('uses the tab-strip title identity when hooks have not reported yet', () => {
    expect(resolve({ title: 'codex' })).toBe('codex')
  })

  it('does not let a codex mention in the title steal a launched Claude pane', () => {
    expect(
      resolve({
        launchAgent: 'claude',
        title: 'fix codex parser'
      })
    ).toBe('claude')
  })

  it('does not treat a hyphenated codex mention as occupancy', () => {
    expect(resolve({ title: 'session-scanner-codex-parser' })).toBeNull()
  })
  ...
  it('uses the focused sleeping session, matching useTabAgent', () => {
    expect(
      resolve({
        layout: layout(LEAF_A),
        sleepingAgentSessionsByPaneKey: {
          [makePaneKey(TAB_ID, LEAF_A)]: sleeping(LEAF_A, 'codex'),
  ...
  it('does not invent a sibling sleeping occupant when the focused leaf has none', () => {
    expect(
      resolve({
        layout: layout(LEAF_A),
        sleepingAgentSessionsByPaneKey: {
          [makePaneKey(TAB_ID, LEAF_B)]: sleeping(LEAF_B, 'codex')
        }
      })
    ).toBeNull()
  })
  ...
  it('falls back to a completed focused hook after launchAgent is gone', () => {
    expect(
      resolve({
        layout: layout(LEAF_A),
        agentStatusByPaneKey: {
          [makePaneKey(TAB_ID, LEAF_A)]: status(LEAF_A, 'codex', 'done')
        }
      })
    ).toBe('codex')
  })
