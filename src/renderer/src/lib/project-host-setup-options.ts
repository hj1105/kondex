...
function buildNeedsSetupOptions({
  ...
  return hosts
    .filter((host) => !readySetupByHost.has(host.id) && !isRuntimeOwnedSshSetupHost(host.id))
    .map((host) => {
...
// COMPAT(retiredEphemeralVmRuntime): old profiles can still contain hidden
// runtime-owned SSH setup ids. Never surface those inert records as run targets.
...
function getHostSetupAvailability(host: ExecutionHostRegistryEntry): {
  ...
  if (host.health === 'blocked') {
    return {
      isAvailable: false,
      detail: 'Kondex server version is incompatible'
    }
  }
  ...
  if (host.kind === 'runtime') {
    ...
    if (
      ...
      return {
        isAvailable: false,
        detail: 'Update Kondex on this host to set up projects'
      }
    }
  }
  return {
