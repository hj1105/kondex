...
export function resolveTabAgentFromSignals(args: {
  ...
  // Durable focused-pane owner (launch intent → hook → session); focused-pane-scoped so a sibling cannot re-own the focused title.
  ...
  // Title carries identity only as a reuse override or a standalone id when no hook.
  ...
  // Normalize foreground identity against the durable pane owner.
 ...
 * 2. Process identity — recognized foreground process (local only), normalized against the owner.
 * 3. Title — only a reuse override or standalone identity.
