...
describe('resolveTabAgent', () => {
  ...
  it('resolves via the prefix scan when the layout is missing', () => {
    ...
    expect(resolveTabAgent(map, undefined, 'tab-1')).toBe('codex')
  })
  ...
  it.each(['droid', 'openclaude'])(
    'does not revive a retired provider icon for %s',
    (agentType) => {
      const map = { [`tab-1:${LEAF_A}`]: entry(`tab-1:${LEAF_A}`, agentType) }
      expect(resolveTabAgent(map, layout(LEAF_A), 'tab-1')).toBeNull()
      expect(resolveTabAgent(map, undefined, 'tab-1')).toBeNull()
    }
  )
  ...
  it('keeps the terminal glyph for an agent Kondex has no icon for', () => {
