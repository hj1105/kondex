...
describe('orderRecentWorkspaceTabs', () => {
  ...
  it('promotes a hookless pane whose live title reads as a permission prompt', () => {
    const rows = [
      row('titled', {
        terminalTab: { id: 'titled', title: 'Codex - action required' }
      })
    ]
    const paneSources = sources([], {
      ptyIdsByTabId: { titled: ['pty-1'] },
      runtimePaneTitlesByTabId: { titled: { 1: 'Codex - action required' } }
    })
  ...
  it('does not let a slept tab leak its stale title into the ranking', () => {
    const rows = [
      row('slept', {
        terminalTab: { id: 'slept', title: 'Codex - action required' }
      })
    ]
    const paneSources = sources([], {
      runtimePaneTitlesByTabId: { slept: { 1: 'Codex - action required' } }
    })
