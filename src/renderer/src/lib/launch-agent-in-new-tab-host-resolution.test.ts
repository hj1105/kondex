...
import type * as LaunchAgentStartupPromptPlanModule from './launch-agent-startup-prompt-plan'

vi.mock('./launch-agent-startup-prompt-plan', async (importOriginal) => {
  const actual = await importOriginal<typeof LaunchAgentStartupPromptPlanModule>()
  return { ...actual, planLaunchAgentStartupPrompt: vi.fn(actual.planLaunchAgentStartupPrompt) }
})
...
async function launchOnLinux(): Promise<void> {
  ...
  launchAgentInNewTab({ agent: 'claude', worktreeId: 'wt-1', launchPlatform: 'linux' })
}
...
describe('launchAgentInNewTab execution host resolution', () => {
  ...
  it('shapes the launch from the worktree host, not a rival repo row on another SSH host', async () => {
    ...
    expect(queuedCommand()).toContain('claude')
    expect(planLaunchAgentStartupPrompt).toHaveBeenCalledWith(
      expect.objectContaining({ base: expect.objectContaining({ isRemote: false }) })
    )
  })
  ...
  it('keeps a worktree on one SSH host remote while a rival row names another', async () => {
    ...
    expect(queuedCommand()).toContain('claude')
    expect(planLaunchAgentStartupPrompt).toHaveBeenCalledWith(
      expect.objectContaining({ base: expect.objectContaining({ isRemote: true }) })
    )
  })

  it('keeps a runtime host reaching a nested SSH target remote', async () => {
    store.repos = [
  ...
  it('keeps a runtime host with no nested SSH target local', async () => {
    store.repos = [
