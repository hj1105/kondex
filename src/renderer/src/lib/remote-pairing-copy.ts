  ...
  switch (kind) {
    case 'invalid-input':
      return translate(
        ...
        'Enter a Kondex access link or bare pairing code.'
      )
  ...
  switch (kind) {
    case 'host-identity-mismatch':
      return translate(
        ...
        'Kondex reached {{endpoint}}, but that host does not match this link. Generate a new link on the other host.',
        { endpoint: endpoint ?? 'Kondex' }
      )
    ...
    case 'protocol-incompatible':
      return translate(
        ...
        'Update Kondex on this device and the other host, then try again.'
      )
    ...
    case 'environment-save-failed':
      return translate(
        ...
        'The host was verified, but Kondex could not save it. Check the name and local settings storage, then try again.'
      )
    case 'host-unreachable':
      return translate(
        'auto.components.settings.RuntimeHostAccessForm.unavailableHelp',
        'Make sure Kondex is running on the other host and that the network or SSH tunnel can reach {{endpoint}}.',
        { endpoint: endpoint ?? 'Kondex' }
      )
  }
}
