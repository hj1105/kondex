...
describe('worktree-palette-search', () => {
  ...
  it('matches a pasted GitLab issue URL to the linked worktree', () => {
    const results = searchWorktrees(
      [
        makeWorktree({
          id: 'wt-gitlab',
          linkedWorkItem: {
            ...
            type: 'issue',
            number: 4052,
            title: 'Agent terminals disappearing',
            url: 'https://gitlab.com/acme/atlas/-/issues/4052'
          }
        }),
        makeWorktree({ id: 'wt-name', displayName: 'gitlab.com' })
      ],
      'https://gitlab.com/acme/atlas/-/issues/4052',
      repoMap
    )

    expect(results.map((result) => result.worktreeId)).toEqual(['wt-gitlab'])
  })
