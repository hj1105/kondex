...
describe('workspace-tab-palette-search', () => {
  ...
  it('stamps codex occupancy from the idle OSC title the sidebar already shows', () => {
    const titledOnly = buildEntries({
      tabsByWorktree: { 'wt-1': [makeTerminalTab({ title: 'codex' })] },
      unifiedTabsByWorktree: { 'wt-1': [makeUnifiedTab({ label: 'codex' })] }
    })
    expect(titledOnly[0]?.occupantAgent).toBe('codex')
    expect(searchWorkspaceTabs(titledOnly, 'codex')[0]?.occupantAgent).toBe('codex')
  })
  ...
  it('stamps occupancy from the live unified label when the terminal record title is stale', () => {
    ...
    expect(staleRecord[0]?.title).toBe('codex')
    expect(staleRecord[0]?.occupantAgent).toBe('codex')
  })

  it('does not stamp codex occupancy from a hyphenated filename-style title', () => {
    const hyphenated = buildEntries({
      tabsByWorktree: { 'wt-1': [makeTerminalTab({ title: 'session-scanner-codex-parser' })] },
      unifiedTabsByWorktree: {
        'wt-1': [makeUnifiedTab({ label: 'session-scanner-codex-parser' })]
      }
    })
