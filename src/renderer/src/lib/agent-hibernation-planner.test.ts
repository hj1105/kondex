...
describe('live resume anchors do not block hibernation (#10238 regression)', () => {
  // Live identity is not a sleep capture and must not prevent either retained agent from sleeping.
  ...
  it.each(SUPPORTED_AGENTS)(
    'plans a long-idle done %s pane holding only its live anchor',
    (agent) => {
      const providerSession = { key: 'session_id' as const, id: `${agent}-session-1` }
      const agentEntry = entry({ agentType: agent, providerSession })
      expect(
        plannedPaneKeys(
          snapshot({
