...
import type { LaunchSource } from '../../../shared/agent-launch-context'
...
export function buildAgentSessionContinuationPrompt(
  ...
  const sourceLines = [
    ...
    source.sourceLabel ? `Kondex pane: ${source.sourceLabel}` : null,
  ...
  return [
    'Continue work from the prior Kondex session using the context below.',
    ...
    ...(statusHints.length > 0 ? ['', 'Latest Kondex status hints:', ...statusHints] : []),
    '',
