...
describe('seedNativeChatLaunchDraftForAgentTab', () => {
  ...
  it('mirrors multi-line text — the majority of real drafts', () => {
    // Linked issue context is multi-line; rejecting newlines would hide the
    // draft from chat. Send now clears every parked line before submission.
    const text = 'Linked GitHub issue: #1234\nhttps://github.com/o/r/issues/1234\n'
    seedNativeChatLaunchDraftForAgentTab({ tabId: 'issue-context-tab', agent: 'codex', text })

    expect(mocks.seedNativeChatLaunchDraft).toHaveBeenCalledWith({
      tabId: 'issue-context-tab',
      agent: 'codex',
      text,
...
describe('deliverLaunchPromptToAgentTab', () => {
  ...
  it('does not seed a launch prompt for drafts, unsupported agents, or empty content', async () => {
    ...
    await deliverLaunchPromptToAgentTab({
      ...
      // @ts-expect-error Exercise stale unsupported values at the runtime boundary.
      agent: 'gemini',
      content: 'Fix failing checks',
      submit: true,
      forcePaste: true
