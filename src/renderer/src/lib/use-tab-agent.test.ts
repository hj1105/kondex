...
describe('resolveTabAgentFromSignals', () => {
  ...
  it("uses completed Codex hook identity over Claude's generic task-title heuristic", () => {
    expect(
      resolveTabAgentFromSignals({
        hasObservedAgentSignal: true,
        isRemote: false,
  ...
  // Why: #8940 — an Codex session whose task text mentions Claude flipped the tab icon
  // to Claude Code as soon as its hook row went stale (restart, mobile, between turns).
  it('keeps an Codex tab Codex when its task title merely mentions Claude', () => {
    for (const title of [
...
describe('useTabAgent', () => {
  ...
  it('clears hookless launch identity once its own title evidence ends at a shell', async () => {
    const codexTab = { ...baseTab, launchAgent: 'codex' as const, title: '✦ Codex' }
    ...
    const root = await renderHookProbe(codexTab)
    expect(clearTabLaunchAgent).not.toHaveBeenCalled()

    await rerenderHookProbe(root, { ...codexTab, title: 'zsh' })
