...
> {
  return {
    listLocalNetworkInterfaces: () => Promise.resolve({ interfaces: [] }),
    createLocalAccessLink: () => Promise.resolve({ available: false }),
    listLocalAccessGrants: () => Promise.resolve({ grants: [] }),
    revokeLocalAccessGrant: () => Promise.resolve({ revoked: false }),
    list: async () => {
    ...
    addFromPairingCode: async ({ name, pairingCode }) => {
      ...
      if (!offer) {
        throw new Error('Invalid Kondex pairing code.')
      }
      ...
      try {
        ...
        return {
          ...
          message: translate(
            ...
            'Cannot reach Kondex at {{endpoint}}.',
      ...
      try {
        ...
        return {
          ...
          message: translate(
            ...
            'Kondex verified the host but could not save it. Check browser storage and try again.'
          )
        }
      }
      manuallyDisconnectedEnvironmentIds.clear()
