...
export class WebRuntimeConnectionWaiters {
  ...
  wait(timeoutMs = 30_000): Promise<void> {
    ...
    if (this.options.isIntentionallyClosed()) {
      return Promise.reject(new Error('Remote Kondex runtime connection closed.'))
    }
        ...
        reject(
          new Error(
            withRemoteRuntimeTailscaleHint(
              'Timed out while connecting to the remote Kondex runtime.',
  ...
  rejectUnavailable(): void {
    this.rejectAll(
      new Error(
        withRemoteRuntimeTailscaleHint(
          'Could not connect to the remote Kondex runtime.',
          this.options.endpoint
        )
      )
    )
