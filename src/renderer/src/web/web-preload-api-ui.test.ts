...
describe('web UI preload API', () => {
      ...
      WebRuntimeClient: class {
        call(method: string, params?: unknown): Promise<RuntimeRpcResponse<unknown>> {
          ...
          return Promise.resolve({
            ...
            ok: true,
            result: {
              ui: { hideWorkspacesFromOtherDevices: false }
            },
            _meta: { runtimeId: 'runtime-1' }
          })
        }
  ...
  it('drops retired feature-interaction fields from the browser-local UI state', async () => {
    vi.doMock('./web-runtime-client', () => ({
      WebRuntimeClient: class {
        call(method: string): Promise<RuntimeRpcResponse<unknown>> {
          return Promise.resolve({
    ...
    globals.storage.setItem(
      ...
      JSON.stringify({
        ...
        featureInteractions: { tasks: { firstInteractedAt: 100, interactionCount: 1 } }
      })
    )
    const { installWebPreloadApi } = await import('./web-preload-api')
    installWebPreloadApi()
    ...
    await globals.window.api.ui.set({
      ...
      featureInteractions: { tasks: { firstInteractedAt: 200, interactionCount: 2 } }
    } as never)
    const ui = await globals.window.api.ui.get()
    ...
    expect('featureInteractions' in (ui as Record<string, unknown>)).toBe(false)
    expect(stored.featureInteractionTelemetryBuckets).toBeUndefined()
    expect(stored.featureInteractions).toBeUndefined()
  })
