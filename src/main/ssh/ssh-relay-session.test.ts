...
describe('SshRelaySession', () => {
  ...
  it('does not register providers if dispose wins during home resolution', async () => {
    process.env.ORCA_FEATURE_REMOTE_AGENT_HOOKS = '1'
    let resolveHome!: () => void
    muxRequestMock.mockImplementation(async (method: string) => {
      if (method === 'session.resolveHome') {
        return new Promise((resolve) => {
          resolveHome = () => resolve('/home/me')
        })
      }
      return { ok: true }
    })
    ...
    await vi.waitFor(() =>
      expect(muxRequestMock).toHaveBeenCalledWith('session.resolveHome', expect.anything())
    )
    session.dispose()
    resolveHome()
