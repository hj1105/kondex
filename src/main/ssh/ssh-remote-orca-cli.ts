...
const HOST_INTERACTIVE_COMMANDS: Record<string, string> = {
  serve:
    'orca serve starts a foreground headless Kondex server and cannot run through the SSH relay bridge. Run it directly on the machine that should host Kondex.',
  'claude-teams':
    'orca claude-teams starts an interactive Claude Code session and cannot run through the SSH relay bridge. Run it in a terminal on the Kondex host machine.',
  'agent-teams-tmux':
    'orca agent-teams-tmux is a tmux pane shim for the Kondex host machine and cannot run through the SSH relay bridge.',
  'account add':
    'orca account add runs an interactive agent login and cannot run through the buffered SSH relay bridge. Run it directly in a terminal on the Kondex host machine.'
}
...
async function dispatchRemoteCli(
  ...
  switch (command) {
    ...
    default:
      ...
      throw new Error(
        `Unsupported SSH Kondex CLI command: ${command} (full Kondex CLI bridge unavailable: ${passthroughFailureReason})`
      )
  }
}
