...
export type AuthenticatedRuntimeWs = {
  ws: WebSocket
  sharedKey: Uint8Array
}

export async function authenticateRuntimeWsSession(
  pairingUrl: string
): Promise<AuthenticatedRuntimeWs> {
  ...
  const sharedKey = deriveSharedKey(clientKeys.secretKey, serverPublicKey)
...
export async function authenticateRuntimeWs(pairingUrl: string): Promise<WebSocket> {
  return (await authenticateRuntimeWsSession(pairingUrl)).ws
}

export function sendEncryptedWsRequest(
  session: AuthenticatedRuntimeWs,
  request: Record<string, unknown>
): void {
  session.ws.send(encrypt(JSON.stringify(request), session.sharedKey))
}

export function createEncryptedWsResponseReader(session: AuthenticatedRuntimeWs): {
  next: (
    id: string,
