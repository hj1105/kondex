...
/** Visible tokens, or null on an unsupported platform or an incomplete read. */
export async function scanAgentSessionSpawnTokenProcesses(
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      // A failed read makes the snapshot incomplete, even if another token was readable.
      return null
    }
    if (token === null) {
      continue
 ...
 * Linux read-back as ownership, absence or orphan-reaping proof. "verified"
 * describes the observations only, not completeness across PID namespaces.
 */
...
/** Positive sightings only; a missing token in /proc cannot prove global absence. */
export async function findAgentSessionSpawnTokenProcesses(
  ...
  const pids = observed?.get(spawnToken)
