...
async function install(deps: StructuredAgentSessionRuntimeDeps): Promise<InstalledRuntime> {
  ...
  // Absence from this profile's store is not authority to signal a host process.
  try {
...
export function createStructuredAgentSessionOwnerProbe(
  ...
  return async (record) => {
    ...
    if (!owner) {
      ...
      // /proc can prove a sighting, not absence; missing or unreadable tokens stay fenced.
