...
describe('agent-session create operation ledger', () => {
  ...
  it('trusts the resolved local route, not a repo row on another host', async () => {
    // Display metadata must not send a resolved local launch to a rival SSH host.
    ...
    await runtime.createAgentSession(request(operationId(), { agent: 'codex', prompt: '' }))
    ...
    expect(
      (runtime as unknown as { markLocalWorkspaceTrustedForAgent: ReturnType<typeof vi.fn> })
        .markLocalWorkspaceTrustedForAgent
    ).toHaveBeenCalledWith('codex', '/repo/worktree-1')
    expect(
      (runtime as unknown as { markRemoteWorkspaceTrustedForAgent: ReturnType<typeof vi.fn> })
  ...
  it('selects nested SSH legacy fallback before reading a Claude transcript path locally', async () => {
    const runtime = createRuntime()
    const internal = runtime as unknown as {
      resolveTerminalWorkspaceLaunchScope: ReturnType<typeof vi.fn>
      markRemoteWorkspaceTrustedForAgent: ReturnType<typeof vi.fn>
    ...
    await expect(
      runtime.ensureAgentSession({
        ...
        providerSession: {
          ...
          transcriptPath: '/remote-only/claude/session.jsonl'
        }
      })
    ).rejects.toThrow('agent_session_legacy_required')
