...
describe('E2EE text reply backpressure', () => {
  ...
  it('rejects an oversized reply before sending it', () => {
    const ctx = setup()
    const baseline = ctx.ws.sent.length
    ...
    expect(ctx.onError).toHaveBeenCalledExactlyOnceWith(1013, 'Outbound reply buffer overflow')
    expect(ctx.ws.sent).toHaveLength(baseline)
  })

  it('rejects aggregate queue growth across independently backpressured sockets', () => {
    const outboundMemoryBudget = createRuntimeE2EEOutboundMemoryBudget({
    ...
    expect(outboundMemoryBudget.evidence().queuedFrames).toBe(1)
    first.channel.destroy()
    expect(outboundMemoryBudget.evidence().queuedBytes).toBe(0)
    expect(outboundMemoryBudget.evidence().queuedFrames).toBe(0)
  })
})
