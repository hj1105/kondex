...
import { RuntimeE2EESocketWiring, type RuntimeSocketTransport } from './runtime-e2ee-socket-wiring'
...
class FakeTransport implements RuntimeSocketTransport {
  private messageHandler: Parameters<RuntimeSocketTransport['onMessage']>[0] | null = null
  readonly setClientId = vi.fn()
  readonly terminateClientConnections = vi.fn(() => 0)

  onMessage(handler: Parameters<RuntimeSocketTransport['onMessage']>[0]): void {
...
describe('runtime auth critical path', () => {
  ...
  function authenticate(
    ...
    const wiring = new RuntimeE2EESocketWiring({
  ...
  it('emits e2ee_authenticated without spawning a single ACL process', () => {
    ...
    const device = registry.addDevice('Runtime client', 'runtime')
