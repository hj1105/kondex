...
describe('client UI RPC methods', () => {
  it('returns the runtime host agent settings needed by paired create flows', async () => {
    const settings = {
  ...
  it('persists retained host settings for paired clients', async () => {
    const settings = {
    ...
    await dispatcher.dispatch(makeRequest('settings.update', {}))
  ...
  it('accepts persisted literal UI arrays and nested UI state', async () => {
    const updated: PersistedUIState = {
      ...
      statusBarItems: ['codex', 'claude', 'ports'],
  ...
  it.each([
    ...
    ['retired artifacts view', { activeView: 'artifacts' }]
  ...
  it('rejects the retired telemetry increment RPC', async () => {
    const runtime = {
      getRuntimeId: () => 'test-runtime',
    ...
    expect(CLIENT_UI_METHODS.some((method) => method.name === 'ui.recordFeatureInteraction')).toBe(
      false
    )
  })
})
