...
import { configureWorkerRuntime } from './orchestration-federation-worker-runtime-test-fixture'
...
describe('orchestration federation', () => {
  ...
  it('does not report preferences as effective when the remote launch validator rejects them', async () => {
    const task = createHomeTask()
    vi.mocked(workerRuntime.validateOrchestrationAgentLauncher).mockImplementationOnce(() => {
      throw new OrchestrationError('invalid_argument', 'The remote host refused this launcher')
    })

    const response = await homeDispatcher.dispatch(
      startRequest(task.id, { agent: 'codex', model: 'account-model' })
    )
    ...
    expect(response).toMatchObject({
      ...
      result: {
        state: 'failed',
        launch: {
          requested: { agent: 'codex', model: 'account-model', effort: null },
          effective: null
        }
      }
    })
    expect(workerRuntime.validateOrchestrationAgentLauncher).toHaveBeenCalledWith('codex')
  ...
  it('rejects a retired agent before creating any remote worker', async () => {
    const task = createHomeTask()
    const response = await homeDispatcher.dispatch(
