...
describe('client UI RPC pairing-local field seams', () => {
  ...
  it.each(PAIRING_LOCAL_UI_FIELDS.map((field) => [field] as const))(
    'the ui.set response omits the pairing-local field %s',
    async (field) => {
      ...
      expect(setResponse).toMatchObject({ ok: true })
      const ui = (setResponse as { result: { ui: Record<string, unknown> } }).result.ui
      expect(Object.keys(ui)).not.toContain(field)
    }
  )

  it('does not register the retired feature-telemetry mutation', async () => {
