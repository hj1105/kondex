...
describe('skills.install RPC', () => {
  it('advertises and routes the explicit bundled install surface with cancellation', async () => {
    expect(RUNTIME_CAPABILITIES).toContain(BUNDLED_SKILL_INSTALL_CAPABILITY)
    const request = {
      operationId: 'bundled-1',
      skillNames: ['kondex-cli'],
      providers: ['codex'],
      destination: { scope: 'global' }
    }
    const signal = new AbortController().signal
    const installBundledSkills = vi.fn(async () => ({ status: 'complete' }))
    await method('skills.installBundled').handler(
      method('skills.installBundled').params!.parse(request),
      {
        runtime: { installBundledSkills },
        signal
      } as unknown as RpcContext
    )
    expect(installBundledSkills).toHaveBeenCalledExactlyOnceWith(request, signal)
    expect(() =>
      method('skills.installBundled').params!.parse({
        ...request,
        ingress: { kind: 'local-file', path: '/client' }
      })
    ).toThrow()
  })
