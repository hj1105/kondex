...
const SEND_MESSAGE_TYPE_ERROR = [
  ...
  'To answer a worker question, use the same Kondex CLI executable with orchestration reply --id <msg_id> --body <text>.'
].join(' ')
...
export const CheckParams = z
  .object({
    ...
    compatibilityCliCommand: z
      .enum(['kondex', 'kondex-dev', 'orca', 'orca-ide', 'orca-dev'])
      .optional(),
    run: OptionalString,
...
export const AskParams = z
  .object({
    ...
    compatibilityCliCommand: z
      .enum(['kondex', 'kondex-dev', 'orca', 'orca-ide', 'orca-dev'])
      .optional(),
    compatibilityWindowsCommand: z.enum(['kondex', 'orca', 'orca-ide']).optional()
  })
  .superRefine((params, ctx) => {
