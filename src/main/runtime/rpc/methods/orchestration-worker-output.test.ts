...
describe('exact orchestration worker output', () => {
  ...
  it('reads Claude through the shared Native Chat transcript decoder', async () => {
    await writeFile(
      transcriptA,
      `${JSON.stringify({
        uuid: 'claude-a',
        type: 'assistant',
        message: { role: 'assistant', content: [{ type: 'text', text: 'Claude worker only' }] }
      })}\n`
    )
    providerSession = {
      ...providerSession!,
        ...
        messages: [
          {
            id: 'claude-a',
            role: 'assistant',
            blocks: [{ type: 'text', text: 'Claude worker only' }]
          }
        ]
      }
    })
