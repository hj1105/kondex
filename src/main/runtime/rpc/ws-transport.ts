...
export class WebSocketTransport implements RpcTransport {
  ...
  // Why: with port 0 the OS assigns a random port; callers read the real bound port for metadata and access links.
  ...
  // Why: force-terminate soon after the 1013 close since a half-open remote client may never ack and would hold the descriptor past the WS cap; the 'error' listener absorbs a reset while closing.
  ...
  private handleConnection(ws: WebSocket): void {
    ...
    // Why: clean up connection-scoped state so a dropped remote client cannot leave orphaned terminal ownership.
