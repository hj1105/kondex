...
describe('runtime RPC startup failure reporting', () => {
  ...
  it('records the startup error locally', () => {
    ...
    expect(consoleError).toHaveBeenCalledExactlyOnceWith(
      '[runtime] Failed to start local RPC transport:',
      error
    )
    consoleError.mockRestore()
  })

  it('records a non-Error rejection without throwing', () => {
    const consoleError = vi.spyOn(console, 'error').mockImplementation(() => {})

    expect(() => recordRuntimeRpcStartFailure('RPC failed')).not.toThrow()
    expect(consoleError).toHaveBeenCalledExactlyOnceWith(
      '[runtime] Failed to start local RPC transport:',
      'RPC failed'
    )
  ...
  it('shows the CLI impact and local cause', async () => {
    ...
    expect(showMessageBoxMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        type: 'error',
        title: 'Kondex CLI unavailable',
        message: "Kondex couldn't start its local command transport.",
        detail: expect.stringMatching(
          /kondex status.*kondex terminal.*orchestration.*Cause: metadata write failed/s
        )
      })
    )
  })
  ...
  it.each([
    ['EACCES', "Check permissions on Kondex's data folder"],
    ['EPERM', "Check permissions on Kondex's data folder"],
    ...
    ['ENOENT', "Kondex's data folder may be missing"],
  ...
  it('falls back to a plain restart when the cause is unrecognised', async () => {
    ...
    const detail = showMessageBoxMock.mock.calls[0]?.[1]?.detail as string
    expect(detail).toContain('Restart Kondex to try again.')
    expect(detail).not.toContain("Check permissions on Kondex's data folder")
  })

  it('preserves upstream names inside the original diagnostic cause', async () => {
    const cause = 'Orca-host: /srv/Orca Project/orca-runtime.json'
    await showRuntimeRpcStartupFailureDialog(createParentWindow(), new Error(cause))

    const options = showMessageBoxMock.mock.calls[0]?.[1]
    expect(options.title).toBe('Kondex CLI unavailable')
    expect(options.detail).toContain('kondex status')
    expect(options.detail).toContain('kondex terminal')
    expect(options.detail).toContain('Restart Kondex to try again.')
    expect(options.detail.slice(options.detail.indexOf('Cause: ') + 'Cause: '.length)).toBe(cause)
  })
