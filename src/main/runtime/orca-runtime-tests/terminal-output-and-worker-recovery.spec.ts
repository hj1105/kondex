...
describe('OrcaRuntimeService', () => {
  ...
  it('preserves retired Grok title frames without provider-specific normalization', async () => {
    const runtime = new OrcaRuntimeService(store)
    syncSinglePty(runtime)
    ...
    expect(pty?.lastOscTitle).toBe('⠋ - Waiting for response… - grok')
    expect(pty?.lastAgentStatus).toBe('working')

    // Generic spinner activity does not authorize retired-provider title rewriting.
    runtime.onPtyData('pty-1', '\x1b]0;⠴ - Thinking - grok\x07', 101)
    expect(pty?.lastOscTitle).toBe('⠴ - Thinking - grok')
    expect(pty?.lastAgentStatus).toBe('working')
  })

  it('coalesces retained Codex title updates within one notification window', async () => {
    ...
    runtime.onPtyData('laptop-created-pty', '\x1b]0;⠋ Codex working\x07', 100)
    runtime.onPtyData('laptop-created-pty', '\x1b]0;⠴ Codex working\x07', 101)
    runtime.onPtyData('laptop-created-pty', '\x1b]0;⠙ Codex working\x07', 102)
  ...
  it('republishes mobile session tabs with hook payloads for title-less OSC 9999 transitions', async () => {
    ...
    runtime.onPtyData(
      'hook-only-pty',
      '\x1b]9999;{"state":"waiting","prompt":"fix the tests","agentType":"claude"}\x07',
      101
    )
  ...
  // Legacy foreground metadata must not relabel a retained provider's hook payload.
  it('keeps retained Claude hook metadata when legacy Pi foreground metadata is present', async () => {
    ...
    runtime.setPtyController({
      ...
      write: () => true,
      kill: () => true,
      // Simulate legacy foreground metadata in a restored pane.
    ...
    runtime.onPtyData(
      'omp-flicker-pty',
      '\x1b]0;⠋ Pi\x07' +
        '\x1b]9999;{"state":"working","prompt":"fix the bug","agentType":"claude"}\x07',
      100
    )
  ...
  it('preserves retired Pi title text without inventing idle status', async () => {
    const runtime = new OrcaRuntimeService(store)
    syncSinglePty(runtime)
    ...
    // Stored text alone cannot supply retired-provider readiness to later readers.
    expect(detectAgentStatusFromTitle(pty?.lastOscTitle ?? '')).toBeNull()
  })

  it('preserves hydration-seeded retired titles with the same generic status as live frames', async () => {
    const runtime = new OrcaRuntimeService(store)
    syncSinglePty(runtime)
    ...
    expect(pty?.lastOscTitle).toBe('⠴ - Thinking - grok')
    // Hydration preserves generic spinner activity without retired-provider normalization.
  ...
  it('leaves messages pending for a retired Cursor provider without terminal writes', async () => {
    vi.useFakeTimers()
    try {
      const runtime = new OrcaRuntimeService(store)
      const db = new InMemoryOrchestrationMessages()
      ...
      expect(db.getUnreadMessages(mailbox)).toHaveLength(0)
