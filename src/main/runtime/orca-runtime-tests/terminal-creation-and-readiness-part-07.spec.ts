...
describe('OrcaRuntimeService', () => {
  ...
  it('does not dismiss workspace trust based on a retired Antigravity ready screen', async () => {
    const runtime = new OrcaRuntimeService(store)
    runtime.setPtyController({
      spawn: vi.fn().mockResolvedValue({ id: 'pty-bg' }),
      write: () => true,
  ...
  it('does not infer trust dismissal from a retired Cursor idle screen', async () => {
    const runtime = new OrcaRuntimeService(store)
    runtime.setPtyController({
      spawn: vi.fn().mockResolvedValue({ id: 'pty-bg' }),
      write: () => true,
    ...
    const { handle } = await runtime.createTerminal(`path:${TEST_WORKTREE_PATH}`)
    // An unsupported provider screen cannot prove this trust prompt was answered.
    runtime.onPtyData(
      'pty-bg',
      [
        // Keep the historical banner order so it cannot revive the retired parser.
        'Cursor Agent\n',
  ...
  it('does not infer trust dismissal from a retired Cursor busy screen', async () => {
    const runtime = new OrcaRuntimeService(store)
    runtime.setPtyController({
      spawn: vi.fn().mockResolvedValue({ id: 'pty-bg' }),
      write: () => true,
    ...
    runtime.onPtyData(
      'pty-bg',
      [
        // Preserve the same historical trust/banner order as the idle case.
        'Cursor Agent\n',
        '⚠ Workspace Trust Required\n',
        'Do you trust the contents of this directory?\n',
        '  ▶ [a] Trust this workspace\n',
    ...
    // A retired busy-screen parser cannot establish that the trust modal was dismissed.
    await expect(
