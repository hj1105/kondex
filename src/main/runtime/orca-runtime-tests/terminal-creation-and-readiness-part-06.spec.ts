...
describe('OrcaRuntimeService', () => {
  ...
  it('resolves live-leaf tui-idle from a wrapped Codex ready title', async () => {
    const runtime = new OrcaRuntimeService(store)
    runtime.setPtyController({
      write: () => true,
      kill: () => true,
    ...
    syncSinglePty(runtime, 'remote:pty-1', {
      ...
      paneTitle: 'ssh build-host | Codex ready'
    })
  ...
  it('does not resolve tui-idle from a retired Antigravity ready screen', async () => {
    const runtime = new OrcaRuntimeService(store)
    runtime.setPtyController({
      spawn: vi.fn().mockResolvedValue({ id: 'pty-bg' }),
      write: () => true,
  ...
  it('resolves Codex readiness in newline-heavy pasted tails without splitting', async () => {
    const runtime = new OrcaRuntimeService(store)
    runtime.setPtyController({
      spawn: vi.fn().mockResolvedValue({ id: 'pty-bg' }),
      write: () => true,
    ...
    const splitReadyTail = splitSpy.mock.contexts.some((context) => {
      ...
      return (
        value.toLowerCase().includes('openai codex') && value.includes('pasted text pasted text')
      )
    })
    expect(splitReadyTail).toBe(false)
  })

  it('does not let retired Antigravity prompt ordering dismiss workspace trust', async () => {
    const runtime = new OrcaRuntimeService(store)
    runtime.setPtyController({
      spawn: vi.fn().mockResolvedValue({ id: 'pty-bg' }),
      write: () => true,
  ...
  it('resolves live-leaf tui-idle from a Codex ready prompt preview', async () => {
    ...
    runtime.onPtyData(
      'pty-1',
      ['OpenAI Codex', 'model: gpt-5.5', 'directory: /tmp/worktree-a'].join('\n'),
      Date.now()
    )
    const [terminal] = (await runtime.listTerminals()).terminals
