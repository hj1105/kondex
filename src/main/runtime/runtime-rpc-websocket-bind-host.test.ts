...
describe('OrcaRuntimeRpcServer WebSocket bind host (STA-2370)', () => {
  ...
  it('widens the listener to all interfaces when a network access link is created', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'orca-runtime-rpc-'))
    const server = new OrcaRuntimeRpcServer({
      runtime: new OrcaRuntimeService(),
      userDataPath,
    ...
    try {
      ...
      const offer = server.createPairingOffer({ address: '100.64.1.20' })
      ...
      // Why: the widen reuses the same port so already-advertised endpoints stay valid.
  ...
  it('binds all interfaces at startup when a previously-connected device can reconnect', async () => {
    ...
    const device = registry.getOrCreatePendingDevice('Paired runtime client', 'runtime')
  ...
  it('stays on loopback at startup for a pending device that has never connected', async () => {
    ...
    const registry = new DeviceRegistry(userDataPath)
    registry.getOrCreatePendingDevice('Pending runtime client', 'runtime')
  ...
  it('binds all interfaces at startup for a connected device paired before pairingReach existed', async () => {
    ...
    // keep the reconnect widen or an already-paired runtime client would be stranded by the upgrade.
    const legacyDevice = {
      deviceId: 'legacy-device',
      name: 'Legacy runtime client',
      token: 'legacy-token',
      scope: 'runtime',
      pairedAt: Date.now(),
      lastSeenAt: Date.now()
  ...
  it('keeps the same runtime socket wiring instance across a pairing widen', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'orca-runtime-rpc-'))
    const server = new OrcaRuntimeRpcServer({
      runtime: new OrcaRuntimeService(),
      userDataPath,
    ...
    await server.start()
    try {
      const wiringBeforeWiden = server.getRuntimeSocketWiring()
      ...
      // Why: the widen swaps transports under the same wiring so authenticated runtime state survives.
      expect(server.getRuntimeSocketWiring()).toBe(wiringBeforeWiden)
