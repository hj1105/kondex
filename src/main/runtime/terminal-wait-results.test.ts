...
describe('terminal wait results', () => {
  it('preserves exit provenance for every wait result shape', () => {
    ...
    const results = [
      ...
      buildTerminalWaitBlockedResult('terminal', 'exit', terminal, 'codex-interactive-prompt'),
      buildPtyTerminalWaitResult('pty', 'exit', terminal),
      buildPtyTerminalWaitBlockedResult('pty', 'exit', terminal, 'codex-interactive-prompt')
    ]
