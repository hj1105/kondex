...
describe('runtime metadata', () => {
  ...
  describe('clearRuntimeMetadataIfOwned', () => {
    ...
    it('retains metadata when another runtime owns the pid', () => {
      const userDataPath = mkdtempSync(join(tmpdir(), 'orca-runtime-metadata-'))
      tempDirs.push(userDataPath)
      writeRuntimeMetadata(userDataPath, {
    ...
    it('retains metadata when only the runtimeId differs', () => {
      // Why: pid reuse is possible across a rapid runtime replacement. The
      // runtimeId check is the second-level guard that catches this even when
      // the pid collides.
      const userDataPath = mkdtempSync(join(tmpdir(), 'orca-runtime-metadata-'))
      tempDirs.push(userDataPath)
      writeRuntimeMetadata(userDataPath, {
