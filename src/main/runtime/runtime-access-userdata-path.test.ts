...
import { DEVICE_REGISTRY_FILENAME, E2EE_KEYPAIR_FILENAME } from './runtime-access-credential-files'
...
describe('runtime access userData path stability', () => {
  let root: string
  ...
  it('writes DeviceRegistry + E2EE keypair under the canonical path, not the late one', async () => {
    ...
    registry.addDevice('Runtime client')
  ...
  it('migrates existing runtime access files from the late path as an all-or-nothing pair', async () => {
    appState.userData = canonicalDir
    const {
      initDataPath,
    ...
    migrateRuntimeAccessCredentialsToCanonicalUserDataPath(appState.userData)
    ...
    const { DeviceRegistry } = await import('./device-registry')
    const registry = new DeviceRegistry(getCanonicalUserDataPath())
    expect(registry.getDevice('late-client')?.token).toBe('late-token')
  ...
  it('skips legacy migration when only part of the canonical credential pair exists', async () => {
    appState.userData = canonicalDir
    const { initDataPath, migrateRuntimeAccessCredentialsToCanonicalUserDataPath } =
      await import('../persistence')
    initDataPath()
  ...
  it('no-ops when the source path equals the canonical path (no rename happened)', async () => {
    ...
    expect(() => migrateRuntimeAccessCredentialsToCanonicalUserDataPath(canonicalDir)).not.toThrow()
  ...
  it('a previously paired device is still found after a restart on the canonical path', async () => {
    ...
    appState.userData = canonicalDir
    {
      ...
      appState.userData = lateDir
      const { DeviceRegistry } = await import('./device-registry')
      new DeviceRegistry(getCanonicalUserDataPath()).addDevice('Runtime client')
    }
    ...
    expect(registry.listDevices().map((d) => d.name)).toContain('Runtime client')
  })
})
