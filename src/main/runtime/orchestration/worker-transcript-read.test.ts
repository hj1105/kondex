...
describe('worker transcript reads', () => {
  ...
  it('reuses the Native Chat Claude decoder', async () => {
    await writeFile(transcriptPath, `${claudeMessage('claude-one', 'Claude structured output')}\n`)
      ...
      messages: [
        {
          id: 'claude-one',
          role: 'assistant',
          blocks: [{ type: 'text', text: 'Claude structured output' }]
        }
      ]
    })
  })
