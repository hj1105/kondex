...
export type PreambleParams = {
  ...
  workerHandle: string
  devMode?: boolean
  // Direct-SSH execution hosts may keep their existing bare `orca` bridge.
...
export function buildDispatchPreamble(params: PreambleParams): string {
  // Why: in dev mode, agents must use kondex-dev to connect to the dev runtime's
  ...
  const cli = params.devMode ? 'kondex-dev' : (params.cliCommand ?? 'kondex')
  ...
  const header = `You are working inside Kondex, a multi-agent IDE. You are a dispatched worker.
