...
describe('resolveGroupAddress', () => {
  ...
  describe('agent name groups', () => {
    ...
    it.each([
      ['@codex', 'codex'],
      ['@claude', 'claude']
    ] as const)('routes %s to its agent id', (group, agentIdentity) => {
      const terminals = [
        makeSummary('sender'),
        makeSummary('target', { agentIdentity }),
    ...
    it('excludes the sender even when the sender is that agent', () => {
      const terminals = [
        makeSummary('sender', { agentIdentity: 'claude' }),
        makeSummary('target', { agentIdentity: 'claude' })
      ]
      expect(resolveGroupAddress('@claude', 'sender', terminals, noStatus)).toEqual(['target'])
    })

    describe('a task title can no longer redirect a message', () => {
      // The bug. Recorded titles of this exact shape exist: a Claude pane named
      // "Switch Claude and Codex off the load balancer… - claude" received both @claude and @codex.
      ...
      it('does not route @codex to a Claude pane whose task text names Codex', () => {
        const terminals = [
          makeSummary('sender'),
          makeSummary('claude_pane', {
            agentIdentity: 'claude',
            title: 'Switch Claude and Codex off the load balancer… - claude'
          })
        ]
