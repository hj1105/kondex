...
describe('buildDispatchPreamble', () => {
  ...
  it('uses Kondex CLI by default when devMode is not set', () => {
    const result = buildDispatchPreamble(baseParams())
    expect(result).toContain('kondex orchestration send')
    expect(result).toContain('kondex orchestration check')
    expect(result).toContain('kondex orchestration ask')
  })

  it('uses kondex-dev CLI when devMode is true', () => {
    const result = buildDispatchPreamble(baseParams({ devMode: true, cliCommand: 'kondex' }))
    expect(result).toContain('kondex-dev orchestration send')
    expect(result).toContain('kondex-dev orchestration check')
    expect(result).toContain('kondex-dev orchestration ask')
    const fragments = result.split('kondex-dev')
    for (const fragment of fragments) {
      expect(fragment).not.toMatch(/kondex orchestration/)
    }
  })

  it('uses Kondex CLI when devMode is false', () => {
    const result = buildDispatchPreamble(baseParams({ devMode: false }))
    expect(result).toContain('kondex orchestration send')
    expect(result).toContain('kondex orchestration check')
  })

  it('uses the exact kondex command for packaged WSL workers', () => {
    const result = buildDispatchPreamble(baseParams({ cliCommand: 'kondex' }))
  ...
  it('honors the legacy Orca command explicitly selected by an SSH host', () => {
    const result = buildDispatchPreamble(baseParams({ cliCommand: 'orca' }))
    expect(result).toContain('orca orchestration send')
    expect(result).not.toContain('kondex orchestration send')
  })
