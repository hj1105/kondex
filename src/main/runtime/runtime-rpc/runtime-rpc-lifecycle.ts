...
import { RuntimeE2EESocketWiring } from '../rpc/runtime-e2ee-socket-wiring'
...
export class RuntimeRpcLifecycle extends RuntimeRpcWebSocketDispatch {
  ...
  // Why: builds and starts a WS transport bound to `host`, wiring the session-scoped E2EE socket
  ...
  protected async startWebSocketTransport(options: {
    ...
    const runtimeSocketWiring = this.ensureRuntimeSocketWiring(deviceRegistry, e2eeKeypair)
    this.detachWebSocketWiring = runtimeSocketWiring.attachTransport(wsTransport)

    try {
      await wsTransport.start()
    } catch (error) {
      // Why: a listener that never bound must not stay attached to the session wiring; detach before
      // propagating so revocation targets only live transports.
  ...
  // Why: one socket wiring instance survives a loopback-to-network rebind so authenticated
  // connections and revocation targeting remain attached to the current server session.
  protected ensureRuntimeSocketWiring(
    deviceRegistry: DeviceRegistry,
    e2eeKeypair: E2EEKeypair
  ): RuntimeE2EESocketWiring {
    if (this.runtimeSocketWiring) {
      return this.runtimeSocketWiring
    }
    const runtimeSocketWiring = new RuntimeE2EESocketWiring({
      deviceRegistry,
      e2eeKeypair,
      ...
      onReady: () => {
        // Why: the first authenticated remote client starts path-candidate tracking.
