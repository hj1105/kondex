...
describe('OrcaRuntimeRpcServer', () => {
  it('rejects WebSocket requests whose request token differs from the authenticated channel token', async () => {
    ...
    const channelDevice = server['deviceRegistry']!.addDevice('first-client', 'runtime')
  ...
  it('rejects retired credentials and stamps runtime scope onto status.get', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'orca-runtime-rpc-'))
    const runtime = new OrcaRuntimeService()
    const server = new OrcaRuntimeRpcServer({ runtime, userDataPath, enableWebSocket: false })
    server['deviceRegistry'] = new DeviceRegistry(userDataPath)
    const retired = server['deviceRegistry']!.addDevice('retired-client', 'mobile')
    ...
    const retiredReply = await sendStatus(retired.token)
    expect(retiredReply).toMatchObject({
      id: 'req_status',
      ok: false,
