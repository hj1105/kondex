...
import { kontextScheduleRepositoryPath } from '../kontext/kontext-schedule-repository'
import { kontextSourceWorkspacePath } from '../kontext/kontext-source-workspace'
...
export class OrcaRuntimeWithPersistHeadlessTerminalTitle extends OrcaRuntimeWithMoveHeadlessMobileSessionTab {
  ...
  async resolveKontextScheduleRepository(worktreeSelector: string): Promise<string> {
    return kontextScheduleRepositoryPath(await this.resolveRuntimeGitTarget(worktreeSelector))
  }

  async resolveKontextSourceWorkspace(workspaceSelector: string): Promise<string> {
    return kontextSourceWorkspacePath(await this.resolveRuntimeFileTarget(workspaceSelector))
  }
