// Bound process-table scans while retaining prompt refresh and unavailable-scan safety.
...
describe('daemon pty foreground scan cadence', () => {
  ...
  it.each(['darwin', 'win32'] as const)(
    'keeps node-wrapped Codex reads cached between bounded %s scans',
    async (targetPlatform) => {
      resolveAgentForegroundProcessMock.mockResolvedValue('codex')
      const { handle } = await spawnShellSubprocess('node', targetPlatform)
      ...
      // Codex even when its 1s cache entry expires while the next scan is in flight.
      expect(reads).toEqual(['node', ...Array.from({ length: 12 }, () => 'codex')])
      expect(resolveAgentForegroundProcessMock).toHaveBeenCalledTimes(4)

      await expect(handle.confirmForegroundProcess!()).resolves.toBe('codex')
  ...
  it.each(['darwin', 'win32'] as const)(
    'keeps authoritative %s Claude reads on the zero-scan path',
    async (targetPlatform) => {
      const { handle } = await spawnShellSubprocess('claude', targetPlatform)
  ...
  it.each(['darwin', 'win32'] as const)(
    'keeps cached Codex ownership when a %s wrapper scan is unavailable',
    async (targetPlatform) => {
      ...
      expect(await readForegroundAt(handle, 1_000)).toBe('codex')
      expect(await readForegroundAt(handle, 2_500)).toBe('codex')
    }
  )
})
