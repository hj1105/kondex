...
describe('daemon-init: runRestartDaemon (7-step sequence)', () => {
  ...
  it('keeps respawn callbacks functional during and after consecutive manual restarts', async () => {
    const mod = await importFresh()
    await mod.initDaemonPtyProvider()
    ...
    expect(mod.getDaemonProvider()).toBe(adapterInstances[1])
    expect(adapterInstances[1].options.socketPath).toBe('/fake/restarted-socket')
    const spawner = spawnerInstances[0]
    spawner.resetHandle.mockClear()
    spawner.ensureRunning.mockClear()
    await outgoingRespawn?.('daemon_died')
    expect(spawner.resetHandle).toHaveBeenCalledOnce()
    expect(spawner.ensureRunning).toHaveBeenCalledOnce()
    ...
    expect(mod.getDaemonProvider()).toBe(adapterInstances[2])
    expect(adapterInstances[2].options.socketPath).toBe('/fake/restarted-socket-2')
    expect(spawnerInstances).toHaveLength(1)
  })
