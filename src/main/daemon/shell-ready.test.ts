...
describePosix('daemon shell-ready launch config', () => {
  ...
  it('restores Codex home without emitting retired-provider overlays or shims', async () => {
    const { getShellReadyLaunchConfig } = await importFreshShellReady()
    getShellReadyLaunchConfig('/bin/zsh')
    getShellReadyLaunchConfig('/bin/bash')

    // Zsh's single epilogue is invoked by both login and interactive startup.
    const wrappers = [
      readFileSync(join(getShellReadyWrapperRoot(), 'zsh', '.zshenv'), 'utf8'),
      readFileSync(join(getShellReadyWrapperRoot(), 'bash', 'rcfile'), 'utf8')
    ]
    for (const wrapper of wrappers) {
      ...
      for (const retired of [
        ...
        'command prime-agent --extension',
        'command omp --extension'
      ]) {
        expect(wrapper).not.toContain(retired)
      }
    }
  })
