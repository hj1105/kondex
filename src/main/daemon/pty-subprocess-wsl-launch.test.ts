...
describe('createPtySubprocess', () => {
  ...
  it('marks retained terminal and CLI identity for WSL import without the retired Hermes query', async () => {
    const proc = mockPtyProcess()
    spawnMock.mockReturnValue(proc)
    const platform = Object.getOwnPropertyDescriptor(process, 'platform')
    ...
    // test owns only terminal/CLI identity and the Powerlevel10k WSLENV contract.
    ...
    expect(
      spawnCall[2].env.WSLENV?.split(':').map((entry: string) => entry.split('/')[0])
    ).not.toContain('ORCA_HERMES_STARTUP_QUERY')
  })
