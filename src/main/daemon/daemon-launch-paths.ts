...
// Why: pass a log-file arg so field failures are diagnosable, but honor the KONDEX_DIAGNOSTICS_DISABLED privacy switch.
export function daemonLogArgs(): string[] {
  const disabled = (process.env.KONDEX_DIAGNOSTICS_DISABLED ?? '').trim().toLowerCase()
