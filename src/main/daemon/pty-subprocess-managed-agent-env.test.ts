...
describe('createPtySubprocess', () => {
  ...
  it('uses shell wrapper when the remote CLI directory must survive shell startup', async () => {
    ...
    try {
      await createPtySubprocess({
        ...
        env: {
          SHELL: '/bin/zsh',
          ORCA_REMOTE_CLI_BIN_DIR: '/tmp/kondex-remote-bin'
        }
      })
    } finally {
      if (platform) {
  ...
  it('does not enable a wrapper from retired OpenCode configuration', async () => {
    const proc = mockPtyProcess()
    spawnMock.mockReturnValue(proc)
    const platform = Object.getOwnPropertyDescriptor(process, 'platform')
    Object.defineProperty(process, 'platform', { value: 'linux' })
    ...
    expect(lastCall[2].env.ORCA_SHELL_FEATURES).toBeUndefined()
  })

  it('does not enable a wrapper from retired MiMo configuration', async () => {
    const proc = mockPtyProcess()
    spawnMock.mockReturnValue(proc)
    const platform = Object.getOwnPropertyDescriptor(process, 'platform')
    Object.defineProperty(process, 'platform', { value: 'linux' })
  ...
  it('does not enable a wrapper from a retired OMP extension', async () => {
    const proc = mockPtyProcess()
    spawnMock.mockReturnValue(proc)
    const platform = Object.getOwnPropertyDescriptor(process, 'platform')
    Object.defineProperty(process, 'platform', { value: 'linux' })
  ...
  it('does not enable a wrapper from the removed Agent Teams shim', async () => {
    const proc = mockPtyProcess()
    spawnMock.mockReturnValue(proc)
    const platform = Object.getOwnPropertyDescriptor(process, 'platform')
    Object.defineProperty(process, 'platform', { value: 'linux' })
