...
describe('daemon-init: runRestartDaemon (7-step sequence)', () => {
  ...
  it.each([
    { platform: 'darwin', adopted: true },
    { platform: 'darwin', adopted: false },
    { platform: 'linux', adopted: true },
    { platform: 'linux', adopted: false }
  ] as const)(
    'installs a permanent lease on $platform with adopted=$adopted',
    async ({ platform, adopted }) => {
      vi.spyOn(process, 'platform', 'get').mockReturnValue(platform)
      try {
        const mod = await importFresh()
        ensureRunningOverrides.push(async () => ({
          socketPath: '/fake/ready-socket',
          tokenPath: '/fake/ready-token',
          ...(adopted ? { adopted: true as const } : {})
        }))
        await mod.initDaemonPtyProvider()

        const adapter = adapterInstances[0]
        expect(adapter.options.socketPath).toBe('/fake/ready-socket')
        expect(mod.getDaemonProvider()).toBe(adapter)
        expect(setLocalPtyProviderMock).toHaveBeenCalledWith(adapter)
        expect(adapter.establishLifecycleLease).toHaveBeenCalledOnce()
        expect(adoptionLeaseReleases[0]).toHaveBeenCalledOnce()
        expect(adapter.establishLifecycleLease.mock.invocationCallOrder[0]).toBeLessThan(
          adoptionLeaseReleases[0].mock.invocationCallOrder[0]
        )
