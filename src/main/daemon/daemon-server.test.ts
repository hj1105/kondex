...
describe('DaemonServer', () => {
  ...
  describe('RPC routing', () => {
    ...
    it.each(['claude', 'codex'] as const)(
      'persists %s launch identity across reattach',
      async (agent) => {
        await startServer()
        const c = await connectClient()
        ...
        expect(first).toMatchObject({ isNew: true, launchAgent: agent })

        const second = await c.request('createOrAttach', {
          sessionId: 'agent-session',
          ...size
        })
        expect(second).toMatchObject({ isNew: false, launchAgent: agent })

        for (const unsupported of ['droid', 'not-an-agent']) {
          const sessionId = `unsupported-${unsupported}`
          const unknown = await c.request('createOrAttach', {
            sessionId,
            ...size,
            launchAgent: unsupported
          } as never)
          expect(unknown).toMatchObject({ isNew: true })
          expect(unknown).not.toHaveProperty('launchAgent')
          const reattached = await c.request('createOrAttach', { sessionId, ...size })
          expect(reattached).toMatchObject({ isNew: false })
          expect(reattached).not.toHaveProperty('launchAgent')
        }
      }
    )
