 ...
 * daemon's record as empty or torn. Renaming it aside would strand that daemon's record. An
 * in-flight publish is by definition fresh; a record left corrupt by a dead writer ages past this
 * floor and is quarantined on a later launch.
 */
const QUARANTINE_MIN_RECORD_AGE_MS = 60_000

/**
 * A pid record that parses to nothing cannot participate in reliable daemon adoption. Quarantine
 * the record (rename in place, bytes kept for diagnosis) once it is old enough — unless a process
 * still answers for a pid salvaged from the corrupt bytes, in which case the record may belong to
 * a live daemon and stays in place until that pid exits. Returns the unverifiable reason.
 */
