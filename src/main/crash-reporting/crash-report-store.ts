    ...
    return this.withWrite(async (reports) => {
      ...
      const nextReports = reports.map((report) => {
        if (report.id !== id) {
          ...
          // burst. Dismissing one should not reopen sibling prompts.
          if (anchor?.status === 'pending' && isRelatedCrashEvent(anchor, report)) {
        ...
        result = { ...report, status: 'dismissed' as const }
        return result
      })
      return { reports: nextReports, result }
    })
