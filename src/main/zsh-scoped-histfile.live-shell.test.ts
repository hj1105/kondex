...
describe.skipIf(process.platform === 'win32')('the deferred hook delivers every feature', () => {
  ...
  itWithZsh(
    'restores Codex home while leaving the user PATH free of the retired agent-teams shim',
    withHome(
      {
        ...USER_FILES,
      ...
      async (home) => {
        ...
        // Only Codex home remains managed; retired shim env must not override user PATH.
        expect(values.CODEX_HOME).toBe('/orca/codex')
        expect(values.PATH.startsWith('/user/bin:')).toBe(true)
        expect(values.PATH.split(':')).not.toContain('/orca/shim')
      }
    )
  )
})
