...
describe('Codex shell launch preflight command', () => {
  ...
  it.each([
    { platform: 'darwin' as const, bundled: 'kondex' },
    { platform: 'linux' as const, bundled: 'kondex' },
    { platform: 'win32' as const, bundled: 'kondex.exe' }
  ...
  it.each([
    { platform: 'darwin' as const, retired: 'orca' },
    { platform: 'linux' as const, retired: 'orca-ide' },
    { platform: 'win32' as const, retired: 'orca.exe' }
  ])('does not select another app launcher on $platform', ({ platform, retired }) => {
    const { userDataPath, resourcesPath } = makeCliRoot()
    writeExecutable(join(resourcesPath, 'bin', retired), '#!/bin/sh\nexit 0\n')
    expect(
      resolveCodexShellLaunchPreflightCommand({
        hooksEnabled: true,
        isPackaged: true,
  ...
  it('never returns an unqualified command name that a profile-rewritten PATH could hijack', () => {
    const { userDataPath, resourcesPath } = makeCliRoot()
    writeExecutable(join(resourcesPath, 'bin', 'kondex'), '#!/bin/sh\nexit 0\n')
