...
import { REAL_SHELL_TEST_BUDGET_MS, REAL_SHELL_WAIT_MS } from './real-shell-fixture-budgets'
...
describe('legacy terminal shim neutralization', () => {
  ...
  itOnPosix(
    'keeps a real Bash command hash working with trailing PATH separators',
    async () => {
      const userData = makeUserDataDir()
      const shimDir = join(userData, 'orca-terminal-attribution', 'posix')
      ...
      neutralizeLegacyTerminalShimDir(userData)
      const closed = waitForChildClose(child, REAL_SHELL_WAIT_MS)
