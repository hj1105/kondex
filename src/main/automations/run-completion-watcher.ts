...
export function describeStrandedAutomationRun(run: AutomationRun): string {
  if (run.status === 'dispatching') {
    return 'Kondex stopped before this run reported that its agent started.'
  }
  return 'Kondex lost the terminal for this run before it reported completion.'
}
...
function describeObservationError(error: unknown): string {
  ...
  return 'Kondex stopped watching this run before it reported completion.'
}
