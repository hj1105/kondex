...
describe('persisted state survives a save/load round trip', () => {
  it('reloads settings, secrets and both session partitions unchanged', () => {
    ...
    written.updateSettings({
      // Multiple encrypted slots exercise sentinel substitution in one save.
      httpProxyUrl: 'http://proxy.example:8080/?a=é&c=$&'
    })
    ...
    expect(onDisk.settings.httpProxyUrl).not.toBe('http://proxy.example:8080/?a=é&c=$&')
    expect(Buffer.from(onDisk.settings.httpProxyUrl, 'base64').toString('utf8')).toContain(
      'http://proxy.example:8080/?a=é&c=$&'
    )
