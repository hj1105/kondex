...
describe('retired artifact-sharing settings', () => {
  it('drops every removed artifact setting while preserving unrelated values', () => {
    const normalized = normalizeLegacyProfile({
      artifactsEnabled: true,
      artifactSharingEnabled: true,
      showArtifactsButton: true,
      skipDeleteArtifactConfirm: true,
      showSkillsButton: true
    })

    expect('artifactsEnabled' in normalized).toBe(false)
    expect('artifactSharingEnabled' in normalized).toBe(false)
    expect('showArtifactsButton' in normalized).toBe(false)
    expect('skipDeleteArtifactConfirm' in normalized).toBe(false)
    expect(normalized.showSkillsButton).toBe(true)
  })
})
