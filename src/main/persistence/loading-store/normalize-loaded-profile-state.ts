...
export function normalizeLoadedProfileState(
  ...
  const parsedWithRetiredFields = parsed as PersistedState & {
    featureInteractionTelemetryBuckets?: unknown
  }
  const { featureInteractionTelemetryBuckets: retiredInteractionBuckets, ...activeParsed } =
    parsedWithRetiredFields
  if (retiredInteractionBuckets !== undefined) {
    markNeedsSave()
  }
