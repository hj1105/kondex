...
function normalizeLinkedMetadata(meta: WorktreeMeta): boolean {
  const legacyMeta = meta as WorktreeMeta & {
    linkedLinearIssue?: unknown
    linkedLinearIssueWorkspaceId?: unknown
    linkedLinearIssueOrganizationUrlKey?: unknown
  }
  let changed = false
  for (const key of [
    'linkedLinearIssue',
    ...
    if (Object.hasOwn(legacyMeta, key)) {
      delete legacyMeta[key]
      changed = true
    }
  }
