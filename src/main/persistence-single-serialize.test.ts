...
describe('persistence single-serialize save guard', () => {
  ...
  it('still writes when state actually changes (including a secret change)', async () => {
    ...
    store.updateSettings({ httpProxyUrl: 'http://rotated:password@proxy.test:8080' })
    vi.advanceTimersByTime(2000)
    await store.waitForPendingWrite()
  ...
  it('persists a swap between a plaintext field and a secret when the plaintext equals the secret ciphertext (deterministic cipher)', async () => {
    ...
    const P = 'http://user:password@proxy.test:8080'
    // Persist the Kagi secret P, then read its on-disk ciphertext C (what a user could
    // copy out of orca-data.json).
    store.updateUI({ browserKagiSessionLink: P })
    vi.advanceTimersByTime(1000)
    await store.waitForPendingWrite()
    const C = (
    ...
    // The secret is still P (which also encrypts to C under the deterministic IV).
    store.updateSettings({ httpProxyBypassRules: C })
    store.updateUI({ browserKagiSessionLink: P })
    vi.advanceTimersByTime(2000)
    await store.waitForPendingWrite()
    ...
    store.updateSettings({ httpProxyBypassRules: P })
