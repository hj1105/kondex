...
export class CliInstaller extends CliPathRegistration {
  ...
  async getStatus(): Promise<CliInstallStatus> {
    ...
    if (!launcherPath) {
      const detail = this.hasUnverifiedAppImageRuntime
        ? 'Kondex could not verify the inherited AppImage runtime identity, so CLI registration is unavailable.'
        : this.isLinuxAppImage() && this.appImagePath
          ...
          : this.isPackaged
            ? 'The bundled CLI launcher is missing from this Kondex build.'
    ...
    const baseStatus =
      ...
      !isAppImageInstalledLauncherCurrent(extractionOptions)
        ? {
            ...
            detail: `${spec.commandPath} does not point to the current Kondex AppImage payload.`
          }
        : inspectedStatus
  ...
  private async installUnlocked(): Promise<CliInstallStatus> {
    ...
    if (initialStatus.state === 'conflict') {
      throw new Error(
        `Refusing to replace non-Kondex command at ${initialStatus.commandPath}. Remove it and register again if it is no longer needed.`
      )
    }
    ...
    if (status.state === 'conflict') {
      throw new Error(
        `Refusing to replace non-Kondex command at ${status.commandPath}. Remove it and register again if it is no longer needed.`
      )
    }
  ...
  private async removeUnlocked(): Promise<CliInstallStatus> {
    ...
    if (status.state === 'conflict') {
      throw new Error(`Refusing to remove non-Kondex command at ${status.commandPath}.`)
    }
    if (status.state === 'stale' && status.installMethod !== 'symlink') {
      throw new Error(`Refusing to remove a command not owned by Kondex at ${status.commandPath}.`)
    }
