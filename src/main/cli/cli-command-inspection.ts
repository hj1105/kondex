...
// Keep this ownership allowlist aligned with resources/linux/packaging/after-install.sh.
const PACKAGED_LINUX_LAUNCHER_DIRECTORIES = ['/opt/Kondex', '/opt/kondex']
    ...
    try {
      ...
      if (!stats.isSymbolicLink()) {
        if (stats.isFile()) {
          ...
          if (managedTarget) {
            return this.buildStatus({
              ...
              detail: `${commandPath} contains an older Kondex launcher.`
            })
          }
        }
        ...
        return this.buildStatus({
          ...
          state: 'conflict',
          currentTarget: null,
          detail: `${commandPath} exists but is not a Kondex symlink.`
        })
      }
      ...
      return this.buildStatus({
        ...
        detail: isInstalled
          ...
          : isManagedStaleTarget
            ? `${commandPath} points to an older Kondex launcher.`
            : `${commandPath} points to a non-Kondex launcher.`
      })
    } catch (error) {
      if (isMissingError(error)) {
        return this.buildStatus({
          ...
          state: 'not_installed',
          currentTarget: null,
          detail: `Register ${commandPath} to use Kondex from the terminal.`
        })
      }
      throw error
    }
    ...
    try {
      ...
      if (!stats.isFile()) {
        return this.buildStatus({
          ...
          state: 'conflict',
          currentTarget: null,
          detail: `${commandPath} exists but is not a Kondex launcher script.`
        })
      }
      ...
      if (isMissingError(error)) {
        return this.buildStatus({
          ...
          state: 'not_installed',
          currentTarget: null,
          detail: `Register ${commandPath} to use Kondex from Command Prompt or PowerShell.`
        })
      }
      throw error
    }
