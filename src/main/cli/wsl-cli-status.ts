...
export async function resolveReadyWslCliState(args: {
  ...
  if (!hostStatus.launcherPath) {
    return {
      status: unsupportedWslCliStatus(
        ...
        hostStatus.detail ?? 'The Windows Kondex CLI launcher is missing.'
      )
    }
  }
  ...
  if (!interopReady) {
    return {
      status: unsupportedWslCliStatus(
        'launcher_missing',
        'WSL Windows interop is unavailable; Kondex cannot launch the Windows CLI from WSL.'
      )
    }
  }
