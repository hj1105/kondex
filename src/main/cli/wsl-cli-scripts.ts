const MANAGED_MARKER = '# Kondex managed WSL CLI launcher'
const BRIDGE_MANAGED_MARKER = '# Kondex managed WSL CLI PowerShell bridge'

export function buildWslLauncher(
  windowsLauncherPath: string,
  bridgePath = '${XDG_DATA_HOME:-$HOME/.local/share}/kondex/kondex-wsl-bridge.ps1'
): string {
...
try {
  ...
  if ($args.Count -lt 1) {
    throw 'Invalid Kondex WSL CLI bridge invocation.'
  }
  ...
  if ($null -eq $Process) {
    throw 'Unable to start the Kondex Windows CLI launcher.'
  }
  $Process.WaitForExit()
...
export function getBridgePathFromCommandPath(commandPath: string): string {
  return `${commandPath.replace(/\/\.local\/bin\/(?:kondex|orca|orca-ide)$/, '/.local/share/kondex')}/kondex-wsl-bridge.ps1`
}
...
export function buildRegistrationLockPrelude(commandPath: string): string {
  ...
  // Why: the per-distro queue only serializes one Kondex process; flock covers
  ...
  return [
    ...
    `  exec 9>${quoteShell(`${lockDir}/.kondex-wsl-cli.lock`)}`,
    '  flock -x -w 30 9',
    'fi'
  ].join('\n')
}
