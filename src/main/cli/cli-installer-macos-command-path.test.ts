...
describe('CliInstaller', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const absentUsrLocalBin = join(fixture.root, 'usr', 'local', 'bin', 'kondex')
      ...
      expect(status.commandPath).toBe(join(homePath, '.local', 'bin', 'kondex'))
      expect(status.state).toBe('not_installed')
      expect(status.supported).toBe(true)

      const installed = await installer.install()
      expect(installed.state).toBe('installed')
      expect(installed.commandPath).toBe(join(homePath, '.local', 'bin', 'kondex'))
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const installPath = join(usrLocalBin, 'kondex')
      const installer = new CliInstaller({
        platform: 'darwin',
        isPackaged: true,
        resourcesPath,
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const defaultInstallPath = join(usrLocalBin, 'kondex')
      const userInstallPath = join(userLocalBin, 'kondex')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      await expect(installer.install()).rejects.toThrow('Refusing to replace non-Kondex command')
      await expect(lstat(defaultInstallPath)).rejects.toMatchObject({ code: 'ENOENT' })
      await expect(readFile(userInstallPath, 'utf8')).resolves.toContain('other-orca')
    }
  )
  ...
  // The fallback directory must not change the public command name.
  it.skipIf(process.platform === 'win32')(
    'reports commandName as kondex when falling back to ~/.local/bin on macOS',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
      ...
      const status = await installer.getStatus()
      expect(status.commandName).toBe('kondex')
    }
  )
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      expect(s1.commandPath).toBe(join(homePath, '.local', 'bin', 'kondex'))
    }
  )
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const bundledLauncher = join(resourcesPath, 'bin', 'kondex')
