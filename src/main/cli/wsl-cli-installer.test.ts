...
function makeHostStatus(
  launcherPath = 'C:\\Users\\me\\AppData\\Local\\Programs\\Kondex\\resources\\bin\\kondex.exe'
) {
  return {
    platform: 'win32',
    commandName: 'kondex',
    commandPath: launcherPath,
    pathDirectory: 'C:\\Users\\me\\AppData\\Local\\Programs\\Kondex\\resources\\bin',
    pathConfigured: true,
    launcherPath,
...
// Legacy-shaped Kondex fixture for batch-to-native launcher repair; not a published release.
const LEGACY_MANAGED_WSL_LAUNCHER = `#!/usr/bin/env bash
set -euo pipefail
# Kondex managed WSL CLI launcher
ORCA_WIN_LAUNCHER='C:\\Program Files\\Kondex\\resources\\bin\\kondex.cmd'
ORCA_BRIDGE_PS1='/home/alice/.local/share/kondex/kondex-wsl-bridge.ps1'
if command -v powershell.exe >/dev/null 2>&1; then
  ...
  echo "Kondex WSL CLI requires Windows interop and could not find powershell.exe." >&2
  exit 1
fi
...
function createWslRunner(
  ...
  const commandPath = '/home/alice/.local/bin/kondex'
  const legacyCommandPath = '/home/alice/.local/bin/orca'
  const bridgePath = '/home/alice/.local/share/kondex/kondex-wsl-bridge.ps1'
  ...
  const runner = vi.fn(async (_distro: string, command: string) => {
    ...
    if (command.includes('cat > "$command_tmp"')) {
      ...
      if (
        ...
        !files.get(bridgePath)?.includes('# Kondex managed WSL CLI PowerShell bridge')
      ) {
      ...
      if (files.get(legacyCommandPath)?.includes('# Kondex managed WSL CLI launcher')) {
    ...
    if (command.includes('rm -f')) {
      ...
      if (
        ...
        files.get(legacyCommandPath)?.includes('# Kondex managed WSL CLI launcher')
      ) {
        files.delete(legacyCommandPath)
      }
      return ''
...
describe('WslCliInstaller', () => {
  ...
  it('installs a WSL launcher that forwards to the Windows Kondex launcher', async () => {
    ...
    await expect(installer.getStatus()).resolves.toMatchObject({
      state: 'not_installed',
      commandPath: '/home/alice/.local/bin/kondex'
    })
    ...
    expect(wsl.getFile()).toBe(
      _internals.buildWslLauncher(
        'C:\\Users\\me\\AppData\\Local\\Programs\\Kondex\\resources\\bin\\kondex.exe',
        '/home/alice/.local/share/kondex/kondex-wsl-bridge.ps1'
      )
    )
  ...
  it('continues checking WSL when the host launcher exists but host PATH is unknown', async () => {
    ...
    const hostStatus = {
      ...makeHostStatus(),
      pathConfigured: null,
      detail: 'Kondex could not read the Windows user PATH registry value.'
    } satisfies CliInstallStatus
    const installer = new WslCliInstaller({
      platform: 'win32',
      distro: 'Ubuntu',
  ...
  it('derives the shared WSL bridge path for current and legacy command names', () => {
    expect(_internals.getBridgePathFromCommandPath('/home/alice/.local/bin/kondex')).toBe(
      '/home/alice/.local/share/kondex/kondex-wsl-bridge.ps1'
    )
  ...
  it('reports installed WSL launchers whose bin directory is missing from PATH', async () => {
    ...
    const installer = new WslCliInstaller({
      platform: 'win32',
      distro: 'Ubuntu',
      hostInstaller: { getStatus: async () => makeHostStatus('C:\\Kondex\\kondex.cmd') },
      wslRunner: wsl.runner
    })
  ...
  it('accepts current managed WSL scripts with an extra heredoc trailing newline', async () => {
    ...
    const installer = new WslCliInstaller({
      ...
      wslRunner: async (distro, command) => {
        if (command.includes('cat /home/alice/.local/share/kondex/kondex-wsl-bridge.ps1')) {
    ...
    await expect(installer.getStatus()).resolves.toMatchObject({
      state: 'installed',
      currentTarget: 'C:\\Kondex\\kondex.cmd'
    })
  })
  ...
  it('generates a launcher that forwards arguments through a PowerShell file bridge', () => {
    const launcher = _internals.buildWslLauncher(
      'C:\\Program Files\\Kondex\\resources\\bin\\kondex.exe',
      '/home/alice/.local/share/kondex/kondex-wsl-bridge.ps1'
    )
    ...
    expect(launcher).toContain(
      'Kondex WSL CLI requires Windows interop and could not find powershell.exe.'
    )
  ...
  it('marks stale managed launchers that point at the old app bin instead of packaged resources', async () => {
    const oldLauncher = _internals.buildWslLauncher(
      'C:\\Users\\me\\AppData\\Local\\Programs\\Kondex\\bin\\kondex.cmd',
      '/home/alice/.local/share/kondex/kondex-wsl-bridge.ps1'
    )
    ...
    await expect(installer.getStatus()).resolves.toMatchObject({
      state: 'stale',
      currentTarget: 'C:\\Users\\me\\AppData\\Local\\Programs\\Kondex\\bin\\kondex.cmd',
      launcherPath: 'C:\\Users\\me\\AppData\\Local\\Programs\\Kondex\\resources\\bin\\kondex.exe'
    })

    await expect(installer.install()).resolves.toMatchObject({
      state: 'installed',
      currentTarget: 'C:\\Users\\me\\AppData\\Local\\Programs\\Kondex\\resources\\bin\\kondex.exe'
    })
  })

  it('repairs the frozen pre-rc4 registration so orchestration send/reply reach native rc4', async () => {
    const nativeLauncher = 'C:\\Program Files\\Kondex\\resources\\bin\\kondex.exe'
    const wsl = createWslRunner(LEGACY_MANAGED_WSL_LAUNCHER)
    const installer = new WslCliInstaller({
      platform: 'win32',
      distro: 'Ubuntu',
    ...
    const simulateRc4Launch = (args: string[]): number => {
      ...
      return target?.toLowerCase().endsWith('kondex.cmd') &&
  ...
  it('does not adopt or remove the separate Orca app launcher', async () => {
    const upstream = '#!/usr/bin/env bash\n# Orca managed WSL CLI launcher\nexec orca.exe "$@"\n'
    const wsl = createWslRunner(null, true, { initialLegacyFile: upstream })
    const installer = new WslCliInstaller({
      platform: 'win32',
      distro: 'Ubuntu',
      hostInstaller: { getStatus: async () => makeHostStatus() },
    ...
    expect(wsl.getLegacyFile()).toBe(upstream)
    expect(wsl.getFile()).toBeNull()
    expect(wsl.calls.some((command) => command.includes('cat > "$command_tmp"'))).toBe(false)
  })

  it('repairs a managed launcher whose bridge is missing, but preserves a conflicting bridge', async () => {
    const nativeLauncher = 'C:\\Kondex\\resources\\bin\\kondex.exe'
    const missingBridge = createWslRunner(LEGACY_MANAGED_WSL_LAUNCHER, true, {
      initialBridge: null
    })
    ...
    const staleBridge = createWslRunner(LEGACY_MANAGED_WSL_LAUNCHER, true, {
      initialBridge: '# Kondex managed WSL CLI PowerShell bridge\nWrite-Output "stale"\n'
    })
    ...
    const conflictingBridge = createWslRunner(LEGACY_MANAGED_WSL_LAUNCHER, true, {
    ...
    expect(conflictingBridge.getFile()).toBe(LEGACY_MANAGED_WSL_LAUNCHER)
    expect(
  ...
  it('moves a legacy-only managed registration to kondex without touching unmanaged names', async () => {
    const nativeLauncher = 'C:\\Kondex\\resources\\bin\\kondex.exe'
    const managedLegacy = createWslRunner(null, true, {
      initialBridge: _internals.buildWslBridgeScript(),
      initialLegacyFile: LEGACY_MANAGED_WSL_LAUNCHER
    })
    const installer = new WslCliInstaller({
      platform: 'win32',
      distro: 'Ubuntu',
  ...
  it('does not adopt a legacy-managed registration when the bridge is user-owned', async () => {
    ...
    expect(wsl.getLegacyFile()).toBe(LEGACY_MANAGED_WSL_LAUNCHER)
    expect(wsl.calls.some((command) => command.includes('cat > "$command_tmp"'))).toBe(false)
  })
  ...
  it('keeps the pre-rc4 files on a transactional replacement failure', async () => {
    const bridge = _internals.buildWslBridgeScript()
    const wsl = createWslRunner(LEGACY_MANAGED_WSL_LAUNCHER, true, {
    ...
    const installer = new WslCliInstaller({
      ...
      hostInstaller: {
        getStatus: async () =>
          makeHostStatus('C:\\Program Files\\Kondex\\resources\\bin\\kondex.exe')
      },
      wslRunner: wsl.runner
    })
    ...
    expect(wsl.getFile()).toBe(LEGACY_MANAGED_WSL_LAUNCHER)
    ...
    // would leave a window where a concurrent shell finds no kondex at all.
    expect(installCommand).not.toContain('command_backup')
    expect(installCommand).not.toContain(`mv -f '/home/alice/.local/bin/kondex'`)
  })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const commandPath = join(home, '.local', 'bin', 'kondex')
      const bridgePath = join(home, '.local', 'share', 'orca', 'kondex-wsl-bridge.ps1')
      const bridge = _internals.buildWslBridgeScript()
      await mkdir(join(home, '.local', 'bin'), { recursive: true })
      await mkdir(join(home, '.local', 'share', 'orca'), { recursive: true })
      await writeFile(commandPath, LEGACY_MANAGED_WSL_LAUNCHER, 'utf8')
      ...
      try {
        await expect(installer.repairManagedRegistration()).rejects.toThrow()
        await expect(readFile(commandPath, 'utf8')).resolves.toBe(LEGACY_MANAGED_WSL_LAUNCHER)
  ...
  it('skips automatic repair when WSL interop is unavailable', async () => {
    const wsl = createWslRunner(LEGACY_MANAGED_WSL_LAUNCHER, true, { interopReady: false })
    const installer = new WslCliInstaller({
      platform: 'win32',
      distro: 'Ubuntu',
      hostInstaller: { getStatus: async () => makeHostStatus() },
  ...
  it('is idempotent after repairing an old managed registration', async () => {
    const nativeLauncher = 'D:\\Custom Kondex\\resources\\bin\\kondex.exe'
    const wsl = createWslRunner(LEGACY_MANAGED_WSL_LAUNCHER)
    const installer = new WslCliInstaller({
      platform: 'win32',
      distro: 'Ubuntu',
    ...
    expect(wsl.getFile()).toContain(
      "ORCA_WIN_LAUNCHER='D:\\Custom Kondex\\resources\\bin\\kondex.exe'"
    )
  })
