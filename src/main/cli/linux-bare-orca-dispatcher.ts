...
// our own file idempotently but never clobber a user's own ~/.local/bin/kondex.
const DISPATCHER_MARKER = '# kondex-serve-cli-dispatcher'

export type LinuxBareOrcaDispatcherOptions = {
  /** Packaged app resources root; the bundled `kondex` launcher lives under it. */
  ...
  /** Test seam — defaults to $XDG_CACHE_HOME/kondex/appimage. */
...
export type LinuxBareOrcaDispatcherResult = {
  ...
  /** The bundled `kondex` launcher the dispatcher execs. */
  target: string | null
}

// Why: headless AppImage sessions need a stable command path after the transient
// FUSE mount exits. This dispatcher publishes that stable Kondex launcher.
export async function installLinuxBareOrcaDispatcher(
  ...
  const dispatcherPath = join(options.homePath ?? homedir(), '.local', 'bin', 'kondex')
...
/** Script that execs the one Linux Kondex CLI launcher. */
...
async function publishDispatcher(dispatcherPath: string, content: string): Promise<boolean> {
  ...
  const temporaryPath = join(directoryPath, `.kondex-dispatcher-${process.pid}-${randomUUID()}`)
  ...
  try {
    ...
    const displacedPath = join(
      directoryPath,
      `.kondex-preserved-dispatcher-${process.pid}-${randomUUID()}`
    )
    try {
