...
const linuxLauncherAsset = new URL('../../../resources/linux/bin/kondex', import.meta.url)
const darwinLauncherAsset = new URL('../../../resources/darwin/bin/kondex', import.meta.url)
const unixLauncherFixtures = [
  {
    name: 'Linux',
    asset: linuxLauncherAsset,
    appDir: ['Kondex'],
    launcher: ['resources', 'bin', 'kondex'],
    executable: ['kondex'],
  ...
  },
  {
    ...
    appDir: ['Kondex.app'],
    launcher: ['Contents', 'Resources', 'bin', 'kondex'],
    executable: ['Contents', 'MacOS', 'Kondex'],
...
`,
  ...
  itRunsUnixShell(
    ...
    async () => {
      ...
      try {
        const appDir = join(root, 'Kondex')
        const resourcesDir = join(appDir, 'resources')
        const launcherDir = join(resourcesDir, 'bin')
        const cliDir = join(resourcesDir, 'app.asar.unpacked', 'out', 'cli')
        const launcherPath = join(launcherDir, 'kondex')
        const electronPath = join(appDir, 'kondex')
...
`,
        ...
        const commandPath = join(commandDir, 'kondex')
        await mkdir(commandDir, { recursive: true })
        await mkdir(join(homeDir, 'kondex'), { recursive: true })
...
`,
      ...
      await execFileAsync(launcherPath, ['serve', '--json'], {
      ...
      expect(payload.argv).toEqual([cliPath, 'serve', '--json'])
      expect(payload.runAsNode).toBe('1')
    } finally {
      await rm(root, { recursive: true, force: true })
    }
