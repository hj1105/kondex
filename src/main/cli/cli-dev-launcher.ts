...
export async function ensureDevLauncher(args: {
  ...
  if (args.commandName === DEV_COMMAND_NAME && args.platform !== 'win32') {
    // Why: dev PTYs prepend this dir to PATH, so keep a local `kondex` alias without claiming the global command.
    await writeFile(join(dirname(launcherPath), 'kondex'), content, {
      encoding: 'utf8',
      mode: 0o755
    })
  }
