...
const SHIM_DIR_NAME = 'linux-kondex-cli-shim'
...
// Kept as a bounded launcher refresh seam for AppImage sessions. Kondex uses one
// command name on every platform, so this no longer creates an Orca alias.
...
fail() {
  printf 'Kondex CLI is unavailable; reopen Kondex or register the CLI again.\\n' >&2
  exit 1
}
