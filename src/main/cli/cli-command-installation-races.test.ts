...
async function createMacCommandFixture() {
  ...
  const commandPath = join(commandDirectory, 'kondex')
  const resourcesPath = join(root, 'Current.app', 'Contents', 'Resources')
  const launcherPath = join(resourcesPath, 'bin', 'kondex')
  const staleLauncherPath = join(root, 'Old.app', 'Contents', 'Resources', 'bin', 'kondex')
...
async function recoveryPath(commandDirectory: string): Promise<string> {
  ...
  return join(commandDirectory, transactionName, 'kondex')
}
...
describe.skipIf(process.platform === 'win32')('CLI command filesystem races', () => {
  ...
  it('restores a foreign symlink raced into command removal', async () => {
    ...
    await expect(installer.remove()).rejects.toThrow('Refusing to remove non-Kondex command')
    await expect(readlink(fixture.commandPath)).resolves.toBe(foreignTarget)
  })
  ...
  it('keeps a foreign Kondex command when readlink loses the inspection race', async () => {
    ...
    const launcherPath = join(resourcesPath, 'bin', 'kondex')
    const legacyPath = join(commandDirectory, 'kondex')
    const managedLegacyTarget = '/opt/Kondex/resources/bin/kondex'
  ...
  it('keeps a foreign Kondex command whose quarantined inode is reused', async () => {
    ...
    const launcherPath = join(resourcesPath, 'bin', 'kondex')
    const legacyPath = join(commandDirectory, 'kondex')
    const managedTarget = '/opt/Kondex/resources/bin/kondex'
    const foreignTarget = join(root, 'foreign-orca')
    await mkdir(commandDirectory, { recursive: true })
    await mkdir(dirname(launcherPath), { recursive: true })
    await writeFile(launcherPath, '#!/usr/bin/env bash\n', { mode: 0o755 })
