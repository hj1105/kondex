...
async function makeFixture(): Promise<{ userDataPath: string; resourcesPath: string }> {
  ...
  writeFileSync(join(resourcesPath, 'bin', 'kondex'), '#!/usr/bin/env bash\n', 'utf8')
...
describe('ensureLinuxTerminalOrcaCliShimDir', () => {
  it('uses the mounted bundled launcher when only APPDIR is inherited', async () => {
    ...
    expect(shimDir).toBe(join(userDataPath, 'linux-kondex-cli-shim'))
    expect(readFileSync(join(shimDir!, 'kondex'), 'utf8')).toContain(
      `exec '${join(resourcesPath, 'bin', 'kondex')}' "$@"`
    )
  })
  ...
  it('writes an executable bare-orca shim that execs the bundled orca-ide launcher', async () => {
    ...
    expect(shimDir).toBe(join(userDataPath, 'linux-kondex-cli-shim'))
    const content = readFileSync(join(shimDir!, 'kondex'), 'utf8')
    // Single-quoted so a resources path with shell metacharacters can't break out.
    expect(content).toContain(`exec '${join(resourcesPath, 'bin', 'kondex')}' "$@"`)
    const mode = statSync(join(shimDir!, 'kondex')).mode & 0o777
  ...
  it('reuses the shim path and re-asserts its exec bit', async () => {
    ...
    const shimPath = join(first!, 'kondex')
    ...
    mkdirSync(join(otherUserData, 'linux-kondex-cli-shim'), { recursive: true })
    writeFileSync(join(otherUserData, 'linux-kondex-cli-shim', 'kondex'), 'stale contents', 'utf8')
    chmodSync(join(otherUserData, 'linux-kondex-cli-shim', 'kondex'), 0o644)
    ...
    const healedPath = join(healed!, 'kondex')
    expect(readFileSync(healedPath, 'utf8')).toContain('kondex')
  ...
  it.skipIf(!canFenceAppImageRuntime)(
    ...
    async () => {
      ...
      const liveLauncherPath = join(resourcesPath, 'bin', 'kondex')
      ...
      const shimPath = join(shimDir!, 'kondex')
  ...
  it.skipIf(!canFenceAppImageRuntime)(
    ...
    async () => {
      ...
      await writeFile(appImagePath, '#!/usr/bin/env bash\n', { encoding: 'utf8', mode: 0o755 })
      const cacheRootPath = join(userDataPath, 'cache')
      const firstLauncher = join(resourcesPath, 'bin', 'kondex')
      ...
      const nextLauncher = join(nextResourcesPath, 'bin', 'kondex')
  ...
  it.skipIf(!canFenceAppImageRuntime)(
    ...
    async () => {
      ...
      await mkdir(userDataPath, { recursive: true })
      await writeFile(appImagePath, '#!/usr/bin/env bash\n', { mode: 0o755 })
      const liveLauncher = join(resourcesPath, 'bin', 'kondex')
  ...
  it('returns null (and does not memoize) when the bundled launcher is missing', async () => {
    ...
    expect(recovered).toBe(join(userDataPath, 'linux-kondex-cli-shim'))
  })
})
