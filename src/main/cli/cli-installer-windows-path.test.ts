...
describe('CliInstaller', () => {
  ...
  it('creates a windows wrapper and updates the user PATH', async () => {
    ...
    const installPath = join(fixture.root, 'Programs', 'Kondex', 'bin', 'kondex.cmd')
    ...
    expect(userPath).toContain(join(fixture.root, 'Programs', 'Kondex', 'bin'))

    const wrapperContent = await readFile(installPath, 'utf8')
    expect(wrapperContent).toContain('ORCA_LAUNCHER=')
    expect(wrapperContent).toContain('kondex.cmd')
    ...
    expect(userPath).not.toContain(join(fixture.root, 'Programs', 'Kondex', 'bin'))
  })
  ...
  it('skips the Windows PATH write when removing an absent entry', async () => {
    ...
    const installer = new CliInstaller({
      ...
      execPath: 'C:\\Users\\me\\AppData\\Local\\Orca\\Orca.exe',
      appPath: fixture.appPath,
      commandPathOverride: join(fixture.root, 'Programs', 'Kondex', 'bin', 'kondex.cmd'),
      userPathReader: async () => userPathRead('C:\\Windows\\System32'),
      userPathWriter
    })
  ...
  it('matches expandable Windows PATH entries case-insensitively without rewriting them', async () => {
    const fixture = await makeFixture()
    const installPath = join(fixture.root, 'Local App Data', 'Kondex', 'bin', 'kondex.cmd')
    const userPathWriter = vi.fn()
    const installer = new CliInstaller({
      platform: 'win32',
      isPackaged: false,
      ...
      userPathReader: async () => userPathRead('%localappdata%\\Kondex\\bin\\', true),
      userPathWriter
    })
  ...
  it('resolves custom-install packaged Windows command path from resourcesPath', async () => {
    ...
    await writeFile(join(resourcesPath, 'bin', 'kondex.exe'), 'native launcher', 'utf8')
    ...
    expect(status.commandPath).toBe(join(resourcesPath, 'bin', 'kondex.exe'))
  })
  ...
  it('keeps a bundled Windows launcher installed when the user PATH read is unknown', async () => {
    ...
    const bundledLauncher = join(resourcesPath, 'bin', 'kondex.exe')
