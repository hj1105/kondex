...
describe('Store', () => {
  ...
  it('merges loaded data with defaults for missing fields', async () => {
    ...
    expect(store.getSettings()).not.toHaveProperty('visibleTaskProviders')
  ...
  it('drops retired visible task providers on load', async () => {
    writeDataFile({
      schemaVersion: 1,
      repos: [],
      worktreeMeta: {},
  ...
  it('drops retired provider visibility migration metadata', async () => {
    writeDataFile({
      schemaVersion: 1,
      repos: [],
      worktreeMeta: {},
  ...
  it('drops retired task provider defaults without restoring removed integrations', async () => {
    writeDataFile({
      schemaVersion: 1,
      repos: [],
      worktreeMeta: {},
    ...
    const store = await createStore()
    expect(store.getSettings()).not.toHaveProperty('defaultTaskSource')
    expect(store.getSettings()).not.toHaveProperty('visibleTaskProviders')
  })

  it('drops invalid retired task provider defaults on load', async () => {
    writeDataFile({
      schemaVersion: 1,
      repos: [],
      worktreeMeta: {},
