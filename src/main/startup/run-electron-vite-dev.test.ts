...
describe('run-electron-vite-dev', () => {
  ...
  it('forwards dev instance identity to electron-vite', async () => {
    ...
    expect(envSnapshot.dockTitle).toBe('Kondex: feature/billing-shell')
  ...
  it.skipIf(process.platform === 'win32')(
    'prepares userData kondex and kondex-dev wrappers for dev terminals',
    async () => {
      const tempDir = mkdtempSync(join(tmpdir(), 'orca-dev-wrapper-'))
      ...
      const trackedPids = trackPidFile(pidFile)
      const devWrapper = readFileSync(join(userDataPath, 'cli', 'bin', 'kondex-dev'), 'utf8')
      const publicAliasWrapper = readFileSync(join(userDataPath, 'cli', 'bin', 'kondex'), 'utf8')
      expect(publicAliasWrapper).toBe(devWrapper)
      expect(publicAliasWrapper).toContain('KONDEX_CLI_COMMAND=kondex-dev')
