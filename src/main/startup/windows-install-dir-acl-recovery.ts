...
const CAUSE =
  "Windows permissions on Kondex's install folder are blocking its own sandboxed processes from reading the files it shipped with."
...
export function describeInstallDirAclPoison(): InstallDirAclPoisonDiagnosis | null {
  ...
  if (poison.stage === 'repaired') {
    return { detail: `${CAUSE}\n\nKondex repaired the permissions. Reload to use them.`, commands }
  }
  const status =
    poison.stage === 'pending'
      ? 'Kondex is repairing the permissions now.'
      : 'Kondex could not repair them, which usually means the folder needs an administrator.'
  return {
    detail: `${CAUSE} ${status}\n\nRun these in an Administrator Command Prompt, then relaunch Kondex:\n\n${commands.join('\n')}`,
    commands
  }
}
