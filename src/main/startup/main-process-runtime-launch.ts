...
import {
  ...
  migrateRuntimeAccessCredentialsToCanonicalUserDataPath
} from '../persistence'
import { OrcaRuntimeRpcServer } from '../runtime/runtime-rpc'
import { registerRuntimeAccessHandlers } from '../ipc/runtime-access'
...
function installRuntimeRpc(
  ...
  migrateRuntimeAccessCredentialsToCanonicalUserDataPath(app.getPath('userData'))
  ...
  // Why: pin dev to 6769 so `pnpm dev` doesn't race packaged Orca on 6768 and fall back to a random port.
  const devWsPort = is.dev && !isE2E ? 6769 : undefined
  const runtimeRpc = new OrcaRuntimeRpcServer({
    runtime,
    // Why: runtime access credentials need the stable pre-setName() path, not a late
    // app.getPath('userData') that drops saved clients across restarts.
  ...
  registerRuntimeAccessHandlers(runtimeRpc)
  return runtimeRpc
}
...
async function launchServeMode(
  ...
  if (process.platform === 'darwin' || process.platform === 'linux') {
    try {
      ...
      console.log(
        `[serve] Kondex CLI install: ${cliStatus.state}${cliStatus.commandPath ? ` (${cliStatus.commandPath})` : ''}`
      )
    } catch (error) {
      console.warn(
        '[serve] Kondex CLI install skipped:',
        error instanceof Error ? error.message : String(error)
      )
    }
  }
  // Why: AppImage mounts are transient; publish a stable ~/.local/bin/kondex
  // dispatcher for headless sessions. Best-effort.
  if (process.platform === 'linux' && app.isPackaged && process.resourcesPath) {
    try {
      ...
      console.log(
        `[serve] Kondex CLI dispatcher ${dispatcher.state}: ${dispatcher.dispatcherPath}` +
          `${dispatcher.target ? ` -> ${dispatcher.target}` : ''}`
      )
    } catch (error) {
      console.warn(
        '[serve] Kondex CLI dispatcher install skipped:',
        error instanceof Error ? error.message : String(error)
      )
    }
  }
