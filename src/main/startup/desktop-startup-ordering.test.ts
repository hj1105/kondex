...
describe('startup ordering', () => {
  it('registers macOS notifications after the window shows without an onboarding gate', () => {
    const source = readFileSync(
      join(process.cwd(), 'src/main/startup/main-process-runtime-launch.ts'),
      'utf8'
    )
    const showHandlerStart = source.indexOf("win.once('show', () => {")
    const showHandlerEnd = source.indexOf('\n  })', showHandlerStart)

    expect(showHandlerStart).toBeGreaterThanOrEqual(0)
    expect(showHandlerEnd).toBeGreaterThan(showHandlerStart)
    const showHandler = source.slice(showHandlerStart, showHandlerEnd)
    expect(showHandler).toContain('triggerStartupNotificationRegistration(store)')
    expect(showHandler).not.toContain('getOnboarding')
  })
