...
describe('configureDevUserDataPath', () => {
  ...
  it('moves dev runs onto a Kondex-only userData path', async () => {
    const { app } = await import('electron')
    const { configureDevUserDataPath } = await import('./configure-process')
    ...
    // Why: production code uses path.join so every platform keeps native separators.
    expect(app.setPath).toHaveBeenCalledWith('userData', join('/tmp/app-data', 'kondex-dev'))
  })
