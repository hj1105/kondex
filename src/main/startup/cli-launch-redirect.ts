...
export function maybeRedirectCliLaunch(
  ...
  if (env[REDIRECT_ATTEMPT_ENV] === '1') {
    process.stderr.write('Unable to start the Kondex CLI through Electron node mode.\n')
    return { redirected: true, status: 1 }
  }
  if (!exists(cliEntryPath)) {
    process.stderr.write(`Unable to locate the Kondex CLI entrypoint at ${cliEntryPath}\n`)
    return { redirected: true, status: 1 }
  }
