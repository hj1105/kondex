...
import { disposeKontextSidecarService } from '../kontext/kontext-sidecar-runtime'
...
function installWillQuitHandler(): void {
  app.on('will-quit', (event: Event) => {
    ...
    const kontextSidecarShutdown = disposeKontextSidecarService()
    ...
    settleTeardownWithinDeadline([
      ...
      { name: 'kontext-sidecar', promise: kontextSidecarShutdown },
      ...
      .catch(() => {
        /* swallow — local diagnostics must never prevent app.quit() */
      })
      .then(() => {
