...
describe('initial proxy application ordering', () => {
  ...
  it('does not hold desktop startup behind the removed cloud-relay proxy barrier', () => {
    ...
    expect(desktop).toContain('await Promise.all([')
    expect(desktop).toContain('.then(() => runtimeRpc.start())')
    expect(desktop).not.toContain('await state.initialProxyApplicationReady')
    expect(desktop).not.toContain('DesktopRelayService')
  })
