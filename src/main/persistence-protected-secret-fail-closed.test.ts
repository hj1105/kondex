...
describe('protected persistence when safeStorage fails', () => {
  ...
  it('JSON-escapes retained legacy plaintext while storage is unavailable', async () => {
    const legacyProxy = 'legacy-"cookie\\value'
    writeFileSync(dataFile(), JSON.stringify({ settings: { httpProxyUrl: legacyProxy } }))
    cipherState.availability = 'unavailable'
    const store = await createStore()

    store.updateSettings({ httpProxyUrl: PENDING.proxy })
    await settleSave(store)

    expect(readState().settings.httpProxyUrl).toBe(legacyProxy)
  })
  ...
  it('accepts validated legacy plaintext for HTTP proxy and SSH migration', async () => {
    const legacyProxy = 'http://legacy:token@proxy.test:8080'
    const legacyOwnerLease = '9ab3f53d-0de9-4b80-af38-0cc15f62a6ba'
    writeFileSync(
      dataFile(),
      JSON.stringify({
        settings: { httpProxyUrl: legacyProxy },
        sshPtyConsumerRecoveries: [
          {
            targetId: 'ssh-1',
            clientInstanceId: 'client-1',
    ...
    const store = await createStore()
    expect(store.getSettings().httpProxyUrl).toBe(legacyProxy)
