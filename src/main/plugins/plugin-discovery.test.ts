...
describe('instructional plugin discovery identity', () => {
  it('changes dev consent when an agent profile instruction changes', async () => {
    const pluginsDir = await tempPluginsDir()
    const devRoot = await tempPluginsDir()
    await mkdir(join(devRoot, 'agents'))
    await writeFile(
      ...
      JSON.stringify({
        ...
        engines: { orca: '>=1.0.0' },
        pluginApi: 1,
        contributes: { agents: [{ path: 'agents/reviewer.json' }] },
        capabilities: []
      })
    )
    const profilePath = join(devRoot, 'agents', 'reviewer.json')
    await writeFile(
      profilePath,
      JSON.stringify({ instructions: 'Review changes against the domain contract.' })
    )
    ...
    await writeFile(
      profilePath,
      JSON.stringify({
        instructions: 'Review changes against the domain contract and latest decisions.'
      })
    )
