...
describe('PluginContentPackRegistry', () => {
  ...
  it('rolls back commands and valid packs when another language pack is invalid', async () => {
    const rootDir = await mkdtemp(join(tmpdir(), 'orca-plugin-content-pack-vm-'))
    roots.push(rootDir)
    await mkdir(join(rootDir, 'locales'))
    await Promise.all([
      writeFile(join(rootDir, 'locales', 'valid.json'), JSON.stringify({ settings: 'Ajustes' })),
      writeFile(join(rootDir, 'locales', 'invalid.json'), JSON.stringify({ settings: 42 }))
    ])
    const manifest = pluginManifestSchema.parse({
      manifestVersion: 1,
      id: 'mixed-recipes',
