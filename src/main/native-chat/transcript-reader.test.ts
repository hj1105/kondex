...
describe('readNativeChatTranscript (claude)', () => {
  it('rejects retired OpenClaude even when the file uses Claude transcript format', async () => {
    ...
    await expect(readNativeChatTranscript('openclaude', 'session', { filePath })).resolves.toEqual({
      error: 'Unsupported agent for Chat UI transcript: openclaude'
    })
  })
