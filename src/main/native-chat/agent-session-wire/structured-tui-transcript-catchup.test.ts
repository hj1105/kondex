...
const { watchEvents } = vi.hoisted(() => ({ watchEvents: new Map<string, () => void>() }))

// Native event latency is platform-dependent; this suite owns catchup/journal semantics.
vi.mock('../transcript-native-watcher', () => ({
  createTranscriptNativeWatcher: (path: string, onEvent: () => void) => ({
    bind: () => {
      watchEvents.set(path, onEvent)
      return true
    },
    invalidate: () => watchEvents.delete(path),
    needsRebind: () => false,
    dispose: () => watchEvents.delete(path)
  })
}))

let root: string
let store: AgentSessionRecordStore
const catchups = new Set<StructuredTuiTranscriptCatchup>()
...
function notifyAppend(path: string): void {
  const notify = watchEvents.get(path)
...
describe('StructuredTuiTranscriptCatchup', () => {
  it('tails only TUI-era appends from the durable account home', async () => {
    ...
    expect(watchEvents.has(fixture.rollout)).toBe(false)
