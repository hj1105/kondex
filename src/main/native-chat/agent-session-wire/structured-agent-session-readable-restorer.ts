...
export class StructuredAgentSessionReadableRestorer {
  ...
  private async restoreReadableSessions(sessionIds?: readonly string[]): Promise<void> {
    ...
    const records = this.input.store
      ...
      .filter(
        (record) =>
          (this.input.supportsRecord(record) ||
            (record.location.executionHostId === this.input.store.hostId &&
              record.location.wslDistro === null)) &&
          (!targetOrder || targetOrder.has(record.sessionId))
      )
    if (targetOrder) {
      records.sort(
    ...
    await restoreStructuredAgentSessionsOnRestart({
      ...this.input,
      records,
      canRecoverRecord: this.input.supportsRecord
    })
  }
}
