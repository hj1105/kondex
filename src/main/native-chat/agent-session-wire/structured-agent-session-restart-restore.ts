...
export async function restoreStructuredAgentSessionsOnRestart(input: {
  ...
  canRecoverRecord?: (record: AgentSessionRecord) => boolean
  ...
  await mapWithConcurrency(input.records, JOURNAL_RESTORE_CONCURRENCY, async (record) => {
    const { sessionId } = record
    const canRecover = input.canRecoverRecord?.(record) ?? true
    // Local journal reads do not confer an unsupported provider's recovery authority.
    if (canRecover) {
