...
describe('send', () => {
  it('writes the submission before dispatching and resolves it accepted', async () => {
    ...
    const captured = host.readJournalSnapshot(SESSION)
    expect(captured.snapshot.items).toEqual(page.page.items)
    expect(captured).toMatchObject({ provider: 'codex', location: { workspaceId: 'workspace-1' } })
    expect(JSON.stringify(captured)).not.toContain('accountHome')
    expect([acquire.mock.calls.length, dispatch.mock.calls.length]).toEqual([1, 1])
  })
...
describe('restart', () => {
  ...
  it('restores durable journals for read-only history without acquiring a provider', async () => {
    ...
    expect(host.readJournalSnapshot(SESSION).snapshot.items).toEqual(history.page.items)
