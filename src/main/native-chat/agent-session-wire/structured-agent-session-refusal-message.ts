...
function latchedMessage(record: AgentSessionRecord): string {
  ...
  if (record.lease.settlementRetryRequired) {
    return 'The provider exited, but Kondex has not finished settling the terminal chat state. Reopen this chat to retry the settlement.'
  }
  if (record.lease.claimStatus === 'conflicted') {
    return owner
      ? `Two runtimes claimed this session and Kondex cannot yet prove that ${ownerDescription(record)} has exited. Quit that process, or reopen this chat once it is gone, and Kondex will take the session back.`
      : 'Two runtimes claimed this session and the record names no process to check. Quit any other Kondex or agent process using this workspace, then reopen this chat.'
  }
  return owner
    ? `Kondex cannot prove that ${ownerDescription(record)} — the previous owner of this session — has exited, so it will not start a second agent on the same conversation. Quit that process and reopen this chat.`
    : 'Kondex cannot tell whether an agent started for this session before the app stopped, so it will not start a second one on the same conversation. Quit any leftover agent process for this workspace and reopen this chat.'
}
...
export function structuredAgentSessionRefusalMessage(
  ...
  if (code === 'execution_owner_reconciling') {
    return 'Kondex is still working out who owns this session on this machine. Reopen the chat in a moment.'
  }
  return null
}
