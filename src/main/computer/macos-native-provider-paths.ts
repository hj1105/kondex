...
export function resolveMacOSComputerUseAppPath(): string | null {
  ...
  const packaged = [join(process.resourcesPath ?? '', 'Kondex Computer Use.app')]
  const dev = [
    join(process.cwd(), 'native/computer-use-macos/.build/release/Kondex Computer Use.app'),
    resolve(__dirname, '../../native/computer-use-macos/.build/release/Kondex Computer Use.app')
  ]
