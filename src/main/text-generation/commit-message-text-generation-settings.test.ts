...
describe('resolveCommitMessageSettings', () => {
  ...
  it('preserves dynamic persisted models that were discovered by the CLI', () => {
    ...
    settings.commitMessageAi = {
      enabled: true,
      agentId: 'claude',
      selectedModelByAgent: { claude: 'discovered-model' },
      ...
      selectedThinkingByModel: { 'discovered-model': 'xhigh' },
      customPrompt: '',
      customAgentCommand: ''
    }
    syncSourceControlAiFromLegacy(settings)
  ...
  it('uses host-scoped discovered models for SSH worktrees', () => {
    ...
    settings.commitMessageAi = {
      ...
      selectedModelByAgentByHost: { 'ssh:conn-1': { claude: 'remote-only' } },
      discoveredModelsByAgent: { claude: [{ id: 'auto', label: 'Auto' }] },
      discoveredModelsByAgentByHost: {
        'ssh:conn-1': { claude: [{ id: 'remote-only', label: 'Remote Only' }] }
      },
      selectedThinkingByModel: {},
      customPrompt: '',
      customAgentCommand: ''
