...
describe('prepareLocalCommitMessageAgentEnv', () => {
  it.each(['opencode', 'pi', 'cursor'])(
    'does not resolve retained accounts for retired %s',
    async (agentId) => {
      const prepareForCodexLaunch = vi.fn()
      const prepareForClaudeLaunch = vi.fn()
      await expect(
        prepareLocalCommitMessageAgentEnv(agentId, {
  ...
  it('does not hydrate a retired OpenCode config dir from shell startup files', async () => {
    const home = makeHome()
  ...
  it('does not remap inherited OpenCode overlays to retired config roots', async () => {
  ...
  it('does not hydrate a retired Pi agent dir from shell startup files', async () => {
    const home = makeHome()
  ...
  it('does not remap inherited Pi overlays to retired config roots', async () => {
