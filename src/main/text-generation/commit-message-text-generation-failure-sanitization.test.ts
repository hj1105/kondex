...
describe('generateCommitMessageFromContext', () => {
  ...
  it('redacts a Windows drive path with JSON-escaped backslashes in a payload', async () => {
    ...
    expect(result).toEqual({
      success: false,
      error:
        'Claude CLI command failed with code 1: 401: {"message":"Invalid key loaded from [path]"}'
    })
  })
  ...
  it('keeps a scheme:// remedy URL intact while still redacting paths', async () => {
    ...
    expect(result).toEqual({
      success: false,
      error:
        'Claude CLI command failed with code 1: 401: Visit https://console.anthropic.com/settings/keys then check [path]'
    })
  })
  ...
  it('redacts key=/path shapes in provider bodies', async () => {
    ...
    expect(result).toEqual({
      success: false,
      error:
        'Claude CLI command failed with code 1: 401: {"message":"rejected credential_path=[path]"}'
    })
  })
  ...
  it.each([
    [
      ...
      'Claude CLI command failed with code 1: 401: {"message":"candidate list a,[path] rejected"}'
    ],
    [
      'non-drive colon-prefixed path',
      '401: {"message":"slot 1:/Users/name/alt failed"}',
      'Claude CLI command failed with code 1: 401: {"message":"slot 1:[path] failed"}'
    ]
  ...
  it('passes the live colonless 400 gateway payload through the sanitizer unmangled', async () => {
    ...
    expect(
      result.error.startsWith('Claude CLI command failed with code 1: 400 {"type":"error"')
    ).toBe(true)
