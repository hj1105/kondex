...
describe('discoverCommitMessageModelsLocal', () => {
  it('discovers dynamic models through the agent CLI', async () => {
    ...
    listeners.get('stdout:data')?.(Buffer.from(claudeModelListOutput))
  ...
  it('discovers dynamic models through the configured agent command override', async () => {
    ...
    const pending = discoverCommitMessageModelsLocal('claude', undefined, 'npx claude')
    ...
    if (process.platform === 'win32') {
      expect(spawnMock).toHaveBeenCalledWith(
        ...
        ['/d', '/c', expect.stringMatching(/npx\.cmd$/i), 'claude', ...claudeDiscoveryArgs],
        expect.objectContaining({ windowsHide: true })
      )
    } else {
      expect(spawnMock).toHaveBeenCalledWith(
        'npx',
        ['claude', ...claudeDiscoveryArgs],
        expect.objectContaining({ windowsHide: true })
      )
    }
  })
  ...
  it('discovers dynamic models through the selected WSL distro login shell', async () => {
    await withPlatform('win32', async () => {
      ...
      const pending = discoverCommitMessageModelsLocal('claude', undefined, undefined, {
        cwd: 'C:\\repo',
        wslDistro: 'Ubuntu'
      })
      ...
      expect(shellCommand).toContain("'claude'")
      expect(shellCommand).toContain('--input-format')
    })
  })
  ...
  it('settles and detaches model discovery when timeout kill is ignored', async () => {
    ...
    try {
      ...
      const assertion = expect(pending).resolves.toMatchObject({
        success: false,
        error: 'Claude model discovery timed out after 60s.'
      })
  ...
  it('settles and detaches model discovery when output exceeds the limit', async () => {
    ...
    await expect(pending).resolves.toMatchObject({
      success: false,
      error: 'Claude returned too much model data.'
    })
    expectChildTerminated(child)
    expect(child.stdout.listenerCount('data')).toBe(0)
    expect(child.stderr.listenerCount('data')).toBe(0)
...
describe('generateCommitMessageFromContext', () => {
  it('discovers dynamic models through a remote execution plan', async () => {
    const execute = vi.fn(async (plan, cwd, timeoutMs) => {
      expect(plan).toEqual({
        binary: 'npx',
        args: ['claude', ...claudeDiscoveryArgs],
        stdinPayload: expect.stringContaining('"list_models"'),
        label: 'Claude'
      })
      expect(cwd).toBe('/remote/repo')
      expect(timeoutMs).toBe(60_000)
      return {
        stdout: claudeModelListOutput,
        stderr: '',
        exitCode: 0,
        timedOut: false
      }
  ...
  it('reports remote model discovery transport timeouts without PATH guidance', async () => {
    ...
    expect(result).toEqual({
      success: false,
      error:
        'Claude model discovery took longer than 60s and may still be running on the remote host.'
    })
  })
  ...
  it('reports remote model discovery spawn failures with remote install guidance', async () => {
    const result = await discoverCommitMessageModelsRemote('claude', '/remote/repo', async () => ({
      stdout: '',
      stderr: '',
      exitCode: null,
      timedOut: false,
    ...
    expect(result).toEqual({
      success: false,
      error: 'claude not found on the remote PATH. Install Claude there.'
    })
  })
})
