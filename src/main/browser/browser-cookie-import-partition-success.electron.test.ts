...
async function runFixture(): Promise<{ fixture: FixtureResult; sourceChips: SourceChipsRow }> {
  ...
  const fixture = (await runBrowserRouteEgressElectron(root, fixturePath)) as FixtureResult
  return { fixture, sourceChips }
}

describe('STA-4300: partitioned (CHIPS) cookies on the native Chromium import success path', () => {
  it(
    'preserves the source partitionKey through importCookiesFromBrowser',
    async () => {
