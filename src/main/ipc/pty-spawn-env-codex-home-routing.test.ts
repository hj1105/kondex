...
describe('registerPtyHandlers', () => {
  ...
  describe('spawn environment', () => {
    it('does not remap explicit Pi env on a retired OMP launch', async () => {
      ...
      expect(env.ORCA_PI_CODING_AGENT_DIR).toBe('/tmp/parent-orca-pi-overlay')
      expect(env.ORCA_PI_SOURCE_AGENT_DIR).toBe('/tmp/user-pi-agent')
    })
    it('does not remap explicit OMP env on a retired Pi launch', async () => {
      const env = await spawnAndGetEnv(
        {
      ...
      expect(env.ORCA_OMP_CODING_AGENT_DIR).toBe('/tmp/parent-orca-omp-overlay')
      expect(env.ORCA_OMP_SOURCE_AGENT_DIR).toBe('/tmp/user-omp-agent')
      expect(env.ORCA_OMP_STATUS_EXTENSION).toBeUndefined()
    })
    it('preserves explicit Pi env without remapping it when hooks are disabled', async () => {
      const env = await spawnAndGetEnv(
        {
          PI_CODING_AGENT_DIR: '/tmp/parent-orca-pi-overlay',
          ORCA_PI_CODING_AGENT_DIR: '/tmp/parent-orca-pi-overlay',
    ...
    it('preserves explicit retired Prime env when hooks are disabled', async () => {
      const env = await spawnAndGetEnv(
        {
    ...
    posixOnlyIt('does not synthesize retired Pi extensions from shell-only config', async () => {
