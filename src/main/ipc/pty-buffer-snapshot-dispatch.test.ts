...
describe('registerPtyHandlers', () => {
  ...
  describe('spawn compatibility with legacy telemetry fields', () => {
    it('spawns successfully when legacy telemetry fields are supplied', async () => {
      handlers.clear()
    ...
    it('spawns a bare shell without telemetry metadata', async () => {
      handlers.clear()
      registerPtyHandlers(mainWindow as never)
      await handlers.get('pty:spawn')!(null, { cols: 80, rows: 24 })
      expect(spawnMock).toHaveBeenCalledOnce()
    })
    it('ignores unrecognized legacy telemetry fields without blocking spawn', async () => {
      handlers.clear()
      registerPtyHandlers(mainWindow as never)
      await handlers.get('pty:spawn')!(null, {
        cols: 80,
    ...
    it('propagates provider failure even when legacy telemetry fields are supplied', async () => {
      setLocalPtyProvider({
        spawn: vi.fn(async () => {
