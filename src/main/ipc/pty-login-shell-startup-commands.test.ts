...
describe('registerPtyHandlers', () => {
  ...
  it('uses the POSIX shell wrapper for the selected managed Codex home', async () => {
    ...
    try {
      handlers.clear()
      registerPtyHandlers(mainWindow as never, undefined, () => '/tmp/managed-codex-home')
      await handlers.get('pty:spawn')!(null, {
        cols: 80,
        rows: 24,
        cwd: '/tmp'
      })
      const [shell, args, options] = spawnMock.mock.calls.at(-1)!
      expect(shell).toBe('/bin/zsh')
      expect(args).toEqual(['-l'])
      expect(options.env.CODEX_HOME).toBe('/tmp/managed-codex-home')
      expect(options.env.ORCA_CODEX_HOME).toBe('/tmp/managed-codex-home')
  ...
  it('preserves a user Pi config without synthesizing a retired shell wrapper', async () => {
    const originalPlatform = process.platform
    const originalShell = process.env.SHELL
    ...
    try {
      ...
      expect(options.env.ORCA_PI_SOURCE_AGENT_DIR).toBeUndefined()
