...
describe('preflight', () => {
  ...
  it('reports only Claude when the retired Orca team shim is also present', async () => {
    execFileAsyncMock.mockImplementation(async (command, args) => {
      if (command !== 'which') {
        throw new Error(`unexpected command ${String(command)}`)
      }
    ...
    execFileAsyncMock.mockImplementation(async (command, args) => {
      ...
      if (String(args[0]) === 'codex') {
        return {
          environmentResolved: true,
          code: 0,
          stdout: '/Users/test/.local/bin/codex\n',
          stderr: '',
          timedOut: false
        }
      }
  ...
  it('ignores retired Mistral Vibe even when its executable exists', async () => {
    execFileAsyncMock.mockImplementation(async (command, args) => {
      if (command !== 'which') {
        throw new Error(`unexpected command ${String(command)}`)
      }
  ...
  it('ignores both current and legacy Mistral Vibe executables', async () => {
    execFileAsyncMock.mockImplementation(async (command, args) => {
      if (command !== 'which') {
        throw new Error(`unexpected command ${String(command)}`)
      }
