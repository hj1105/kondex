...
describe('registerPtyHandlers', () => {
  ...
  describe('spawn environment', () => {
    describe('daemon-active provider (parity with LocalPtyProvider)', () => {
      ...
      it('keeps the host CLI path ahead of the retired Agent Teams shim for runtime-created daemon PTYs', async () => {
        type RuntimeSpawnController = {
          spawn(args: {
            cols: number
            rows: number
        ...
        // Dev mode must prepend this app's CLI without re-promoting a retired shim.
        ...
        expect(spawnedPath[0]).toBe(join('/tmp/orca-user-data', 'cli', 'bin'))
        expect(spawnedPath.slice(1)).toEqual(['/tmp/orca-agent-teams-bin', '/usr/bin'])
      ...
      it('keeps the host CLI path ahead of the retired Agent Teams shim on daemon pty:spawn', async () => {
        // Dev mode must prepend this app's CLI without re-promoting a retired shim.
        const { app } = await import('electron')
        const mockedApp = app as unknown as { isPackaged: boolean }
        const prevPackaged = mockedApp.isPackaged
