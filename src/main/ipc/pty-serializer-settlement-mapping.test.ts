...
describe('registerPtyHandlers', () => {
  ...
  it('does not clear runtime-owned SSH reattach state on identity mismatch', async () => {
    ...
    try {
      ...
      expect(forgetCodexPaneAccountMock).not.toHaveBeenCalledWith(appPtyId)
      expect(clearMigrationUnsupportedPtyMock).not.toHaveBeenCalledWith(appPtyId)
  ...
  it('cleans up fresh runtime-owned SSH spawns when binding persistence fails', async () => {
    ...
    try {
      ...
      expect(forgetCodexPaneAccountMock).toHaveBeenCalledWith(appPtyId)
      expect(clearMigrationUnsupportedPtyMock).toHaveBeenCalledWith(appPtyId)
  ...
  it('forwards the trusted Orca terminal handle into managed WSL terminals', async () => {
    ...
    expect(env.ORCA_CLI_COMMAND).toBe('kondex')
    expect(env.KONDEX_CLI_COMMAND).toBe('kondex')
    expect(env.WSLENV?.split(':')).toEqual(
      expect.arrayContaining([
        'ORCA_TERMINAL_HANDLE/u',
        'ORCA_USER_DATA_PATH/p',
        'KONDEX_CLI_COMMAND/u',
        'ORCA_CLI_COMMAND/u',
