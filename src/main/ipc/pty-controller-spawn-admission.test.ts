...
describe('registerPtyHandlers', () => {
  ...
  it('clears a fresh runtime session when host-env setup fails', async () => {
    ...
    setLocalPtyProvider(provider as never)
    buildAgentHookEnvMock.mockImplementationOnce(() => {
    ...
    expect(forgetCodexPaneAccountMock).toHaveBeenCalledOnce()
    expect(clearMigrationUnsupportedPtyMock).toHaveBeenCalledOnce()
  })
  ...
  it('preserves operation-owned runtime state when retry host-env setup fails', async () => {
    ...
    forgetCodexPaneAccountMock.mockClear()
    clearMigrationUnsupportedPtyMock.mockClear()
    buildAgentHookEnvMock.mockImplementationOnce(() => {
      throw new Error('host env failed')
    })
