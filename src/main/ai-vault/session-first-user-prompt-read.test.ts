...
describe('readAiVaultFirstUserPrompt', () => {
  ...
  it('resolves null instead of rejecting when the transcript is corrupt', async () => {
    ...
    // A truncated Claude transcript must not reject the caller.
    const summaryPath = join(sessionDir, 'session.jsonl')
    await writeFile(summaryPath, '{"info": {"id": "session-1", "cwd": "/repo/a')

    await expect(
      readAiVaultFirstUserPrompt({ agent: 'claude', filePath: summaryPath })
