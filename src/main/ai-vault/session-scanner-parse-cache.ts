 ...
 * unchanged (mtime+size) and, for append-only JSONL transcripts, resuming the
...
export async function parseAgentSessionFileCached(
  ...
  if (unchanged) {
    ...
    // subagent dir (Claude `<session>/subagents/`) can gain files after the parent's last write (a still-running
