...
const GENERIC_DETAIL =
  'This is often a graphics-driver or installation problem. Reload to try again, or quit and relaunch Kondex.'
...
export async function presentRendererRecoveryPrompt(
  ...
  while (!deps.isQuitting()) {
    ...
    const { response } = await deps.showMessageBox({
      ...
      title: 'Kondex keeps failing to load',
      message: 'The app window crashed repeatedly and stopped reloading automatically.',
      detail: `Kondex tried to recover ${deps.recentRecoveryCount} times in a row without success.\n\n${
