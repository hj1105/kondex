...
describe('Store', () => {
  ...
  it('preserves legacy detailed card choices with the retained inline-agent migration', async () => {
    writeDataFile({
      schemaVersion: 1,
      repos: [],
      worktreeMeta: {},
    ...
    expect(store.getUI()).not.toHaveProperty('_expandedWorktreeCardPropertiesDefaulted')
  })

  it('does not duplicate existing inline agents or force additional card properties', async () => {
    writeDataFile({
      schemaVersion: 1,
      repos: [],
      worktreeMeta: {},
  ...
  it('applies inline-agent migration without replaying retired card expansion', async () => {
    writeDataFile({
      schemaVersion: 1,
      repos: [],
      worktreeMeta: {},
  ...
  it('preserves explicit Compact card properties after expansion has run', async () => {
    writeDataFile({
      ...
      settings: { compactWorktreeCards: true },
      ui: {
        worktreeCardProperties: ['status', 'unread', 'issue', 'pr', 'comment', 'ports'],
        _inlineAgentsDefaultedForAllUsers: true,
        _expandedWorktreeCardPropertiesDefaulted: true
      },
      githubCache: { pr: {}, issue: {} },
