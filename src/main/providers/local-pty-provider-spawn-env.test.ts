...
describe('LocalPtyProvider', () => {
  ...
  describe('spawn', () => {
    ...
    it('honors TERM and deletion overrides while preserving host PATH preparation', async () => {
      provider.configure({
      ...
      expect(spawnCall[2].env.PATH).toBe('/tmp/orca-stale:/tmp/user-tools:/usr/bin')
    ...
    it('uses shell wrapper when Codex home must survive shell startup', async () => {
      provider.configure({
        buildSpawnEnv: (_id, env) => {
          env.CODEX_HOME = '/tmp/kondex-codex-overlay'
          env.ORCA_CODEX_HOME = '/tmp/kondex-codex-overlay'
          return env
        }
      })
    ...
    it('does not re-promote the retired agent-teams shim on Windows `Path`', async () => {
      Object.defineProperty(process, 'platform', { configurable: true, value: 'win32' })
      provider.configure({
        buildSpawnEnv: (_id, env) => {
      ...
      expect(spawnEnv.Path).toBe('/tmp/orca-stale:/tmp/orca-agent-teams-bin:/usr/bin')
    })
