...
describePosix('local PTY shell-ready launch config', () => {
  ...
  it('restores Codex home without retired provider wrappers after user startup files', async () => {
    const { getShellReadyLaunchConfig } = await importFreshLocalPtyShellReady()
    ...
    expect(zshrc).not.toContain(restoreLine)
    expect(zlogin).not.toContain(restoreLine)
    expect(bashRc).not.toContain(restoreLine)
    expect(zshrc).not.toContain(mimoRestoreLine)
    expect(zlogin).not.toContain(mimoRestoreLine)
    expect(bashRc).not.toContain(mimoRestoreLine)
    ...
    expect(zshrc).not.toContain(agentTeamsPathRestoreLine)
    expect(zlogin).not.toContain(agentTeamsPathRestoreLine)
    expect(bashRc).not.toContain(agentTeamsPathRestoreLine)
    ...
    expect(zshrc).not.toContain(ompWrapperLine)
    expect(zlogin).not.toContain(ompWrapperLine)
    expect(bashRc).not.toContain(ompWrapperLine)
