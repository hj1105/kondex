...
describe('resolveWindowsShellLaunchArgs', () => {
  ...
  it('returns PowerShell args that install OSC 133 bootstrap after normal profile loading', () => {
    ...
    for (const retiredIndex of [
      ...
      expect(retiredIndex).toBe(-1)
    }
    for (const restoreIndex of [codexRestoreIndex]) {
  ...
  it('materializes shell-ready wrappers before building WSL shell args', () => {
    ...
    // Retired commands must reach the user's binary without rewriting argv.
    ...
    for (const wrapperFile of [bashRcfile, zshEnv]) {
      expect(wrapperFile).toContain('ORCA_CODEX_HOME')
      expect(wrapperFile).toContain('agent hooks prepare-codex')
      expect(wrapperFile).not.toContain(
        'command omp --extension "${ORCA_OMP_STATUS_EXTENSION}" "$@"'
      )
      expect(wrapperFile).not.toContain('function omp { __orca_omp "$@"; }')
