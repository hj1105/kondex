...
export class SshGitReviewHeadProvider extends SshGitRemoteSyncProvider {
  async fetchGitLabMergeRequestHead(
    ...
    try {
      ...
      if (isJsonRpcMethodNotFoundError(error)) {
        throw new Error(
          'This SSH host is running an older Kondex relay that cannot fetch merge request heads. Reconnect to deploy the latest relay, then try again.'
        )
      }
      throw error
    }
  ...
  async fetchGitHubPullRequestHead(
    ...
    try {
      ...
      if (isJsonRpcMethodNotFoundError(error)) {
        throw new Error(
          'This SSH host is running an older Kondex relay that cannot fetch pull request heads. Reconnect to deploy the latest relay, then try again.'
        )
      }
      throw error
    }
