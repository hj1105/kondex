...
describe('resolveAgentForegroundProcess', () => {
  ...
  it('preserves an unrecognized omp fallback without agent admission', async () => {
    mockPs(['101 100 S+   omp', '102 101 S+   pi'].join('\n'))

    await expect(resolveAgentForegroundProcess(100, 'omp')).resolves.toBe('omp')
  })

  it('does not enrich an unrecognized omp fallback from its child', async () => {
    mockPs(['101 100 S    omp', '102 101 S+   pi'].join('\n'))

    await expect(resolveAgentForegroundProcess(100, 'omp')).resolves.toBe('omp')
  })

  it('preserves an unrecognized pi fallback', async () => {
    mockPs(['101 100 S+   pi'].join('\n'))

    await expect(resolveAgentForegroundProcess(100, 'pi')).resolves.toBe('pi')
  })

  it('does not restore retired wrapper promotion on Windows', async () => {
    ...
    await expect(resolveAgentForegroundProcess(100, 'pi.exe')).resolves.toBe('pi.exe')
  })

  it('does not promote retired Windows wrappers through context matching', async () => {
    Object.defineProperty(process, 'platform', { value: 'win32' })
    mockWindowsRows([
      {
        pid: 100,
  ...
  it('does not admit the retired Windows Cursor launcher process tree', async () => {
    Object.defineProperty(process, 'platform', { value: 'win32' })
    mockWindowsRows([
      {
        pid: 100,
  ...
  it('does not restore a recognized fallback that disappeared before confirmation', async () => {
    ...
    await expect(
      resolveAgentForegroundProcessWithAvailability(100, 'codex', {
        fresh: true,
  ...
  it('fails closed when console attachment cannot be read', async () => {
    ...
    mockWindowsRows([
      ...
      { pid: 201, ppid: 200, name: 'codex.exe', commandLine: 'codex' },
      { pid: 202, ppid: 200, name: 'claude.exe', commandLine: 'claude' }
    ])
  ...
  it('excludes a detached Windows Codex descendant from byte authority', async () => {
    Object.defineProperty(process, 'platform', { value: 'win32' })
    mockWindowsRows([
      {
        pid: 100,
