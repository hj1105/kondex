...
describe('Store', () => {
  ...
  it('normalizes disabled TUI agents on load and update', async () => {
    ...
    const updated = store.updateSettings({
      disabledTuiAgents: ['codex', 'not-real', 'codex', 'opencode'] as never
    })
    expect(updated.disabledTuiAgents).toEqual(['codex'])
  })
  ...
  it('drops retired provider launch settings while preserving supported permission choices', async () => {
    writeFileSync(
      join(testState.dir, 'orca-data.json'),
      JSON.stringify({
        settings: {
    ...
    expect(store.getSettings().agentDefaultArgs).not.toHaveProperty('opencode')
    expect(store.getSettings().agentDefaultArgs).not.toHaveProperty('kilo')
    expect(store.getSettings().agentDefaultArgs?.codex).toBe(
      '--dangerously-bypass-approvals-and-sandbox'
    )
    expect((readDataFile() as PersistedState).settings.agentDefaultArgs).not.toHaveProperty(
      'opencode'
    )
    expect((readDataFile() as PersistedState).settings.agentDefaultArgs).not.toHaveProperty('kilo')
  })
