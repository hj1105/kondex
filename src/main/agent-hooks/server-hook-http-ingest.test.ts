...
describe('AgentHookServer listener replay', () => {
  ...
  it('caches and notifies status/main/plugin before Codex polling and HTTP response', async () => {
    const server = new AgentHookServer()
    await server.start({ env: 'production' })
    const order: string[] = []
  ...
  it('drops hook posts with an empty paneKey without retaining status', async () => {
    const server = new AgentHookServer()
    await server.start({ env: 'production' })
    try {
      const env = server.buildPtyEnv()
  ...
  it('rejects retired Hermes plugin hook posts on /hook/hermes', async () => {
    const server = new AgentHookServer()
    await server.start({ env: 'production' })
    try {
      const env = server.buildPtyEnv()
  ...
  it('rejects retired Amp plugin hook posts on /hook/amp', async () => {
    const server = new AgentHookServer()
    await server.start({ env: 'production' })
    try {
      const env = server.buildPtyEnv()
