...
describe('Last-status persistence', () => {
  ...
  it('persists and hydrates Codex session identity without exposing a turn-status change', async () => {
      ...
      server.ingestRemote(
        {
          ...
          tabId: 'tab-1',
          worktreeId: 'wt-1',
          promptInteractionKey: 'codex-local-message-id',
          payload: { state: 'working', prompt: 'persist status only', agentType: 'codex' }
        },
        'conn-1'
      )
      expect(server.getStatusSnapshot()[0]?.promptInteractionKey).toBe('codex-local-message-id')
      server.flushStatusPersistSync()
      const file = JSON.parse(readFileSync(lastStatusPath(), 'utf8'))
