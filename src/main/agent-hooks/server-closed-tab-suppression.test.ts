...
describe('AgentHookServer listener replay', () => {
  ...
  // A detach/reattach cycle retires the pane until the terminal explicitly restores authority.
  it('re-attaching a retired Claude pane restores status without needing a new turn', async () => {
    const server = new AgentHookServer()
    await server.start({ env: 'production' })
    try {
      const env = server.buildPtyEnv()
      const postHook = (payload: Record<string, unknown>): Promise<Response> =>
        fetch(`http://127.0.0.1:${env.ORCA_AGENT_HOOK_PORT}/hook/claude`, {
          ...
          body: JSON.stringify(buildBody(payload, { launchToken: 'retired-claude-token' }))
        })

      await postHook({ hook_event_name: 'UserPromptSubmit', prompt: 'turn in flight' })
      server.retirePaneAuthority(PANE)
  ...
  it('re-attaching a retired pane while idle re-opens it for a much later first turn', async () => {
    ...
    try {
      ...
      await postHook({ hook_event_name: 'UserPromptSubmit', prompt: 'much later turn' })
      expect(server.getStatusSnapshot()).toEqual([
  ...
  it('re-attach does not lift a closed-tab tombstone', async () => {
    ...
    try {
      ...
      await postHook({ hook_event_name: 'UserPromptSubmit', prompt: 'after tab close' })
      expect(server.getStatusSnapshot()).toEqual([])
  ...
  it('re-attaching restores a legacy numeric pane key alias', async () => {
    ...
    try {
      ...
      await postHook({ hook_event_name: 'UserPromptSubmit', prompt: 'legacy turn' })
      expect(server.getStatusSnapshot()).toEqual([
  ...
  // tree, `claude` + `UserPromptSubmit` already revived this pane. The control below keeps
  // the assertion tied to a completion event rather than a new-turn boundary.
  it('leaves a closed tab pane fenced once its tab id is evicted', async () => {
    ...
    try {
      ...
      await postHook(
        { hook_event_name: 'Stop', prompt: 'after eviction' },
      ...
      await postHook(
        { hook_event_name: 'Stop', prompt: 'unfenced control' },
  ...
  it('keeps a legacy alias fenced when its own tab closed but the owner tab did not', async () => {
    ...
    try {
      ...
      await postHook({ hook_event_name: 'UserPromptSubmit', prompt: 'after tab-1 close' })
      expect(server.getStatusSnapshot()).toEqual([])
    } finally {
      server.stop()
    }
  ...
  it('does not clobber a newer alias when replaying a retired fence', async () => {
    ...
    try {
      ...
      await postHook({ hook_event_name: 'UserPromptSubmit', prompt: 'after rebind' })
      expect(server.getStatusSnapshot()).toEqual([
