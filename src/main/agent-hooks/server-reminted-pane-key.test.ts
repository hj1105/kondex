...
describe('reminted $$ pane keys on the Claude hook pipeline', () => {
  ...
  it('routes a reminted $$ key to its canonical pane and settles at done', async () => {
    ...
    try {
      ...
      const start = await postHookEvent(
        server,
        buildBody(
          { hook_event_name: 'UserPromptSubmit', prompt: 'finish the reminted pane' },
      ...
      const end = await postHookEvent(
        server,
        buildBody({ hook_event_name: 'Stop' }, { paneKey: REMINTED, launchToken: 'launch-remint' }),
        '/hook/claude'
      )
  ...
  it('leaves a canonical UUID pane key on the same path', async () => {
    ...
    try {
      await postHookEvent(
        server,
        buildBody({ hook_event_name: 'UserPromptSubmit', prompt: 'canonical claude' }),
        '/hook/claude'
      )
      await postHookEvent(server, buildBody({ hook_event_name: 'Stop' }), '/hook/claude')
      expect(server.getStatusSnapshot()).toEqual([
        expect.objectContaining({
          paneKey: PANE,
          state: 'done',
          prompt: 'canonical claude',
          agentType: 'claude'
        })
      ])
    } finally {
  ...
  it('does not let a foreign reminted key stamp another pane', async () => {
    ...
    try {
      ...
      await postHookEvent(
        server,
        buildBody(
          { hook_event_name: 'UserPromptSubmit', prompt: 'owned turn' },
          { paneKey: REMINTED }
        ),
        '/hook/claude'
      )
      const foreign = await postHookEvent(
        server,
        buildBody(
          { hook_event_name: 'Stop', prompt: 'stolen done' },
  ...
  it('does not rebind a reminted token onto a different pane', async () => {
    ...
    try {
      ...
      const start = await postHookEvent(
        server,
        buildBody(
          { hook_event_name: 'UserPromptSubmit', prompt: 'owned remint turn' },
  ...
  it('does not let a later persist row rebind a reminted token', async () => {
    ...
    try {
      ...
      await postHookEvent(
        server,
        buildBody(
          { hook_event_name: 'UserPromptSubmit', prompt: 'restored remint' },
          { paneKey: REMINTED, tabId: 'tab-good' }
        ),
        '/hook/claude'
      )
