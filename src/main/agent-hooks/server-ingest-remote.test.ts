...
describe('AgentHookServer ingestRemote', () => {
  it('caches and replays Codex session identity without exposing a turn-status change', () => {
    vi.useFakeTimers()
    ...
    try {
      ...
      expect(server.getStatusSnapshot()).toEqual([
        expect.objectContaining({
          paneKey: PANE,
          providerSessionOnly: true,
          providerSession: expect.objectContaining({ transcriptPath: '/tmp/codex-session-1.jsonl' })
        })
      ])
  ...
  it('fans a Codex session-only status out to plugins, not just the renderer', () => {
    const server = new AgentHookServer()
    const rendererListener = vi.fn()
  ...
  it.each([
    { agentType: 'codex', providerSession: undefined },
    { agentType: 'claude', providerSession: { key: 'conversation_id', id: 'session-1' } },
    { agentType: 'codex', providerSession: { key: 'session_id', id: '' } },
    { agentType: 'pi', providerSession: { key: 'session_id', id: 'retired-session' } }
  ] as const)(
    'rejects invalid remote metadata-only session envelopes: %j',
      ...
      server.ingestRemote(
        {
          ...
          providerSessionOnly: true,
          providerSession,
          payload: { state: 'done', prompt: '', agentType }
        },
        'conn-1'
      )
  ...
  it.each(['codex', 'claude'] as const)(
    'accepts %s session identity without a transcript path',
    (agentType) => {
      const server = new AgentHookServer()
      const listener = vi.fn()
      server.setListener(listener)
