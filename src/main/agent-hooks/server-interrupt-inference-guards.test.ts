...
describe('AgentHookServer listener replay', () => {
  ...
  it.each(['codex', 'claude'] as const)(
    'accepts single plain Escape inference for %s',
    (agentType) => {
      vi.useFakeTimers()
      vi.setSystemTime(1_000)
      try {
  ...
  it('uses generic Ctrl+C inference for an unrecognized remote agent', () => {
    vi.useFakeTimers()
    vi.setSystemTime(1_000)
    try {
      const server = new AgentHookServer()
      server.ingestRemote(
        {
          ...
          tabId: 'tab-1',
          worktreeId: 'wt-1',
          payload: { state: 'working', prompt: 'long task', agentType: 'future-agent' }
        },
        'conn-1'
      )
      const baseline = server.getStatusSnapshot()[0]
      ...
      const applied = server.inferInterrupt({
        ...
        baselineStateStartedAt: baseline.stateStartedAt,
        baselinePrompt: 'long task',
        baselineAgentType: 'future-agent',
        intent: 'ctrl-c'
      })
