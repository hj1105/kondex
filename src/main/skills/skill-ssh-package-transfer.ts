...
type SkillSshTransferInput = {
  ...
  /** Internal archive ownership only; never populate from a renderer/RPC path. */
...
async function transferSkillPackageToSshHostUnobserved(
  ...
  const trusted = input.trustedArchivePath
  if (
    trusted !== undefined &&
    (!isAbsolute(trusted) ||
      input.request.ingress.kind !== 'local-file' ||
      input.request.ingress.path !== trusted)
  ) {
    throw new Error('skill-install-ssh-trusted-ingress-invalid')
  }
  if (!trusted && input.request.ingress.kind !== 'download-grant') {
    throw new Error('skill-install-ssh-ingress-invalid')
  }
  const downloaded = trusted
    ? { archivePath: trusted, cleanup: async () => undefined }
    : input.request.ingress.kind === 'download-grant'
      ? await downloadSkillPackageGrant({
