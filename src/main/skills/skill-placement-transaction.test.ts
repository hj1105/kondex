...
describe('skill placement transaction recovery', () => {
  it('rejects a recovery journal carrying a retired provider root', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kondex-placement-retired-root-'))
    roots.push(root)
    ...
    await writeFile(
      skillPlacementJournalPath(stateDirectory, canonical.path),
      JSON.stringify({ ...journal, providerRootOverrides: { grok: join(root, 'retired') } })
    )
    await expect(readSkillPlacementRecoveryJournal(stateDirectory, canonical.path)).rejects.toThrow(
      'skill-placement-journal-invalid'
    )
  })
  ...
  it('does not treat a skipped historical placement as ownership', async () => {
    ...
    const transaction = createSkillPlacementTransaction({
      ...
      providerRootOverrides: { codex: sharedRoot }
    })
