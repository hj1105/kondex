...
import { describeObservedSkillFile } from './skill-package-identity'
...
import {
  ...
  fixture,
  snapshot,
  writeLegacySkillLockHash
} from './skill-freshness-inventory-fixture'
...
describe('read-only skill freshness inventory', () => {
  it('offers an exact older official name only when all global placements are safe', async () => {
    ...
    expect(inventory.eligibleUpdateNames).toEqual(['kondex-cli'])
  })

  it('does not offer an unowned bundle even when an external CLI lock exists', async () => {
    const test = await fixture()
    await test.writeSkill(join(test.homeDir, '.agents', 'skills'), test.oldMarkdown)
    await rm(join(test.stateDirectory, 'skill-installs', 'receipts'), { recursive: true })
  ...
  it('does not classify unknown bytes as a newer release from an external lock', async () => {
    const test = await fixture()
    ...
    await writeLegacySkillLockHash(test.homeDir, await gitTreeShaOf(canonical))
    ...
    expect(inventory.eligibleUpdateNames).toEqual([])
    expect(getSkillFreshnessDisplayStatus(inventory, 'kondex-cli')).toBe('needs-attention')
  })
  ...
  it('reads up to date after the OS drops a sidecar into an untouched install', async () => {
    ...
    await writeLegacySkillLockHash(test.homeDir, 'a'.repeat(40))
    ...
    expect(getSkillFreshnessDisplayStatus(inventory, 'kondex-cli')).toBe('up-to-date')
    expect(inventory.eligibleUpdateNames).toEqual([])
  })

  it('does not use an external lock to trust unknown bytes with legacy leftovers', async () => {
    const test = await fixture()
    ...
    registry.skills['kondex-cli'][0].files.push(legacy)
    ...
    await writeLegacySkillLockHash(test.homeDir, await gitTreeShaOf(sourceTree))
  ...
  it('does not use an external lock to trust unknown bytes beside an agent sidecar', async () => {
    const test = await fixture()
    const upstreamMarkdown = `${test.newerMarkdown}\nUpstream edit no bundle has shipped.\n`
    const canonical = await test.writeSkill(
      join(test.homeDir, '.agents', 'skills'),
  ...
  it('does not trust an external lock even when it hashes all newly added files', async () => {
    const test = await fixture()
    const upstreamMarkdown = `${test.newerMarkdown}\nUpstream edit no bundle has shipped.\n`
    const canonical = await test.writeSkill(
      join(test.homeDir, '.agents', 'skills'),
  ...
  it('withholds automatic updates after a sidecar changes the receipt-owned package', async () => {
    // Adding a file changes the receipt-owned package, even if a legacy lock still matches.
    const test = await fixture()
    const canonical = await test.writeSkill(
      join(test.homeDir, '.agents', 'skills'),
      test.oldMarkdown
    ...
    await writeLegacySkillLockHash(test.homeDir, (2).toString(16).padStart(40, '0'))
  ...
  it('still flags canonical bytes that do not match what the lock says was installed', async () => {
    ...
    await writeLegacySkillLockHash(test.homeDir, await gitTreeShaOf(elsewhere))
  ...
  it('does not let the lock vouch for a same-name copy outside the placements it wrote', async () => {
    ...
    await writeLegacySkillLockHash(test.homeDir, await gitTreeShaOf(independent))
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    async () => {
      ...
      await symlink(canonical, join(claudeRoot, 'kondex-cli'))
    ...
    async () => {
      ...
      const repos = await Promise.all(
        ['one', 'two'].map(async (id) => {
          ...
          await symlink(shared, join(root, 'kondex-cli'))
  ...
  it('keeps an unreadable foreign-home placement visible without withholding the update', async () => {
    const test = await fixture()
    await test.writeSkill(join(test.homeDir, '.agents', 'skills'), test.oldMarkdown)
    const inaccessiblePath = join(test.homeDir, '.codex', 'skills', 'kondex-cli')
  ...
  it('does not lose an inaccessible known repository placement', async () => {
    ...
    const inaccessiblePath = join(repoPath, '.agents', 'skills', 'kondex-cli')
  ...
  it('does not classify an empty plugin-cache directory as a skill', async () => {
    const test = await fixture()
    await test.writeSkill(join(test.homeDir, '.agents', 'skills'), test.currentMarkdown)
    const emptyRoot = join(test.homeDir, '.codex', 'plugins', 'cache', 'vendor', 'kondex-cli')
  ...
  it('classifies exact current bytes as current when a later snapshot reuses them', async () => {
    ...
    const registry = JSON.parse(await readFile(registryPath, 'utf8'))
    registry.skills['kondex-cli'].push(snapshot(4, test.currentMarkdown))
    await writeFile(registryPath, `${JSON.stringify(registry, null, 2)}\n`)
    await test.writeSkill(join(test.homeDir, '.agents', 'skills'), test.currentMarkdown)
