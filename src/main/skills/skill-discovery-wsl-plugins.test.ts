...
describe('WSL Claude plugin skill discovery', () => {
  ...
  it('reads enabled plugin metadata and scans the selected install inside the distro', async () => {
    ...
    const cwd = '/work/orca'
    // Why: a Windows host's own Claude location says nothing about the distro's,
    // so neither variable may reach the posix scan script.
    vi.stubEnv('CLAUDE_CONFIG_DIR', 'C:\\Users\\alice\\claude-profile')
    ...
    expect(scanScript).toContain('/home/alice/.claude/skills')
    expect(scanScript).not.toContain('claude-profile')
    expect(scanScript).not.toContain('/home/alice/.hermes/skills')
