...
beforeEach(() => {
  ...
  // Keep a developer's relocated Claude profile outside the fixture scan.
...
describe('bounded concurrent skill discovery', () => {
  ...
  it('reports missing roots without walking them', async () => {
    ...
    const missing = result.sources.find((source) => source.id === 'codex-plugin-cache')
    expect(missing?.exists).toBe(false)
    expect(missing?.skippedReason).toBe('missing')
    expect(readdirCountUnder(join(home, '.codex', 'plugins', 'cache'))).toBe(0)
  })
  ...
  it('logs the roots it walked, by id, and never a filesystem path', async () => {
    const { home, noWorkspace, panes } = await buildFixture(MAX_LOGGED_ROOT_IDS)
    const repos = panes.map((path, index) => ({
      id: `repo-${index}`,
      path,
    ...
    const rootCount = 4 + 2 * (panes.length + 1)
    expect(rootCount).toBeGreaterThan(MAX_LOGGED_ROOT_IDS)
    expect(line).toContain(
      `[skills] scan roots=${rootCount} present=${3 + panes.length} walked=${rootCount} skills=${3 + panes.length}`
    )
