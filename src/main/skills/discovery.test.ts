...
beforeEach(() => {
  ...
  clearSkillRootScanCache()
  // Why: the Claude root is the only home root that env vars can move outside the
...
describe('skill discovery', () => {
  ...
  it('keeps every healthy root when one root walk is aborted', async () => {
    ...
    const stalledSkill = join(home, '.agents', 'skills', 'planning')
    ...
    unavailableRootPath = join(home, '.agents', 'skills')
  ...
  it('does not report an aborted root as missing', async () => {
    ...
    const stalledRoot = join(home, '.agents', 'skills')
  ...
  it('discovers home and repo SKILL.md packages with provider metadata', async () => {
    ...
    const sharedSkill = join(home, '.agents', 'skills', 'planning')
    ...
    await writeFile(join(sharedSkill, 'SKILL.md'), '# Planning\n\nPlan shared work.')
  ...
  it('keys every deduped root to an owning source so per-agent coverage resolves', async () => {
    ...
    // A provider alias can link to another retained provider root.
    ...
    expect(owners?.slice().sort()).toEqual(['claude', 'codex'])
  })
  ...
  it('scans only retained provider roots and shared skill locations', () => {
    const roots = buildSkillDiscoverySources({ homeDir: '/home/test', cwd: '/workspace/current' })
    const paths = roots.map((root) => root.path.replace(/\\/g, '/')).sort()
    expect(paths).toEqual(
      [
        ...
        '/workspace/current/.agents/skills',
        '/workspace/current/.claude/skills'
      ].sort()
    )
    expect(roots.find((root) => root.id === 'home-agents')).toMatchObject({
      owner: null,
      providers: ['agent-skills']
    })
    expect(roots.find((root) => root.id === 'home-codex')).toMatchObject({
      owner: 'codex',
      providers: ['codex']
    })
    expect(roots.find((root) => root.id === 'home-claude')).toMatchObject({
      owner: 'claude',
      providers: ['claude']
    })
  })
  ...
  it('discovers Skills installed in the Claude home', async () => {
    const root = await mkdtemp(join(tmpdir(), 'orca-skills-'))
    const home = join(root, 'home')
    const skillDir = join(home, '.claude', 'skills', 'social-media', 'research')
    ...
    expect(result.sources.find((source) => source.id === 'home-claude')).toMatchObject({
      owner: 'claude',
      exists: true
    })
  })

  it('discovers Claude Skills under a relocated CLAUDE_CONFIG_DIR profile', async () => {
    const root = await mkdtemp(join(tmpdir(), 'orca-skills-'))
    const home = join(root, 'home')
    const claudeHome = join(root, 'profiles', 'coder')
    const skillDir = join(claudeHome, 'skills', 'research')
    await mkdir(skillDir, { recursive: true })
    await writeFile(
      join(skillDir, 'SKILL.md'),
