...
export async function inventorySkillFreshness(args: {
  ...
  const registrations = await readBundledSkillUpdateRegistrations(
    artifacts.manifest.skills.map((skill) => skill.name),
    args
  )
  ...
  return {
    schemaVersion: 1,
    installations,
    eligibleUpdateNames: eligibleSkillUpdateNames(installations, new Set(registrations.keys())),
    scanIssues,
    scannedAt: Date.now()
  }
}
