...
describe('WslSkillInstallFilesystem', () => {
  it('allows canonical and Claude destinations inside the selected distro', async () => {
    ...
    for (const directory of ['.agents', '.claude']) {
      await filesystem.createAlias(
        ...
        `${home}\\${directory}\\skills\\linked-skill`
      )
    }
  ...
  it('does not implicitly authorize retired provider destinations', async () => {
    const home = '\\\\wsl.localhost\\Ubuntu-24.04\\home\\jin'
    const filesystem = createWslSkillInstallFilesystem({
      distro: 'Ubuntu-24.04',
      homeDirectory: home
    ...
    for (const directory of [
      ...
      ).rejects.toThrow('skill-install-wsl-path-outside-root')
    }
