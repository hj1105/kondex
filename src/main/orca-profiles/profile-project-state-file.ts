...
import { stripRetiredGlobalSettings } from '../persistence/applying-settings/terminal-settings-migrations'
...
export function readProfileState(profileId: string, userDataPath: string): TransferProfileState {
  ...
  const parsedWithRetiredFields = JSON.parse(
    readFileSync(dataFile, 'utf-8')
  ) as Partial<PersistedState> & { featureInteractionTelemetryBuckets?: unknown }
  const { featureInteractionTelemetryBuckets: _retiredInteractionBuckets, ...parsed } =
    parsedWithRetiredFields
  ...
  return rebuildRepoBackedProjectState({
    ...
    settings: isRecord(parsed.settings)
      ? { ...defaults.settings, ...stripRetiredGlobalSettings(parsed.settings) }
      : defaults.settings,
