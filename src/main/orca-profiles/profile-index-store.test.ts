...
describe('profile index store', () => {
  ...
  it('loads legacy cloud-linked profiles as local profiles without cloud fields', async () => {
    const indexPath = join(testState.dir, 'orca-profile-index.json')
    writeFileSync(
      indexPath,
      JSON.stringify({
        schemaVersion: 1,
        activeProfileId: 'legacy-cloud-profile',
        profiles: [
          {
            id: 'legacy-cloud-profile',
            name: 'Former cloud workspace',
            avatar: { kind: 'initials', initials: 'F', color: 'neutral' },
            kind: 'cloud-linked',
            createdAt: 1,
            updatedAt: 2,
            lastOpenedAt: 3,
    ...
    const { readProfileIndex } = await loadProfileIndexStore()
    ...
    expect(index?.profiles[0]).not.toHaveProperty('cloud')
    expect(index?.profiles[0]).not.toHaveProperty('unknownLegacyField')
  })
