...
export function installedElectronCandidates(
  ...
  if (platform === 'darwin') {
    return [
      ...
      joinPath(homePath, 'Applications', 'Kondex.app', 'Contents', 'MacOS', 'Kondex')
    ]
  }
  if (platform === 'win32') {
    return [
      ...(environment.LOCALAPPDATA
        ? [joinPath(environment.LOCALAPPDATA, 'Programs', 'Kondex', 'Kondex.exe')]
        : []),
      ...(environment.ProgramFiles
        ? [joinPath(environment.ProgramFiles, 'Kondex', 'Kondex.exe')]
        : [])
    ]
  }
  return ['/opt/Kondex/kondex', '/opt/kondex/kondex']
}
