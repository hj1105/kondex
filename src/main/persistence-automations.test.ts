...
describe('Store', () => {
  ...
  it('does not collect feature interactions when automations are created or manually queued', async () => {
    const store = await createStore()
    store.addRepo(makeRepo())
    const automation = store.createAutomation({
      name: 'Nightly',
    ...
    expect(persisted.ui).not.toHaveProperty('featureInteractions')
  })
