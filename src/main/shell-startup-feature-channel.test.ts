...
describePosix('epilogue under hostile user shell options', () => {
  ...
  it.each(['no_unset', 'ksh_arrays'])(
    ...
    async (option) => {
      ...
      const codexHome = join(home, 'codex-overlay')
      ...
      const { values } = await runZshPty({
        ...
        report: ['LINEINIT', 'PRECMD', 'CODEX_HOME', 'ZDOTDIR', 'HISTFILE'],
        commands: [
      ...
      expect(values.CODEX_HOME).toBe(codexHome)
