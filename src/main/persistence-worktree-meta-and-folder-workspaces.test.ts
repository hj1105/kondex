...
describe('Store', () => {
  ...
  it('reloads explicit folder workspaces in groups without a default path', async () => {
    const store = await createStore()
    const group = store.createProjectGroup({ name: 'Sources', createdFrom: 'manual' })
    const workspace = store.createFolderWorkspace({
      projectGroupId: group.id,
      folderPath: '/workspace/source'
    })
    store.flush()
    const restored = await createStore()
