...
import menuBarIconPath from '../../../resources/tray/kondex-menu-barTemplate.png?asset&asarUnpack'
import menuBarIconRetinaPath from '../../../resources/tray/kondex-menu-barTemplate@2x.png?asset&asarUnpack'
...
export type SystemTrayOptions = {
  ...
  onOpenSettings: () => void
  /** Quit Kondex for real (caller must set the quitting latch before quitting). */
  onQuit: () => void
}
...
function baseTooltip(): string {
  ...
  return devIndicator.label ? `Kondex DEV (${devIndicator.label})` : 'Kondex DEV'
}
      ...
      try {
        ...
        tray.setToolTip(
          devIndicator
            ...
            : translateMain('tray.activityWaiting', 'Kondex - activity waiting')
        )
        return
      } catch (error) {
...
export function createSystemTray(opts: SystemTrayOptions): Tray | null {
  ...
  const menu = Menu.buildFromTemplate([
    ...(devIndicator
      ...
      : []),
    {
      label: translateMain('tray.openKondex', 'Open Kondex'),
