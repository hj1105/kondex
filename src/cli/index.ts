...
// Why: the SSH relay bridge executes this CLI on the Kondex host while the
...
export async function main(
  ...
  if (argv.length === 1 && (argv[0] === '--version' || argv[0] === '-v')) {
    ...
    if (!version) {
      process.stderr.write('Could not determine the Kondex version for this build.\n')
      process.exitCode = 1
      return
    }
  ...
  try {
    ...
    // lookup so users do not get misleading "Kondex is not running" failures for
...
async function runClaudeTeams(argv: string[], cwd: string): Promise<void> {
  try {
    // Why: everything after `kondex claude-teams` belongs to Claude Code, not
    // Kondex's own flag parser, so new Claude flags work without Kondex changes.
