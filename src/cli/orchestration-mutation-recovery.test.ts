...
describe('orchestration mutation recovery', () => {
  it.each(['Orca', 'Kondex'])('leaves a read-only %s failure untouched', (product) => {
    const error = new RuntimeClientError(
      'runtime_unavailable',
      `The ${product} runtime closed the connection before responding. Restart ${product} and try again.`
    )

    expect(orchestrationMutationRecoveryError(error)).toBe(error)
  ...
  it('queries a known dispatch before issuing the keyed retry', () => {
    ...
    expect((result.data as { nextSteps?: string[] }).nextSteps).toEqual([
      ...
      'After inspecting the Dispatch, if keyed recovery is still needed, run orca orchestration worker-start --task task_1 --retry-request request_1. --retry-request reuses the same operation identity so Kondex can replay, join, or safely recover it without starting a separate duplicate.'
    ])
  })
  ...
  it('renders the exact executable and safely quotes original arguments', () => {
    ...
    expect((result.data as { nextSteps?: string[] }).nextSteps).toEqual([
      ...
      "After inspecting the Dispatch, if keyed recovery is still needed, run orca-dev orchestration worker-start --task 'task 3' --comment 'literal $(do-not-run)' --retry-request request_3. --retry-request reuses the same operation identity so Kondex can replay, join, or safely recover it without starting a separate duplicate."
    ])
