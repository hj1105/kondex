  ...
  try {
    ...
    if (!metadata || !findTransport(metadata, 'unix', 'named-pipe') || !metadata.authToken) {
      throw new RuntimeClientError(
        'runtime_unavailable',
        `Kondex runtime metadata is incomplete at ${metadataPath}`
      )
    }
    return metadata
  } catch (error) {
    ...
    throw new RuntimeClientError(
      'runtime_unavailable',
      `Could not read Kondex runtime metadata at ${metadataPath}. Start the Kondex app first.`
    )
  }
}
...
export function getDefaultUserDataPath(
  ...
  // Why: in dev mode (and for parallel Kondex instances), the Electron app writes
  ...
  if (platform === 'win32') {
    ...
    if (!appData) {
      throw new RuntimeClientError(
        'runtime_unavailable',
        'APPDATA is not set, so the Kondex runtime metadata path cannot be resolved.'
      )
    }
    return join(appData, 'kondex')
  }
  ...
  return join(process.env.XDG_CONFIG_HOME || join(homeDir, '.config'), 'kondex')
}
