...
describe('orchestration gate commands carry caller identity', () => {
  ...
  it.each(['Orca', 'Kondex'])(
    'reports keyed recovery when a %s mutation connection drops',
    async (product) => {
      process.env.ORCA_TERMINAL_HANDLE = 'term_coord'
      callMock
        ...
        .mockRejectedValueOnce(
          new RuntimeClientError(
            'runtime_unavailable',
            `The ${product} runtime closed the connection before responding. Restart ${product} and try again. Orchestration mutation request ID: mutation_1.`,
            {
      ...
      expect(output.error.message).not.toMatch(/restart (?:Orca|Kondex)/i)
      expect(output.error.data).toMatchObject({
        ...
        recovery: expect.objectContaining({
          ...
          retryCommand: expect.arrayContaining(['orca', '--retry-request', 'mutation_1'])
        }),
        failedStage: 'dispatch_input',
