 ...
 * GUI flow drives this via a browser Kondex can't reach on a headless host.
 */
...
async function assertAccountImportSupported({ client }: HandlerContext): Promise<void> {
  ...
  if (!status.result.capabilities?.includes(ACCOUNT_IMPORT_RUNTIME_CAPABILITY)) {
    throw new RuntimeClientError(
      ...
      'The running Kondex runtime is too old to add accounts from the CLI. Update or restart Kondex and try again.'
    )
  }
}

/** CLI handlers for `kondex account add [--agent claude|codex]` and `kondex account list`. */
export const ACCOUNT_HANDLERS: Record<string, CommandHandler> = {
  'account add': async (ctx) => {
    ...
    rejectAccountRemoteSelectionFlags(ctx, 'kondex account add')
  ...
  'account list': async (ctx) => {
    rejectAccountRemoteSelectionFlags(ctx, 'kondex account list')
