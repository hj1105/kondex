...
export function formatCommandHelp(spec: CommandSpec): string {
  const lines = [`kondex ${spec.path.join(' ')}`, '', `Usage: ${spec.usage}`, '', spec.summary]
...
export function formatGroupHelp(specs: CommandSpec[], group: string): string {
  ...
  const lines = [
    `kondex ${group}`,
    '',
    `Usage: kondex ${group} <command> [options]`,
    '',
    'Commands:'
  ]
  ...
  lines.push('', `Run \`kondex ${group} <command> --help\` for command-specific usage.`)
  return lines.join('\n')
}
...
export function formatFlagHelp(flag: string): string {
  const helpByFlag: Record<string, string> = {
    ...
    comment: '--comment <text>       Comment stored in Kondex metadata',
    ...
    activate: '--activate             Reveal the new worktree in the Kondex app',
    ...
    'display-name': '--display-name <name>  Override the Kondex display name',
    ...
    focus: '--focus                Reveal the created terminal session in Kondex',
    ...
    page: '--page <id>            Stable browser page id from `kondex tab list --json`',
  ...
  if (flag === 'current') {
    return '--current              Use the current Kondex worktree'
  }
  if (flag === 'comments') {
    return '--comments             Include threaded comments'
  }
  if (flag === 'children') {
