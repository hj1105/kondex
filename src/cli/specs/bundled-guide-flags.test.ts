...
// Retain explicit SSH shim examples alongside Kondex command names.
const CLI_INVOCATION = /(?:^|[\s`(])(?:kondex(?:-dev)?|KONDEX|orca(?:-dev|-ide)?|ORCA)\s+([^\n`]*)/g
...
describe('bundled skill guides', () => {
  it.each(['kondex', 'kondex-dev', 'KONDEX', 'orca', 'orca-dev', 'orca-ide', 'ORCA'])(
    'recognizes %s examples without matching a longer executable name',
    (command) => {
      expect([...`${command} orchestration check --peek`.matchAll(CLI_INVOCATION)][0]?.[1]).toBe(
        'orchestration check --peek'
      )
      expect([...`other-${command} orchestration check --peek`.matchAll(CLI_INVOCATION)]).toEqual(
        []
      )
    }
  )
