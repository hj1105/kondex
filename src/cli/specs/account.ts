...
export const ACCOUNT_COMMAND_SPECS: CommandSpec[] = [
  {
    path: ['account', 'add'],
    summary: 'Add a managed Claude or Codex account by signing in on this Kondex host',
    usage: 'kondex account add [--agent claude|codex] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'agent'],
    notes: [
      'Runs the agent login (`claude login` / `codex login`) in this terminal, then registers the account with the local Kondex runtime.',
      ...
      '--agent defaults to claude. Requires the Kondex runtime to be running on this machine.'
    ],
    examples: ['kondex account add', 'kondex account add --agent codex']
  },
  {
    path: ['account', 'list'],
    summary: 'List managed Claude and Codex accounts on this Kondex host',
    usage: 'kondex account list [--json]',
