...
export const CORE_COMMAND_SPECS: CommandSpec[] = [
  {
    path: ['open'],
    summary: 'Launch Kondex and wait for the runtime to be reachable',
    usage: 'kondex open [--json]',
    allowedFlags: [...GLOBAL_FLAGS],
    examples: ['kondex open', 'kondex open --json']
  },
  ...SERVE_COMMAND_SPECS,
  {
    path: ['status'],
    summary: 'Show app/runtime/graph readiness',
    usage: 'kondex status [--json]',
    allowedFlags: [...GLOBAL_FLAGS],
    examples: ['kondex status', 'kondex status --json']
  },
  {
    path: ['claude-teams'],
    argumentMode: 'passthrough',
    summary: 'Start Claude Code Agent Teams in the current Kondex terminal',
    usage: 'kondex claude-teams [claude args...]',
    allowedFlags: [...GLOBAL_FLAGS],
    notes: [
      'Passes all following arguments through to Claude Code after enabling Agent Teams native panes.',
      'Must be run from inside a Kondex terminal. Starts Claude Code Agent Teams in the current pane and opens teammates as native Kondex splits.'
    ],
    examples: ['kondex claude-teams', 'kondex claude-teams --resume <session-id>']
  },
  {
    path: ['repo', 'list'],
    summary: 'List repos registered in Kondex',
    usage: 'kondex repo list [--json]',
    allowedFlags: [...GLOBAL_FLAGS]
  },
  {
    path: ['repo', 'add'],
    summary: 'Add a project to Kondex by filesystem path',
    usage: 'kondex repo add --path <path> [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'path']
  },
  {
    path: ['repo', 'show'],
    summary: 'Show one registered repo',
    usage: 'kondex repo show --repo <selector> [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'repo']
  },
  {
    path: ['repo', 'set-base-ref'],
    summary: "Set the repo's default base ref for future worktrees",
    usage: 'kondex repo set-base-ref --repo <selector> --ref <ref> [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'repo', 'ref']
  },
  {
    path: ['repo', 'search-refs'],
    summary: 'Search branch/tag refs within a repo',
    usage: 'kondex repo search-refs --repo <selector> --query <text> [--limit <n>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'repo', 'query', 'limit']
  },
  {
    path: ['worktree', 'list'],
    summary: 'List Kondex-managed worktrees',
    usage: 'kondex worktree list [--repo <selector>] [--limit <n>] [--json]',
  ...
  },
  {
    ...
    usage: 'kondex worktree show --worktree <selector> [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'worktree']
  },
  {
    path: ['worktree', 'current'],
    summary: 'Show the Kondex-managed worktree for the current directory',
    usage: 'kondex worktree current [--json]',
    allowedFlags: [...GLOBAL_FLAGS],
    notes: [
      'Resolves the current shell directory to a path: selector so agents can target the enclosing Kondex worktree without spelling out $PWD.'
    ],
    examples: ['kondex worktree current', 'kondex worktree current --json']
  },
  {
    path: ['worktree', 'create'],
    summary: 'Create a new Kondex-managed worktree',
    usage:
    ...
    notes: [
      'This creates a new checkout. For a fresh agent in an existing worktree, use `kondex terminal create --worktree active --command "codex"` instead.',
      'By default, Kondex records the new worktree as a child of the caller context when it can infer one from the Kondex terminal or current directory.',
      'If --repo is omitted, Kondex infers the repo from the current Kondex-managed worktree.',
      'Use --project with --host to create on a ready project host setup without spelling the backing repo id.',
      '--host runtime:<environment-id> creates on that paired Kondex server; use the id from `kondex environment list`, not the environment name.',
      'For related work, use the inferred parent or pass --parent-worktree active, folder:<id>, or worktree:<worktreeId> to make the relationship explicit. Worktree ids are the full <repo-id>::<path> values returned by `kondex worktree list --json`.',
      'Use --no-parent when the new worktree should be independent of the current context.',
      '--no-parent only affects Kondex lineage; omit --base-branch to use the repo default base, or pass the default base ref explicitly for independent top-level work.',
      'By default this creates the worktree and its first terminal without switching the active Kondex view.',
    ...
    examples: [
      'kondex worktree create --name agent-task --agent codex --prompt "hi" --json',
      'kondex worktree create --repo id:<repoId> --name related-task --json',
      'kondex worktree create --project github:stablyai/orca --host runtime:03ef704c-b180-4b10-998d-e28fbd5de9a3 --name benchmark --json',
      'kondex worktree create --repo id:<repoId> --name agent-task --agent codex --prompt "hi" --json',
      ...
      'kondex worktree create --repo id:<repoId> --name independent-task --no-parent --json'
    ]
  },
  {
    path: ['worktree', 'set'],
    summary: 'Update Kondex metadata for a worktree',
    usage:
  ...
  },
  {
    ...
    destructive: true,
    summary: 'Remove a worktree from Kondex and git',
    usage: 'kondex worktree rm --worktree <selector> [--force] [--run-hooks] [--json]',
    ...
    notes: [
      ...
      'For Git worktrees, removal also attempts to delete the checked-out local branch, with or without --force. Kondex retains branches it knows predated the worktree and any branch whose changes it cannot prove are already merged.'
    ]
  },
  {
    path: ['worktree', 'ps'],
    summary: 'Show a compact orchestration summary across worktrees',
    usage: 'kondex worktree ps [--limit <n>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'limit'],
    notes: [...WORKTREE_LISTING_SCOPE_NOTES]
  },
  {
    path: ['terminal', 'list'],
    summary: 'List live Kondex-managed terminals',
    usage:
  ...
  },
  {
    ...
    usage: 'kondex terminal show [--terminal <handle>] [--json]',
  ...
  },
  {
    ...
    usage:
      'kondex terminal read [--terminal <handle>] [--cursor <n>] [--limit <n>] [--screen] [--json]',
    ...
    examples: [
      ...
      'kondex terminal read --terminal term_abc123 --cursor 42 --limit 1000 --json',
      'kondex terminal read --terminal term_abc123 --screen --json'
    ]
  },
  {
    path: ['terminal', 'send'],
  ...
  },
  {
    ...
    usage: 'kondex terminal stop --worktree <selector> [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'worktree'],
    notes: [
  ...
  },
  {
    ...
    examples: [
      ...
      'kondex terminal create --worktree path:/projects/myapp --title "RUNNER" --command "opencode"',
      'kondex terminal create --worktree path:/projects/myapp --command "opencode" --focus'
    ]
  },
  {
    ...
    usage: 'kondex terminal switch [--terminal <handle>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'terminal'],
    examples: ['kondex terminal switch --terminal term_abc123']
  },
  TERMINAL_CLOSE_COMMAND_SPEC,
  {
    path: ['terminal', 'rename'],
    summary: 'Set or clear the title of a terminal tab',
    usage: 'kondex terminal rename [--terminal <handle>] [--title <text>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'terminal', 'title'],
    notes: ['Omit --title or pass an empty string to reset to the auto-generated title.'],
    examples: [
      'kondex terminal rename --terminal term_abc123 --title "RUNNER"',
      'kondex terminal rename --terminal term_abc123 --json'
    ]
  },
  {
    path: ['terminal', 'split'],
    ...
    usage:
      'kondex terminal split [--terminal <handle>] [--direction horizontal|vertical] [--command <text>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'terminal', 'direction', 'command'],
    examples: [
      'kondex terminal split --terminal term_abc123 --direction horizontal --json',
      'kondex terminal split --terminal term_abc123 --command "codex"'
    ]
  }
]
