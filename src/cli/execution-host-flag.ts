...
export type HostFlagRoutingSelection = {
  // Why: SSH targets live in the running Kondex host, not on disk, so enumerating them needs a
...
export async function resolveHostFlagEnvironmentId(
  ...
  if (!environment) {
    ...
    throw new RuntimeClientError(
      'invalid_argument',
      `Unknown Kondex server in --host ${host.id}: no paired Kondex server is named or has id ${host.environmentId}.`,
      {
        ...
        nextSteps: [
          ...
          'Run `kondex environment list` to see paired Kondex servers.',
  ...
  if (selection.pairingCode) {
    throw new RuntimeClientError(
      'invalid_argument',
      `--host ${host.id} already selects a paired Kondex server; use either --host runtime:<id> or --pairing-code, not both.`
    )
  }
  if (selection.environmentSelector) {
    ...
    if (selected.id !== environment.id) {
      throw new RuntimeClientError(
        'invalid_argument',
        `--host ${host.id} and ${selection.environmentSelector.label} ${selection.environmentSelector.value} name different Kondex servers.`
      )
    }
  }
...
function assertEnvironmentNameUnambiguous(
  ...
  throw new RuntimeClientError(
    'invalid_argument',
    `Ambiguous Kondex server in ${flag}: ${ambiguous.length} paired servers are named ${name}. Use the environment id.`,
    {
...
export async function assertEnvironmentSelectorResolvable(
  ...
  throw new RuntimeClientError(
    'invalid_argument',
    `Unknown Kondex server in --environment ${selector}: no paired Kondex server is named or has id ${selector}.`,
    {
      knownEnvironments: environments,
      knownSshTargets: sshTargets,
      nextSteps: [
        ...crossKindNextSteps(selector, { environments, sshTargets }, 'environment'),
        'Run `kondex host list` to see every machine you can target and the flag for each.'
      ]
    }
  )
}
