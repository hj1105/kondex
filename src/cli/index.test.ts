...
describe('unknown command surfaces a suggestion', () => {
  ...
  it('prints did-you-mean for a near-miss command and exits non-zero', async () => {
    ...
    expect(stderr).toContain('kondex worktree')
  })
...
describe('unknown help command surfaces a suggestion', () => {
  it.each([
    ...
    expect(process.exitCode).toBe(1)
    expect(logSpy.mock.calls.flat().join('\n')).toContain('Did you mean: kondex worktree')
    logSpy.mockRestore()
    process.exitCode = 0
  })
})
...
describe('orca root help', () => {
  ...
  it('advertises host-local account management', async () => {
    ...
    expect(logSpy.mock.calls.flat().join('\n')).toContain(
      'account add               Add a managed Claude or Codex account on this Kondex host'
    )
    expect(logSpy.mock.calls.flat().join('\n')).toContain(
      'account list              List managed Claude and Codex accounts on this Kondex host'
    )
    logSpy.mockRestore()
  })
  ...
  it('advertises computer-use capabilities discovery', async () => {
    ...
    expect(logSpy.mock.calls[0][0]).toContain(
      'orchestration worker-start Start a supervised worker locally or on a connected Kondex server'
    )
    expect(logSpy.mock.calls[0][0]).toContain(
  ...
  it('omits retired Linear commands and rejects their help without RPC', async () => {
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {})
    ...
    try {
      ...
      for (const command of [['linear'], ['linear', 'issue'], ['linear', 'search']]) {
        logSpy.mockClear()
        errSpy.mockClear()
        await main([...command, '--help'], '/tmp/repo')
        expect([...logSpy.mock.calls, ...errSpy.mock.calls].flat().join('\n')).toContain(
          'Unknown command: linear'
        )
        expect(process.exitCode).toBe(1)
      }
      expect(callMock).not.toHaveBeenCalled()
  ...
  it('omits retired Linear linking while retaining issue linking in worktree help', async () => {
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {})
    logSpy.mockClear()

    await main(['worktree', 'create', '--help'], '/tmp/repo')

    expect(String(logSpy.mock.calls[0][0])).not.toContain('--linear-issue')
    expect(String(logSpy.mock.calls[0][0])).toContain('--issue')

    logSpy.mockClear()
    await main(['worktree', 'set', '--help'], '/tmp/repo')

    const setHelp = String(logSpy.mock.calls[0][0])
    expect(setHelp).not.toContain('--linear-issue')
    expect(setHelp).toContain('--issue')
    expect(callMock).not.toHaveBeenCalled()
  })
  ...
  it('hides removed parent-workspace help and scopes create parent selectors', async () => {
    ...
    expect(createHelp).toContain(
      '--no-parent only affects Kondex lineage; omit --base-branch to use the repo default base'
    )
