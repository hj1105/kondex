...
describe('getForegroundProcessName', () => {
  ...
  it('does not promote retired omp/pi descendants over a shell fallback', async () => {
    await withProcessPlatform('linux', async () => {
      mockExecFile((_command, args) => {
        if (args[0] === '-axo') {
          return {
  ...
  it('keeps a raw pi foreground name without promoting its retired omp parent', async () => {
    await withProcessPlatform('linux', async () => {
      mockExecFile((_command, args) => {
        if (args[0] === '-axo') {
          return {
  ...
  it('returns a recognized Codex fallback without a process-table scan', async () => {
    mockExecFile(() => new Error('unexpected process-table scan'))

    await expect(getForegroundProcessName(100, 'codex')).resolves.toBe('codex')
    expect(execFileMock).not.toHaveBeenCalled()
  })

  it('does not promote a Windows pi fallback to its retired omp wrapper', async () => {
    await withProcessPlatform('win32', async () => {
      mockWindowsProcessTable([
        { pid: 100, ppid: 99, name: 'powershell.exe', commandLine: 'powershell.exe' },
  ...
  it.each([
    ...
    ['/opt/homebrew/bin/pi', '/opt/homebrew/bin/pi']
  ])(
    'normalizes only recognized fallback %s without table confirmation',
    async (fallback, expected) => {
      await withProcessPlatform('linux', async () => {
        ...
        await expect(getForegroundProcessName(100, fallback)).resolves.toBe(expected)
      })
    }
  )
