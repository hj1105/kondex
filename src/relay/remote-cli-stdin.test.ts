...
describe('shouldReadRemoteCliStdin', () => {
  it('does not wait for stdin for retired Linear body-file requests', () => {
    expect(shouldReadRemoteCliStdin(['linear', 'comment', 'add', '--body-file', '-'])).toBe(false)
    expect(shouldReadRemoteCliStdin(['linear', 'create', '--body-file=-'])).toBe(false)
