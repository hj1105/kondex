...
describe('PtyHandler', () => {
  ...
  it.each(['codex', 'claude'])(
    'passes PTY and explicit %s launch identity to env augmenters',
    async (launchAgent) => {
      const seenContexts: {
        id: string
        paneKey?: string
  ...
  it('does not promote a retired launch identity to an agent overlay', async () => {
    const identities: (string | undefined)[] = []
    handler.addEnvAugmenter((ctx) => {
      identities.push(ctx.launchAgent)
      return {}
    })
    await dispatcher.callRequest('pty.spawn', { launchAgent: 'pi' })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      try {
        ...
        const wrapper = readFileSync(rcfile, 'utf8')
        ...
        expect(wrapper).toContain('export PATH="${ORCA_REMOTE_CLI_BIN_DIR}:$PATH"')
        expect(wrapper).not.toContain('export CODEX_HOME=')
        expect(wrapper).not.toContain('ORCA_PI_CODING_AGENT_DIR')
        expect(wrapper).not.toContain('export OPENCODE_CONFIG_DIR=')
        expect(wrapper).not.toContain('command omp --extension')
      } finally {
