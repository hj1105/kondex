...
describe('getRelayShellLaunchConfig', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      const config = getRelayShellLaunchConfig('/bin/zsh', {
        HOME: homeDir,
        ORCA_REMOTE_CLI_BIN_DIR: '/tmp/kondex-relay-bin'
      })
  ...
  it.skipIf(process.platform === 'win32')(
    'restores the remote CLI path without replacing host-owned Codex settings',
    () => {
      const config = getRelayShellLaunchConfig('/bin/zsh', {
        HOME: homeDir,
        ORCA_REMOTE_CLI_BIN_DIR: '/tmp/kondex-relay-bin'
      ...
      expect(config.env.ZDOTDIR).toBe(zshRoot)
      expect(config.env.ORCA_SHELL_FEATURES).toBe('overlay,history,markers')
      expect(zshenv).toContain('export PATH="${ORCA_REMOTE_CLI_BIN_DIR}:$PATH"')
      expect(zshenv).not.toContain('export CODEX_HOME=')
    }
  )

  it
    .skipIf(process.platform === 'win32')
    .each(['ORCA_OPENCODE_CONFIG_DIR', 'ORCA_MIMOCODE_HOME', 'ORCA_OMP_STATUS_EXTENSION'])(
    'does not activate a zsh wrapper for retired %s alone',
    (key) => {
      expect(
        getRelayShellLaunchConfig('/bin/zsh', { HOME: homeDir, [key]: '/retired/overlay' })
      ).toEqual({
        args: ['-l'],
        env: {},
        supportsReadyMarker: false
      })
      expect(existsSync(join(homeDir, '.orca-relay', 'shell-ready', 'zsh'))).toBe(false)
    }
  )
