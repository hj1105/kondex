...
describe('one Unix startup dialect', () => {
  ...
  it('clears an agent draft prefill variable with the portable teardown', () => {
    const original = TUI_AGENT_CONFIG.claude
    TUI_AGENT_CONFIG.claude = {
      ...original,
      draftPromptFlag: undefined,
      draftPromptEnvVar: 'KONDEX_TEST_PREFILL'
    }
    try {
      ...
      expect(plan?.launchCommand).toBe(
        `claude; command test -n "$fish_pid" && set --erase -g KONDEX_TEST_PREFILL; command test -z "$fish_pid" && unset KONDEX_TEST_PREFILL; true`
      )
      expect(plan?.env?.KONDEX_TEST_PREFILL).toBe('hello')
    } finally {
      TUI_AGENT_CONFIG.claude = original
    }
  })
})
