...
// Eligibility and explanation copy share the canonical/alias placement boundary.
...
export function buildTargetedSkillUpdateCommand(names: readonly string[]): string | null {
  ...
  return canonicalNames
    ? `kondex skills update ${canonicalNames.map((name) => `--skill ${name}`).join(' ')}`
    : null
}

// Native updates report structured outcomes and re-scan disk before settling.
export type SkillUpdateRun =
  | { state: 'idle' }
  // Keep the writer reserved until cancellation and the final re-scan settle.
