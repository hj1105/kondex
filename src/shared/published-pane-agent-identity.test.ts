...
describe('resolvePublishedPaneAgentIdentity', () => {
  describe('a task title cannot name the pane', () => {
    ...
    it.each([
      ['Switch Claude and Codex off the load balancer… - grok', 'codex'],
  ...
  describe('title is the last resort, and the parser is what makes that safe', () => {
    ...
    it('does not infer a retired owner from a task title naming multiple agents', () => {
      expect(
        resolve({ title: 'Switch Claude and Codex off the load balancer… - grok' })
      ).toBeUndefined()
    })
