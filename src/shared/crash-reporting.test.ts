...
describe('crash-reporting shared helpers', () => {
  ...
  it('caps formatted reports to the crash endpoint limit', () => {
    ...
    expect(text).toContain('[Crash report truncated to fit the local copy limit.]')
  })

  it('formats uncaptured crash reports so users can still copy them from Help', () => {
  ...
  it('keeps a user note longer than the 240-char detail cap intact', () => {
    // Long prose notes must not be cut by the compact structured-detail budget.
    const note =
      `My workspace is open. ${'The Claude terminal never came back. '.repeat(20)}`.trim()
  ...
  it('keeps user notes when the report is truncated to the local copy cap', () => {
