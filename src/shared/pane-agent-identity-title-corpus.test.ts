...
function kondexAppSupportCandidates(): string[] {
  if (process.platform === 'darwin') {
    return [join(homedir(), 'Library', 'Application Support', 'Kondex')]
  }
  if (process.platform === 'win32') {
    return [join(process.env.APPDATA ?? join(homedir(), 'AppData', 'Roaming'), 'Kondex')]
  }
  return [
    join(process.env.XDG_CONFIG_HOME ?? join(homedir(), '.config'), 'Kondex'),
    join(process.env.XDG_DATA_HOME ?? join(homedir(), '.local', 'share'), 'Kondex')
  ]
}
...
function loadRecordedTitleCorpus(): { checkpointCount: number; titles: string[] } | null {
  const root = kondexAppSupportCandidates()
  ...
  const FIXTURES: readonly { name: string; title: string; expected: TuiAgent | null }[] = [
    {
      ...
      title: 'STA-4011 Linux Antigravity Commit Messages - codex',
      expected: 'codex'
    },
    {
    ...
    },
    {
      ...
      title: 'Compare Antigravity with Gemini 3.7 Flash… - codex',
      expected: 'codex'
    },
    ...
    {
      name: 'unknown vendor markers do not establish identity',
      title: '✦ two sigils',
      expected: null
    },
    {
      name: 'ordinary mentions of both supported agents are not identity',
      title: 'compare claude with codex',
      expected: null
    },
    { name: 'retired Pi title is not identity', title: 'pi', expected: null },
    { name: 'retired OMP title is not identity', title: 'omp ready', expected: null },
    {
