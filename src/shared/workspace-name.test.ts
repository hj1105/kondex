...
describe('getLinkedWorkItemWorkspaceName', () => {
  ...
  it('keeps a resolved GitLab title without duplicating its issue prefix', () => {
    expect(
      getLinkedWorkItemWorkspaceName({
        type: 'issue',
        provider: 'gitlab',
        number: 7,
        title: 'Issue #7: Fix flaky import'
      })
    ).toEqual({
      displayName: 'Fix flaky import',
      seedName: 'fix-flaky-import'
    })
  })
})
...
describe('getWorkspaceIntentName', () => {
  ...
  it('uses a GitLab issue number without duplicating it in the subject', () => {
    expect(
      getWorkspaceIntentName({
        workItem: {
          type: 'issue',
      ...
      displayName: 'Issue 7 Fix Flaky Import',
      seedName: 'issue-7-fix-flaky-import'
    })
  })
