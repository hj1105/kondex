...
describe('normalizeFolderWorkspaces host attribution', () => {
  it.each([null, 'ssh-group'])(
    'restores an explicit folder path without a group default (%s)',
    (connectionId) => {
      const [workspace] = normalizeFolderWorkspaces(
        [
          { id: 'explicit-folder', projectGroupId: folderGroup.id, folderPath: '/workspace/source' }
        ],
        [{ ...folderGroup, parentPath: null, connectionId }]
      )
      expect(workspace).toMatchObject({ folderPath: '/workspace/source', connectionId })
      expect(workspace?.executionHostId).toBeUndefined()
    }
  )

  it('still rejects missing groups and folders without any path', () => {
    expect(
      normalizeFolderWorkspaces(
        [
          { id: 'orphan', projectGroupId: 'absent', folderPath: '/workspace/source' },
          { id: 'pathless', projectGroupId: folderGroup.id }
        ],
        [{ ...folderGroup, parentPath: null }]
      )
    ).toEqual([])
  })
