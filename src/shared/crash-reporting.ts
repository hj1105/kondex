...
// `sent` is retained only so profiles written by older Orca builds remain readable.
...
// User notes need a prose budget separate from the compact structured detail fields.
...
const FORMATTED_REPORT_TRUNCATION_SUFFIX =
  '\n\n[Crash report truncated to fit the local copy limit.]'
...
function truncateFormattedCrashReport(text: string): string {
  ...
  // Keep clipboard writes bounded while retaining the most useful leading context.
