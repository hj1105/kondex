...
describe('buildAgentResumeStartupPlan claude selector guard', () => {
  ...
  it('does not apply Claude-specific argument cleanup to Codex', () => {
    const restored = buildAgentResumeStartupPlan({
      agent: 'codex',
      providerSession,
      cmdOverrides: {},
      agentArgs: '--profile work',
      platform: 'linux'
    })
    expect(restored?.launchCommand).toBe(`codex '--profile' 'work' 'resume' '${SESSION_ID}'`)
  })
