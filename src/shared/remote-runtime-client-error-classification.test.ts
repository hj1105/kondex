...
describe('remote runtime client error classification', () => {
  it.each(['Orca', 'Kondex'])('preserves recovery and code precedence for %s peers', (brand) => {
    for (const message of [
      ...
      const wrapped = `Error invoking remote method 'runtimeEnvironments:call': ${message}`
      expect(isRecoverableRemoteRuntimeConnectionError({ message: wrapped })).toBe(true)
      expect(
        isRecoverableRemoteRuntimeConnectionError({ code: 'unauthorized', message: wrapped })
      ).toBe(false)
    }
  })
