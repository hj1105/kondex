...
describe('content-pack manifest contributions', () => {
  ...
  it('rejects removed VM recipes even with a valid relative path', () => {
    expect(
      parsePluginManifest(manifest({ vmRecipes: [{ path: 'recipes/cloud.json' }] }))
    ).toMatchObject({
      ok: false,
      error: expect.stringContaining('Unrecognized key')
    })
  ...
  it.each([
    ...
    ['agent profile UNC path', { agents: [{ path: '\\\\server\\profile.json' }] }],
