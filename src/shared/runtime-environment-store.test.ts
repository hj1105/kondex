...
describe('runtime environment store', () => {
  ...
  it('strips retired provisioner-owned environments before they can be resolved', () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'orca-runtime-env-store-retired-'))
    tempDirs.push(userDataPath)
    writeFileSync(
        ...
        environments: [
          {
            ...
            endpoints: [
              {
                id: 'ws-retired-runtime',
                kind: 'websocket',
                label: 'WebSocket',
                endpoint: 'ws://127.0.0.1:6768',
                deviceToken: 'retired-device-token',
                publicKeyB64: Buffer.from(new Uint8Array(32).fill(1)).toString('base64')
              }
            ],
            preferredEndpointId: 'ws-retired-runtime'
          }
        ]
      })
    )
