...
describe('withRemoteRuntimeTailscaleHint', () => {
  it.each(['Orca', 'Kondex'])(
    'keeps connectivity guidance and raw messages for %s peers',
    (brand) => {
      for (const message of [
        `Could not connect to the remote ${brand} runtime.`,
        `Remote ${brand} runtime closed the connection.`,
        `Timed out waiting for the remote ${brand} runtime to respond.`,
        `Timed out while connecting to the remote ${brand} runtime.`
      ]) {
        const publicResult = withRemoteRuntimeTailscaleHint(message, 'ws://192.168.1.10:6768')
        expect(publicResult.startsWith(message)).toBe(true)
        expect(publicResult).toContain('connect both devices to Tailscale')
        expect(withRemoteRuntimeTailscaleHint(publicResult, 'ws://192.168.1.10:6768')).toBe(
          publicResult
        )
        expect(withRemoteRuntimeTailscaleHint(message, 'wss://host.ts.net')).toContain(
          'Funnel reverted to tailnet-only'
        )
      }
      const auth = `Remote ${brand} runtime rejected the pairing token.`
