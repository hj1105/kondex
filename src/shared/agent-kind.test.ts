...
import { AGENT_KIND_VALUES, agentKindSchema } from './agent-launch-context'
import { TUI_AGENT_CONFIG } from './tui-agent-config'
import type { TuiAgent } from './tui-agent'

describe('tuiAgentToAgentKind', () => {
  it('maps every shipped TuiAgent to a concrete launch kind', () => {
  ...
  it('keeps concrete launch kinds in exact sync with shipped TuiAgents', () => {
    const agents = Object.keys(TUI_AGENT_CONFIG) as TuiAgent[]
...
describe('agentKindToTuiAgent', () => {
  it('round-trips every shipped TuiAgent through its launch kind', () => {
    const agents = Object.keys(TUI_AGENT_CONFIG) as TuiAgent[]
    for (const agent of agents) {
